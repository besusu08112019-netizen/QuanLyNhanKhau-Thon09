(function () {
  ...
  async function ensureCatalogs(force = false) {
    ...
    if (unitFilter) {
      unitFilter.innerHTML = '<option value="mau">mẫu</option>';
      unitFilter.value = 'mau';
      unitFilter.disabled = true;
    }
    const unitInput = $('#agriculturalLandUnitInput');
    if (unitInput) {
      unitInput.innerHTML = '<option value="mau">mẫu</option>';
      unitInput.value = 'mau';
  ...
  async function renderDashboard() {
    ...
    const cards = [
      ...
      ['Đất giao đã chuyển đổi', m.converted_area, 'fa-right-left'],
  ...
  function renderCharts(charts, unit) {
    ...
    host.innerHTML = [
      ...
      chartCard('Đất giao đã chuyển đổi cơ cấu sản xuất', charts.converted_area || [], unit),
    ].join('');
  }
  ...
  function fallbackText(value) {
    ...
    return text ? esc(text) : '<span class="agri-land-muted">Chưa cập nhật</span>';
  }

  function statusClass(row) {
    const status = String(row.status || row.status_code || row.status_label || '').toUpperCase();
    return status.includes('ACTIVE') || status.includes('ĐANG') || status.includes('HOẠT') ? 'is-active' : 'is-muted';
  }
  ...
  function renderDetailProfile(row) {
    ...
    const usageBody = usage.length
      ...
      : '<div class="agri-land-empty"><i class="fa-solid fa-circle-info"></i> Chưa cập nhật cơ cấu sử dụng đất</div>';
    return '<article class="agri-land-profile"><header class="agri-land-profile-hero"><div class="agri-land-hero-icon"><i class="fa-solid fa-map-location-dot"></i></div><div><span class="agri-land-eyebrow">Hồ sơ khu đất nông nghiệp</span><h3>' + esc(row.zone_name || row.zone_code || 'Khu đất') + '</h3><div class="agri-land-badges"><span class="agri-land-badge is-code"><i class="fa-solid fa-barcode"></i> Mã: ' + esc(row.zone_code || 'Chưa cập nhật') + '</span><span class="agri-land-badge is-year"><i class="fa-solid fa-calendar-days"></i> Năm: ' + esc(row.report_year || 'Chưa cập nhật') + '</span><span class="agri-land-badge ' + statusClass(row) + '"><i class="fa-solid fa-circle-check"></i> ' + esc(row.status_label || 'Chưa cập nhật') + '</span></div></div></header>'
      + '<section class="agri-land-overview"><div class="agri-land-section-head"><span><i class="fa-solid fa-chart-simple"></i></span><div><h4>Quỹ đất</h4><p>Tổng quan nhanh theo đơn vị mẫu</p></div></div><div class="agri-land-stat-grid">'
        + statCard('Tổng diện tích', area(row.total_area, row.unit), 'fa-ruler-combined', 'total')
        + statCard('Đất giao dài hạn', area(row.long_term_allocated_area, row.unit), 'fa-file-signature', 'green')
        + statCard('Đất công ích', area(row.public_utility_area, row.unit), 'fa-landmark', 'blue')
        + statCard('Đất thuê', area(row.leased_area, row.unit), 'fa-handshake', 'orange')
        + statCard('Đất chuyển đổi', area(row.converted_area, row.unit), 'fa-right-left', 'purple')
        + '</div></section>'
      + '<div class="agri-land-business-note"><i class="fa-solid fa-circle-info"></i><span>Đất chuyển đổi nằm trong đất giao dài hạn, không phải quỹ đất phát sinh thêm.</span></div>'
      + '<div class="agri-land-profile-grid">'
      + detailSection('Cơ cấu sử dụng đất', 'fa-seedling', usageBody, 'is-wide')
      + detailSection('Thông tin chung', 'fa-circle-info', '<div class="agri-land-meta-grid">'
        + metaTile('Mã khu', fallbackText(row.zone_code), 'fa-barcode')
        + metaTile('Tên khu', fallbackText(row.zone_name), 'fa-map')
        + metaTile('Năm thống kê', fallbackText(row.report_year), 'fa-calendar-days')
        + metaTile('Trạng thái', '<span class="agri-land-status ' + statusClass(row) + '">' + esc(row.status_label || 'Chưa cập nhật') + '</span>', 'fa-circle-check')
        + '</div>')
      + detailSection('Thông tin mở rộng', 'fa-clipboard-list', infoRows([
        ['Kênh mương', fallbackText(row.irrigation_note)],
        ['Tổ sản xuất', fallbackText(row.production_group_name)],
        ['Cây chủ lực', fallbackText(row.main_crop_type)],
        ['Ghi chú', fallbackText(row.note || row.annual_note)],
      ]), 'is-wide')
      + '</div></article>';
  }
  ...
  function validateForm(form) {
    ...
    if (Math.abs(totalArea - expectedTotal) > 0.0001) {
      toast('Tổng diện tích phải bằng Đất giao dài hạn + Đất công ích + Đất thuê', 'warning');
      return setFieldError(form.elements.total_area, 'Tổng diện tích phải bằng Đất giao dài hạn + Đất công ích + Đất thuê');
    }
    if (convertedArea > longTermArea + 0.0001) {
      toast('Đất chuyển đổi không được lớn hơn diện tích đất giao dài hạn', 'warning');
      return setFieldError(form.elements.converted_area, 'Đất chuyển đổi không được lớn hơn diện tích đất giao dài hạn');
    }
    let usageTotal = 0;
  ...
  async function openDetail(id) {
    const row = await request(API + '/' + encodeURIComponent(id), { cacheTtl: 0 });
    $('#detailTitle') && ($('#detailTitle').textContent = 'Hồ sơ khu đất');
  ...
  async function printDetail(id) {
    ...
    const rows = [
      ...
      ['Ghi chú', row.note || row.annual_note || 'Chưa cập nhật'],
    ];
    if (window.TenantAppPrint?.render) {
      window.TenantAppPrint.render({
        title: 'Hồ sơ khu đất nông nghiệp',
        type: 'agricultural-land-detail',
        orientation: 'portrait',
        paperSize: 'A4',
        headers: ['Thông tin', 'Giá trị'],
        rows,
        filters: { 'Khu đất': row.zone_name || row.zone_code || '', 'Mã khu': row.zone_code || '', 'Năm': row.report_year || '' },
        showSummary: false,
        showSignature: true
      });
      return;
