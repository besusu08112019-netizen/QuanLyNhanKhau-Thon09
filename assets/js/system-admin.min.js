...
<button class="btn btn-outline-success" type="button" data-system-refresh><i class="fa-solid fa-rotate"></i> LÃ m má»›i</button>
...
<button class="btn btn-outline-primary" type="button" data-system-backup="full"><i class="fa-solid fa-box-archive"></i> Backup toÃ n há»‡ thá»‘ng</button>
</div>
</div>
...
setHtml('#systemAdminConfig', '<div class="system-admin-config">' + keys.map(key => '<div><span>' + esc(key) + '</span><strong>' + esc(settings[key] || data[key] || '') + '</strong></div>').join('') + '</div><button class="btn btn-sm btn-outline-primary mt-2" type="button" data-open-settings>Má»Ÿ cáº¥u hÃ¬nh</button>');
...
if (!confirm(type === 'full' ? 'Táº¡o backup toÃ n há»‡ thá»‘ng? Hiá»‡n táº¡i há»‡ thá»‘ng sáº½ xuáº¥t SQL vÃ  ghi nháº­t kÃ½ thao tÃ¡c.' : 'Táº¡o backup database ngay bÃ¢y giá»?')) return;
...
async function revokeSession(id) { if (!confirm('ÄÄƒng xuáº¥t phiÃªn nÃ y?')) return; await apiPost(API + '/sessions/' + id + '/revoke'); notify('ÄÃ£ Ä‘Äƒng xuáº¥t phiÃªn', 'success'); loadSessions(); }
async function revokeAllSessions() { if (!confirm('ÄÄƒng xuáº¥t táº¥t cáº£ phiÃªn khÃ¡c?')) return; await apiPost(API + '/sessions/revoke-all'); notify('ÄÃ£ Ä‘Äƒng xuáº¥t cÃ¡c phiÃªn khÃ¡c', 'success'); loadSessions(); }
async function cleanup(target) { if (!confirm('Dá»n dáº¹p má»¥c nÃ y? Dá»¯ liá»‡u ngÆ°á»i dÃ¹ng sáº½ khÃ´ng bá»‹ xÃ³a.')) return; const data = await apiPost(API + '/cleanup', { target }); notify('ÄÃ£ dá»n ' + (data.label || ''), 'success'); loadMemory(); }
...
function cleanupButton(key) { return ['cache','sessions','tmp'].includes(key) ? '<button class="btn btn-sm btn-outline-danger" data-cleanup="' + esc(key) + '">Dá»n</button>' : '<span class="text-muted small">Chá»‰ xem</span>'; }
