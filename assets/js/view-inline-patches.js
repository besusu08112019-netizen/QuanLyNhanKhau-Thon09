(function(){
  ...
  function navigationRepairModule(){
    ...
    function syncDashboardTree(screen,requested){screen=normalizeScreen(screen);requested=normalizeScreen(requested);var active=!!(dashboardScreens[screen]||dashboardScreens[requested]);var saved='0';try{saved=localStorage.getItem('thon09_dashboard_tree_open')||'0';}catch(error){}setDashboardTreeOpen(active||saved==='1',false);var tree=document.querySelector('[data-dashboard-tree]');if(tree)tree.classList.toggle('is-active',active);document.querySelectorAll('[data-dashboard-tree] .dashboard-tree-link[data-screen]').forEach(function(btn){var current=btn.dataset.screen===screen||btn.dataset.screen===requested;btn.classList.toggle('active',current);btn.setAttribute('aria-current',current?'page':'false');});}
    function repairScreenState(screen,requested){screen=normalizeScreen(screen);var target=document.getElementById(screen+'Screen')||document.getElementById('dashboardScreen');if(!target)return;ensurePublicAssetsMobileNavItem();document.querySelectorAll('.screen').forEach(function(el){el.classList.toggle('active',el===target);});document.querySelectorAll('.sidebar .nav-link').forEach(function(btn){btn.classList.toggle('active',btn.dataset.screen===screen||btn.dataset.screen===requested);btn.setAttribute('aria-current',(btn.dataset.screen===screen||btn.dataset.screen===requested)?'page':'false');});syncDashboardTree(screen,requested);document.querySelectorAll('.mobile-bottom-nav [data-mobile-screen]').forEach(function(btn){var active=btn.dataset.mobileScreen===screen||btn.dataset.mobileScreen===requested;btn.classList.toggle('active',active);btn.setAttribute('aria-current',active?'page':'false');if(active)centerMobileBottomNavItem(btn);});var label=labels[screen]||labels[requested]||'Dashboard';var title=document.getElementById('screenTitle');if(title)title.textContent=label;var breadcrumb=document.getElementById('breadcrumbTrail');if(breadcrumb)breadcrumb.textContent='Trang chủ / '+label;try{localStorage.setItem('thon09_screen',screen);}catch(error){}}
    ...
    document.addEventListener('click',function(event){var toggle=event.target.closest&&event.target.closest('[data-dashboard-toggle]');if(toggle){event.preventDefault();var tree=toggle.closest('[data-dashboard-tree]');setDashboardTreeOpen(!(tree&&tree.classList.contains('is-open')),true);return;}var button=event.target.closest&&event.target.closest('.sidebar .nav-link[data-screen]');if(!button||button.classList.contains('gov-logout'))return;event.preventDefault();window.switchScreen(button.dataset.screen);},true);
