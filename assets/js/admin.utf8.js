  ...
  function installPersonTableRenderFix() {
    ...
    window.personRow = function personRow(row = {}) {
      ...
      return '<tr>'
        ...
        + '<td>' + (window.renderCitizenProfileLink ? window.renderCitizenProfileLink(row, row.full_name || '', { className: 'person-name-link' }) : '<button class="btn btn-link person-name-link" onclick="openCitizenProfile(' + Number(row.id || 0) + ')">' + safe(row.full_name || '') + '</button>') + '</td>'
        ...
        + '<td class="text-end"><button class="btn btn-sm person-row-btn" onclick="openCitizenProfile(' + Number(row.id || 0) + ')">Xem</button>' + (canUpdateCitizen ? ' <button class="btn btn-sm person-row-btn person-row-edit" onclick="openPersonForm(' + Number(row.id || 0) + ')">Sửa</button>' : '') + (canDeleteCitizen ? ' <button class="btn btn-sm btn-outline-danger" onclick="deletePerson(' + Number(row.id || 0) + ')">Xóa</button>' : '') + '</td>'
        + '</tr>';
    };
