...
async function mockApis(page) {
  await page.route('**/api/**', async (route) => {
    ...
    if (url.includes('/api/public/login-config')) return fulfill({ settings: { systemName: 'H? th?ng qu?n l˝ h‡nh chÌnh', hamletName: 'ThÙn 09', communeName: 'X„ H?ng Phong', version: 'v2.0' }, metrics: {} });
...
test.describe(`Production UI audit (${browserName()})`, () => {
  ...
  test('system admin destructive actions use the shared confirm dialog on mobile', async ({ page }) => {
    ...
    await expect(dialog).toContainText('T·∫°o backup database ngay b√¢y gi·ªù?');
