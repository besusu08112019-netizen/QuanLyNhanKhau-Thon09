...
final class PolicySubjectController extends BaseController
{
    ...
    public function storeType(): void
    {
        ...
        try {
            ...
            $this->safeAudit($user, 'policy_subjects', 'create', 'ThÃªm loáº¡i Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $row['id'] ?? null, ['after' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
            $this->logPolicyException('type.store', $e, (array) $this->input());
            $this->fail($this->safeExceptionMessage('KhÃ´ng lÆ°u Ä‘Æ°á»£c loáº¡i Ä‘á»‘i tÆ°á»£ng', $e), 422);
        }
    }
    ...
    public function updateType(string $id): void
    {
        ...
        if (!$before) $this->fail('KhÃ´ng tÃ¬m tháº¥y loáº¡i Ä‘á»‘i tÆ°á»£ng', 404);
        try {
            $row = $this->policySubjects->saveType((array) $this->input(), (int) $user['id'], (int) $id);
            $this->safeAudit($user, 'policy_subjects', 'update', 'Cáº­p nháº­t loáº¡i Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $id, ['before' => $before, 'after' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
            $this->logPolicyException('type.update', $e, ['id' => $id] + (array) $this->input());
            $this->fail($this->safeExceptionMessage('KhÃ´ng cáº­p nháº­t Ä‘Æ°á»£c loáº¡i Ä‘á»‘i tÆ°á»£ng', $e), 422);
        }
    }
    ...
    public function deleteType(string $id): void
    {
        ...
        try {
            ...
            $this->safeAudit($user, 'policy_subjects', 'delete', 'XÃ³a loáº¡i Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $id, ['before' => $before]);
            $this->ok(['id' => (int) $id]);
        } catch (Throwable $e) {
            $this->logPolicyException('type.delete', $e, ['id' => $id]);
            $this->fail($this->safeExceptionMessage('KhÃ´ng xÃ³a Ä‘Æ°á»£c loáº¡i Ä‘á»‘i tÆ°á»£ng', $e), 422);
        }
    }
    ...
    public function show(string $id): void
    {
        ...
        if (!$row) $this->fail('KhÃ´ng tÃ¬m tháº¥y há»“ sÆ¡ chÃ­nh sÃ¡ch', 404);
        $this->ok($row);
    }
    ...
    public function store(): void
    {
        ...
        try {
            ...
            $this->safeAudit($user, 'policy_subjects', 'create', 'ThÃªm há»“ sÆ¡ Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $row['id'] ?? null, ['after' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
            $this->logPolicyException('record.store', $e, (array) $this->input());
            $this->fail($this->safeExceptionMessage('KhÃ´ng lÆ°u Ä‘Æ°á»£c há»“ sÆ¡ chÃ­nh sÃ¡ch', $e), 422);
        }
    }
    ...
    public function update(string $id): void
    {
        ...
        if (!$before) $this->fail('KhÃ´ng tÃ¬m tháº¥y há»“ sÆ¡ chÃ­nh sÃ¡ch', 404);
        try {
            $row = $this->policySubjects->updateRecord((int) $id, (array) $this->input(), (int) $user['id'], $this->requestMeta());
            $this->safeAudit($user, 'policy_subjects', 'update', 'Cáº­p nháº­t há»“ sÆ¡ Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $id, ['before' => $before, 'after' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
            $this->logPolicyException('record.update', $e, ['id' => $id] + (array) $this->input());
            $this->fail($this->safeExceptionMessage('KhÃ´ng cáº­p nháº­t Ä‘Æ°á»£c há»“ sÆ¡ chÃ­nh sÃ¡ch', $e), 422);
        }
    }
    ...
    public function destroy(string $id): void
    {
        ...
        try {
            ...
            $this->safeAudit($user, 'policy_subjects', 'delete', 'XÃ³a má»m há»“ sÆ¡ Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $id, ['before' => $before]);
            $this->ok(['id' => (int) $id]);
        } catch (Throwable $e) {
            $this->logPolicyException('record.delete', $e, ['id' => $id]);
            $this->fail($this->safeExceptionMessage('KhÃ´ng xÃ³a Ä‘Æ°á»£c há»“ sÆ¡ chÃ­nh sÃ¡ch', $e), 422);
        }
    }
    ...
    public function uploadAttachment(string $recordId): void
    {
        ...
        if (empty($_FILES['file'])) $this->fail('Vui lÃ²ng chá»n file', 422);
        try {
            ...
            $this->safeAudit($user, 'policy_subjects', 'upload', 'Upload há»“ sÆ¡ Ä‘Ã­nh kÃ¨m Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $recordId, ['attachment' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
            $this->logPolicyException('attachment.upload', $e, ['recordId' => $recordId]);
            $this->fail($this->safeExceptionMessage('KhÃ´ng upload Ä‘Æ°á»£c há»“ sÆ¡ Ä‘Ã­nh kÃ¨m', $e), 422);
        }
    }
    ...
    public function deleteAttachment(string $id): void
    {
        ...
        try {
            ...
            $this->safeAudit($user, 'policy_subjects', 'delete', 'XÃ³a há»“ sÆ¡ Ä‘Ã­nh kÃ¨m Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $id);
            $this->ok(['id' => (int) $id]);
        } catch (Throwable $e) {
            $this->logPolicyException('attachment.delete', $e, ['id' => $id]);
            $this->fail($this->safeExceptionMessage('KhÃ´ng xÃ³a Ä‘Æ°á»£c há»“ sÆ¡ Ä‘Ã­nh kÃ¨m', $e), 422);
        }
    }
    ...
    public function citizenSummary(string $citizenId): void
    {
        ...
        try {
            ...
            $this->fail($this->safeExceptionMessage('KhÃ´ng táº£i Ä‘Æ°á»£c tÃ³m táº¯t Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', $e), 422);
        }
    }
    ...
    public function exportExcel(): void
    {
        ...
        $this->safeAudit($user, 'policy_subjects', 'export', 'Xuáº¥t Excel bÃ¡o cÃ¡o Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', null, ['totalRows' => $report['totalRows']]);
    ...
    public function exportPdf(): void
    {
        $user = $this->requirePermission('policy_subjects', 'export');
        $report = ExportEncoding::cleanArray($this->policySubjects->report($this->filters()));
        $this->safeAudit($user, 'policy_subjects', 'export', 'Xuáº¥t PDF bÃ¡o cÃ¡o Ä‘á»‘i tÆ°á»£ng chÃ­nh sÃ¡ch', null, ['totalRows' => $report['totalRows']]);
        $pdf = new SimplePdf();
