(function () {
  ...
  function setLoading() {
    ...
    if (host) host.innerHTML = '<div class="data-quality-loading">Äang kiá»ƒm tra cháº¥t lÆ°á»£ng dá»¯ liá»‡u...</div>';
  }
  ...
  function renderSummary() {
    ...
    const kpis = [
      { label: 'Data Quality Score', value: score.value == null ? '--' : score.value + '/100', hint: score.label || 'ChÆ°a Ä‘Ã¡nh giÃ¡', tone: scoreTone(score.value) },
      { label: 'Data Completeness', value: pct(completeness.completePercent), hint: 'Thiáº¿u dá»¯ liá»‡u: ' + pct(completeness.missingPercent), tone: 'success' },
      { label: 'Tá»•ng sá»‘ lá»—i', value: num(totals.issues), hint: 'Cáº§n rÃ  soÃ¡t thá»§ cÃ´ng', tone: 'neutral' },
      { label: 'Critical', value: num(totals.critical), hint: 'áº¢nh hÆ°á»Ÿng trá»±c tiáº¿p Ä‘áº¿n thá»‘ng kÃª/chÃ­nh sÃ¡ch', tone: 'critical' },
      { label: 'High', value: num(totals.high), hint: 'Cáº§n xá»­ lÃ½ sá»›m', tone: 'high' },
      { label: 'Medium', value: num(totals.medium), hint: 'Cáº§n bá»• sung khi rÃ  soÃ¡t', tone: 'medium' },
      { label: 'Low', value: num(totals.low), hint: 'ThÃ´ng tin bá»• sung', tone: 'low' }
    ];
    qs('#dataQualityKpis').innerHTML = kpis.map(kpiCard).join('');

    const generatedAt = qs('#dataQualityGeneratedAt');
    if (generatedAt) generatedAt.textContent = data.engine && data.engine.generatedAt ? 'Cáº­p nháº­t: ' + formatTime(data.engine.generatedAt) : 'ÄÃ£ kiá»ƒm tra';
  ...
  function renderGroups(groups) {
    ...
    if (!host) return;
    if (!groups.length) {
      host.innerHTML = '<div class="data-quality-empty">KhÃ´ng cÃ³ nhÃ³m lá»—i cáº§n xá»­ lÃ½.</div>';
      return;
    }
    host.innerHTML = groups.map(group => (
      ...
      '<small>Critical ' + num(group.critical) + ' Â· High ' + num(group.high) + '</small>' +
      '</button>'
    )).join('');
  }
  ...
  function syncGroupFilter(groups) {
    ...
    select.innerHTML = '<option value="">Táº¥t cáº£ nhÃ³m</option>' + groups.map(group => '<option value="' + esc(group.key) + '">' + esc(group.label || group.key) + '</option>').join('');
  ...
  function renderIssues() {
    ...
    if (!issues.length) {
      host.innerHTML = '<div class="data-quality-empty">KhÃ´ng cÃ³ lá»—i phÃ¹ há»£p bá»™ lá»c.</div>';
      return;
    }
    ...
    host.innerHTML = issues.map(issue => (
      ...
      '<small><strong>áº¢nh hÆ°á»Ÿng:</strong> ' + esc(issue.impact) + '</small>' +
      '</div>' +
      '<div class="data-quality-issue-side">' +
      '<strong>' + num(issue.count) + '</strong>' +
      issueWorkTaskActions(issue) +
      '<button class="btn btn-sm btn-outline-primary" type="button" data-platform-action="dataQuality.issueDetail" data-issue-code="' + esc(issue.code) + '">Xem há»“ sÆ¡</button>' +
      '</div>' +
      '</article>'
    )).join('');
  }
  ...
  function issueWorkTaskActions(issue) {
    ...
    if (task && task.id) {
      return '<span class="badge bg-success">ÄÃ£ táº¡o cÃ´ng viá»‡c</span>' +
        '<button class="btn btn-sm btn-success" type="button" data-platform-action="dataQuality.openWorkTask" data-source-ref="' + esc(ref) + '">Má»Ÿ cÃ´ng viá»‡c</button>';
    }
    return '<button class="btn btn-sm btn-outline-success" type="button" data-platform-action="dataQuality.createWorkTask" data-issue-code="' + esc(issue.code) + '"><i class="fa-solid fa-list-check"></i> Táº¡o cÃ´ng viá»‡c</button>';
  }
  ...
  function createWorkTaskFromIssue(code) {
    ...
    if (!issue) return toast('KhÃ´ng tÃ¬m tháº¥y lá»—i cáº§n xá»­ lÃ½', 'warning');
    const ref = dataQualitySourceRef(issue);
    window.WorkTaskQuickCreate?.open({
      ...
      title: 'RÃ  soÃ¡t dá»¯ liá»‡u: ' + (issue.name || issue.code),
      description: [
        issue.description || '',
        issue.impact ? 'áº¢nh hÆ°á»Ÿng: ' + issue.impact : '',
        issue.suggestion ? 'Gá»£i Ã½ xá»­ lÃ½: ' + issue.suggestion : '',
        'Sá»‘ há»“ sÆ¡ liÃªn quan: ' + num(issue.count),
      ].filter(Boolean).join('\n'),
      priority: issue.severity,
      action: issue.suggestion || 'Má»Ÿ danh sÃ¡ch há»“ sÆ¡ liÃªn quan vÃ  rÃ  soÃ¡t.',
      citizens: ['citizen', 'identity', 'policy', 'labor', 'student'].includes(issue.group) ? num(issue.count) + ' há»“ sÆ¡' : '',
      households: ['household', 'relation', 'gis'].includes(issue.group) ? num(issue.count) + ' há»“ sÆ¡' : '',
      categoryCode: 'other',
    });
  }
  ...
  async function loadIssueDetail(code) {
    ...
    if (host) host.innerHTML = '<div class="data-quality-loading">Äang táº£i há»“ sÆ¡ liÃªn quan...</div>';
    try {
      const data = await apiGet(API + '/issue', { code, pageSize: 30 });
      renderIssueDetail(data);
    } catch (error) {
      if (host) host.innerHTML = '<div class="data-quality-error">' + esc(error.message || 'KhÃ´ng táº£i Ä‘Æ°á»£c chi tiáº¿t') + '</div>';
    }
  }
  ...
  function renderIssueDetail(data) {
    ...
    if (!rows.length) {
      host.innerHTML = '<div class="data-quality-empty">KhÃ´ng cÃ²n há»“ sÆ¡ liÃªn quan Ä‘áº¿n lá»—i nÃ y.</div>';
      return;
    }
    host.innerHTML = '<div class="data-quality-detail-summary">' +
      ...
      '<strong>' + num(issue.count) + '</strong>' +
      '</div>' +
      '<div class="table-responsive"><table class="table data-quality-table align-middle mb-0"><thead><tr><th>Há»“ sÆ¡</th><th>MÃ£ há»™</th><th>ThÃ´ng tin</th><th class="text-end">Thao tÃ¡c</th></tr></thead><tbody>' +
  ...
  function detailRow(row) {
    ...
    const info = [row.identity_number, row.phone, row.relationship, row.date_of_birth, row.message, row.duplicate_value].filter(Boolean).join(' Â· ');
    ...
    return '<tr>' +
      ...
      '<td class="text-end"><button class="btn btn-sm btn-light" type="button" data-platform-action="' + action + '" data-id="' + esc(row.entity_id || '') + '" data-search="' + esc(row.citizen_code || row.household_code || title) + '">Má»Ÿ nhanh</button></td>' +
      '</tr>';
  }

  function renderEmptyDetail() {
    const host = qs('#dataQualityIssueDetail');
    if (host) host.innerHTML = '<div class="data-quality-empty">Chá»n má»™t lá»—i Ä‘á»ƒ xem danh sÃ¡ch há»“ sÆ¡ cáº§n rÃ  soÃ¡t.</div>';
  }

  function renderError(error) {
    const message = error && error.message ? error.message : 'KhÃ´ng táº£i Ä‘Æ°á»£c Data Quality Center';
  ...
  function registerActions() {
    ...
    actions
      .register('dataQuality.refresh', () => loadDataQuality().then(() => toast('ÄÃ£ kiá»ƒm tra láº¡i cháº¥t lÆ°á»£ng dá»¯ liá»‡u', 'success')).catch(() => {}))
