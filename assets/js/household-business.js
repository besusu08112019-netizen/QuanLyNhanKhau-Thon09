(() => {
  ...
  const show = (message, type = 'success') => typeof window.showToast === 'function' ? window.showToast(message, type) : undefined;
  ...
  function sectorCellForRow(row, activities) {
    ...
    if (remaining > 0) {
      html += '<button class="business-more-badge" type="button" data-platform-action="businessHouseholds.detail" data-household-id="' + householdId + '">CÃ²n ' + remaining + ' hoáº¡t Ä‘á»™ng khÃ¡c</button>';
    }
    return html + '</div>';
  }
  ...
  function rowHtml(row, index) {
    ...
    return '<tr class="business-list-row">'
      ...
      + '<td data-label="MÃ£ há»™"><button class="btn btn-link p-0 fw-semibold" type="button" data-platform-action="businessHouseholds.detail" data-household-id="' + householdId + '">' + esc(row.household_code) + '</button></td>'
  ...
  function actionButtons(row) {
    ...
    const buttons = [
      '<button class="btn btn-sm btn-outline-secondary business-icon-btn" type="button" title="Xem chi tiáº¿t" aria-label="Xem chi tiáº¿t" data-platform-action="businessHouseholds.detail" data-household-id="' + householdId + '"><i class="fa-solid fa-eye"></i></button>',
      can('household_business', 'update') && activities.length === 1 && firstActivityId ? '<button class="btn btn-sm btn-outline-primary business-icon-btn" type="button" title="Sá»­a hoáº¡t Ä‘á»™ng" aria-label="Sá»­a hoáº¡t Ä‘á»™ng" data-platform-action="businessHouseholds.edit" data-id="' + firstActivityId + '"><i class="fa-solid fa-pen-to-square"></i></button>' : '',
      can('household_business', 'create') ? '<button class="btn btn-sm btn-outline-primary business-icon-btn" type="button" title="ThÃªm hoáº¡t Ä‘á»™ng" aria-label="ThÃªm hoáº¡t Ä‘á»™ng" data-platform-action="businessHouseholds.create" data-household-id="' + householdId + '"><i class="fa-solid fa-plus"></i></button>' : '',
      can('household_business', 'delete') && activities.length === 1 && firstActivityId ? '<button class="btn btn-sm btn-outline-danger business-icon-btn" type="button" title="XÃ³a hoáº¡t Ä‘á»™ng" aria-label="XÃ³a hoáº¡t Ä‘á»™ng" data-platform-action="businessHouseholds.delete" data-id="' + firstActivityId + '"><i class="fa-solid fa-trash"></i></button>' : ''
    ];
  ...
  function renderPager(data) {
    ...
    host.innerHTML = '<button class="btn btn-sm btn-outline-secondary" ' + (page <= 1 ? 'disabled' : '') + ' data-platform-action="businessHouseholds.page" data-business-page="' + (page - 1) + '">TrÆ°á»›c</button>'
  ...
  function detailHtml(row) {
    const activities = activitiesOf(row);
    const householdId = Number(row.household_id || row.id || 0);
    const addButton = can('household_business', 'create') ? '<button class="btn btn-primary btn-sm" type="button" data-platform-action="businessHouseholds.create" data-household-id="' + householdId + '"><i class="fa-solid fa-plus"></i> ThÃªm hoáº¡t Ä‘á»™ng</button>' : '';
  ...
  function businessActivityDetailPanel(activity, index) {
    ...
    const edit = can('household_business', 'update') ? '<button class="btn btn-sm btn-outline-primary" type="button" data-platform-action="businessHouseholds.edit" data-id="' + Number(activity.id || 0) + '"><i class="fa-solid fa-pen-to-square"></i> Sá»­a</button>' : '';
    const del = can('household_business', 'delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" data-platform-action="businessHouseholds.delete" data-id="' + Number(activity.id || 0) + '"><i class="fa-solid fa-trash"></i> XÃ³a</button>' : '';
    const title = activity.business_name || activity.economic_type || activity.sector_label || 'Hoáº¡t Ä‘á»™ng sá»‘ ' + (index + 1);
    ...
    return '<article class="business-person-activity-panel business-dashboard-activity-view ' + (index === 0 ? 'is-active' : '') + '" data-business-activity-panel="' + index + '">'
      + '<header class="business-dashboard-activity-hero"><meta charset="utf-8"><div><h3>' + esc(title) + '</h3><p>' + esc(activity.business_type_label || activity.business_type || 'ChÆ°a cáº­p nháº­t') + '</p><p>' + esc(sector) + '</p></div><div class="business-dashboard-activity-actions">' + statusBadge(activity.status, activity.status_label) + edit + del + '</div></header>'
  ...
  function businessActivityImages(images, businessId) {
    const add = can('household_business', 'update') ? '<button class="btn btn-sm btn-outline-primary" type="button" data-platform-action="businessHouseholds.edit" data-id="' + Number(businessId || 0) + '"><i class="fa-solid fa-plus"></i> ThÃªm áº£nh</button>' : '';
  ...
  function dashboardActivityRow(activity, index) {
    const edit = can('household_business', 'update') ? '<button class="btn btn-sm btn-outline-primary" type="button" title="Sá»­a" data-platform-action="businessHouseholds.edit" data-id="' + Number(activity.id || 0) + '"><i class="fa-solid fa-pen-to-square"></i></button>' : '';
    const del = can('household_business', 'delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" title="XÃ³a" data-platform-action="businessHouseholds.delete" data-id="' + Number(activity.id || 0) + '"><i class="fa-solid fa-trash"></i></button>' : '';
    const title = activity.business_name || activity.economic_type || activity.sector_label || 'Hoáº¡t Ä‘á»™ng sá»‘ ' + (index + 1);
  ...
  function fileGallery(title, files, businessId, kind = '') {
    ...
    const cards = files.map(file => {
      ...
      const del = can('household_business', 'delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" title="XÃ³a" data-platform-action="businessHouseholds.fileDelete" data-business-id="' + Number(businessId) + '" data-file-id="' + Number(file.id) + '"><i class="fa-solid fa-trash"></i></button>' : '';
  ...
  async function deleteBusinessFile(businessId, fileId) {
    ...
    if (!await confirmAction({ title: 'XÃ¡c nháº­n xÃ³a file', message: 'XÃ³a file Ä‘Ã­nh kÃ¨m nÃ y?', confirmLabel: 'XÃ³a file', tone: 'danger' })) return;
  ...
  async function remove(id) {
    ...
    if (!await confirmAction({ title: 'XÃ¡c nháº­n xÃ³a hoáº¡t Ä‘á»™ng', message: 'XÃ³a hoáº¡t Ä‘á»™ng sáº£n xuáº¥t/kinh doanh nÃ y? Dá»¯ liá»‡u há»™ gia Ä‘Ã¬nh vÃ  nhÃ¢n kháº©u khÃ´ng bá»‹ xÃ³a.', confirmLabel: 'XÃ³a hoáº¡t Ä‘á»™ng', tone: 'danger' })) return;
