(function () {
  ...
  function shell() {
    ...
    host.innerHTML = [
      ...
      '</div></section>',
      '<section class="module-list-card household-list-card"><div class="module-list-head"><div><h3>Kho ảnh</h3><span id="photoGalleryTotalCount">Tổng số: 0 ảnh</span></div></div><div id="photoGalleryLoadState" class="text-muted small px-3 pb-2 d-none"></div><div id="photoGalleryGrid" class="photo-gallery-grid"></div><div id="photoGalleryPager" class="pager module-pager"></div></section>',
      uploadModal(),
      albumModal(),
      detailModal()
    ].join('');
