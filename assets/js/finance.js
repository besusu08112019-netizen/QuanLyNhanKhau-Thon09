(function () {
  ...
  async function load() { if (!$('#financeScreen')) return; shell(); await catalogs(); fillCategoryFilters(); const query = params(); const [list, dashboard] = await Promise.all([request(API + '?' + query), request(API + '/dashboard?' + query, { cacheTtl: 15000 })]); renderDashboard(dashboard); renderRows(list); renderPager(list); window.thon09ApplyAccessControls?.(); window.thon09SyncResponsiveTableLabels?.($('#financeScreen')); }
