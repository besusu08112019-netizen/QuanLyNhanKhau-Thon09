...
if (role === 'VIEWER') return ['dashboard','household','household_business','agriculture','livestock','houses','citizen','report','gis'].includes(module) && action === 'read';
...
if (role === 'OFFICER') return (['dashboard','household','household_business','agriculture','livestock','houses','citizen','movement','report'].includes(module) && ['read','create','update'].includes(action)) || (module === 'gis' && action === 'read');
return false;
}
...
window.thon09NormalizePermissionModule = normalizePermissionModule;
...
if (typeof actions.bind === 'function') actions.bind(document);
if (!window.__thon09LegacyPlatformActionKeysBound) {
window.__thon09LegacyPlatformActionKeysBound = true;
...
if (window.Thon09Platform && window.Thon09Platform.menuRenderer) {
window.Thon09Platform.menuRenderer.renderMobile();
...
applyAccessControls();
setTimeout(() => window.Thon09NavigationController?.navigate(App.screen), 0);
...
window.Thon09NavigationController?.navigate('persons');
...
if (printBtn) printBtn.onclick = () => window.Thon09Print?.currentScreen({ title: 'Hồ sơ nhân khẩu', orientation: 'portrait', showSummary: false }) || showToast('Print Framework is not ready', 'warning');
...
renderAccessDeniedForCurrentScreen(payload?.error?.message || 'B?n kh?ng c? quy?n truy c?p');
...
if (document.querySelector('link[href="' + resolvedHref + '"],link[data-thon09-asset="' + href + '"]')) return Promise.resolve();
if (RuntimeCache.assets.has(resolvedHref)) return RuntimeCache.assets.get(resolvedHref);
const promise = new Promise((resolve, reject) => {
const link = document.createElement('link');
link.rel = 'stylesheet';
link.href = resolvedHref;
link.setAttribute('data-thon09-asset', href);
link.onload = resolve;
...
script.async = true;
script.setAttribute('data-thon09-asset', src);
...
esriWorldImagery: L.tileLayer(blank, { maxZoom: 20, maxNativeZoom: 19, tileSize: 256, zoomOffset: 0, detectRetina: false, attribution: '' })
};
}
return {
...
esriWorldImagery: L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { maxZoom: 20, maxNativeZoom: 19, minZoom: 0, tileSize: 256, zoomOffset: 0, attribution: 'Tiles &copy; Esri &mdash; Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community', updateWhenIdle: true, updateWhenZooming: false, keepBuffer: 6, detectRetina: false, crossOrigin: true })
};
}
...
if (window.THON09_GIS_DEBUG === true) console.debug(label, gisDebugState(extra));
}
...
let thon09GisHouseholdIcon = null;
function gisValidLeafletIcon(icon) {
return Boolean(icon && typeof icon.createIcon === 'function');
}
function gisHouseholdIcon() {
if (gisValidLeafletIcon(thon09GisHouseholdIcon)) return thon09GisHouseholdIcon;
thon09GisHouseholdIcon = L.divIcon({
...
if (!gisValidLeafletIcon(marker.options.icon)) marker.options.icon = gisHouseholdMarkerOptions(row || marker.__thon09HouseholdRow || {}, marker.options).icon;
return marker;
}
function gisIsHouseholdMarker(marker) {
return Boolean(marker && typeof marker.getLatLng === 'function' && typeof marker.setLatLng === 'function' && marker.__thon09GisHouseholdId != null && gisValidLeafletIcon(marker.options?.icon));
}
function gisWatchMapDom() {
const mapEl = document.querySelector('#gisMap');
if (!mapEl || mapEl.__thon09GisDomDebugBound || typeof MutationObserver === 'undefined') return;
mapEl.__thon09GisDomDebugBound = true;
...
if (!map || map.__thon09GisPanesReady || typeof map.createPane !== 'function') return;
const panes = [
['gisAreaPane', 410],
['gisDrawPane', 430],
['gisHeatPane', 440],
...
map.__thon09GisPanesReady = true;
}
...
const source = marker?.__thon09GisBaseLatLng || (marker && typeof marker.getLatLng === 'function' ? marker.getLatLng() : null);
const lat = Number(source?.lat != null ? source.lat : source?.[0]);
const lng = Number(source?.lng != null ? source.lng : source?.[1]);
if (!Number.isFinite(lat) || !Number.isFinite(lng)) return null;
if (marker && !marker.__thon09GisBaseLatLng) marker.__thon09GisBaseLatLng = { lat, lng };
return { lat, lng };
}
...
marker.__thon09GisBaseLatLng = { lat, lng };
if (typeof marker.setLatLng === 'function') marker.setLatLng([lat, lng]);
marker.__thon09GisDisplayOffset = null;
}
...
const base = gisMarkerBaseLatLng(marker);
if (base && marker.__thon09GisDisplayOffset && typeof marker.setLatLng === 'function') marker.setLatLng([base.lat, base.lng]);
if (marker) marker.__thon09GisDisplayOffset = null;
return;
}
...
marker.__thon09GisDisplayOffset = { duplicateCount: bucket.length, index, base };
if (typeof marker.setLatLng === 'function') marker.setLatLng([nextLat, nextLng]);
});
});
}
class Thon09GisClusterManager {
constructor(map) {
this.map = map;
...
bindClusterClick() {
if (!this.useCluster || !this.group || this.group.__thon09GisClusterClickBound || typeof this.group.on !== 'function') return;
this.group.__thon09GisClusterClickBound = true;
...
list.forEach(marker => {
const id = marker?.__thon09GisHouseholdId != null ? String(marker.__thon09GisHouseholdId) : '';
const row = id ? App.gis?.markerCache?.get(id)?.data || marker.__thon09HouseholdRow || {} : {};
...
const id = marker?.__thon09GisHouseholdId != null ? String(marker.__thon09GisHouseholdId) : '';
if (!id) return;
gisEnsureMarkerIcon(marker, App.gis?.markerCache?.get(id)?.data || marker.__thon09HouseholdRow || {});
...
return new Thon09GisClusterManager(map);
}
class Thon09GisLayerManager {
...
return Boolean(App.gis.openPopupId) || gisInPopupInteractionWindow() || Boolean(window.thon09GisHasOpenHouseholdPopup && window.thon09GisHasOpenHouseholdPopup());
}
...
App.gis.mapManager = new Thon09GisMapManager(App.gis);
App.gis.layerManager = new Thon09GisLayerManager(App.gis.map);
App.gis.markerManager = new Thon09GisMarkerManager(App.gis);
App.gis.polygonManager = new Thon09GisPolygonManager();
App.gis.popupManager = new Thon09GisPopupManager();
App.gis.gpsManager = new Thon09GisGPSManager();
App.gis.searchManager = new Thon09GisSearchManager();
App.gis.drawManager = new Thon09GisDrawManager();
App.gis.manager = {
...
cached.marker.__thon09HouseholdRow = item;
...
marker.__thon09GisHouseholdId = id;
marker.__thon09HouseholdRow = item;
...
if (actionKey === 'households.detail' && householdId && typeof window.thon09GisOpenHousehold === 'function') {
event.preventDefault();
window.thon09GisOpenHousehold(householdId);
return;
}
if (!actionKey || !window.Thon09Platform?.actions?.dispatch) return;
event.preventDefault();
window.Thon09Platform.actions.dispatch(actionKey, { event, target: actionTarget, key: actionKey, dataset: Object.assign({}, actionTarget.dataset || {}) });
});
}
...
if (!polygon.__thon09GisAreaClickBound && typeof polygon.on === 'function') {
polygon.__thon09GisAreaClickBound = true;
...
gisSetMarkerBaseLatLng(cached.marker, [savedLat, savedLng]);
