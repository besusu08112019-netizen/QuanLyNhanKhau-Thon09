(() => {
  ...
  function registerImportPlatformActions() {
    ...
    actions.register('import.template', context => downloadImportTemplate(context.dataset.importTemplate));
  ...
  function bindImportNavigation() {
    ...
    document.addEventListener('tenant:screen-change', event => {
      ...
      if (title) title.textContent = 'Import dá»¯ liá»‡u';
    });
  }
  ...
  function renderImportResult(data, processed) {
    ...
    latestImportErrors = data.errors || [];
    const download = latestImportErrors.length ? '<button id="importErrorDownload" class="btn btn-outline-danger btn-sm mt-3" type="button" data-platform-action="import.errors.download"><i class="fa-solid fa-download"></i> Táº£i file lá»—i CSV</button>' : '';
