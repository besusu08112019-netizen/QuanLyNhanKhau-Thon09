...
<style id="thon09-sidebar-redesign">
  ...
  .gov-brand > div:not(.sidebar-official-logo) {
    display: grid !important;
    justify-items: center !important;
    gap: 2px !important;
    text-align: center !important;
  ...
  .sidebar-official-logo::before {
    content: '';
    position: absolute;
    inset: 3px;
    border-radius: 50%;
    background: radial-gradient(circle at 50% 50%, #fff 0 58%, transparent 59%), conic-gradient(from -18deg, #f2b705, #0f8f4d, #f2b705, #0f8f4d, #f2b705);
    box-shadow: 0 10px 24px rgba(10, 143, 77, .12);
    z-index: -2;
  }
  .sidebar-official-logo::after {
    content: '';
    position: absolute;
    inset: 13px;
    border-radius: 50%;
    border: 2px solid #0a8f4d;
    background: linear-gradient(180deg, #fffef6 0%, #eef8ec 100%);
    z-index: -1;
  }
  .sidebar-logo-flag,
  .sidebar-logo-landmark,
  .sidebar-logo-home,
  .sidebar-logo-text,
  .sidebar-official-logo strong {
    position: absolute;
    z-index: 1;
  }
  .sidebar-logo-flag {
  ...
  .sidebar-logo-landmark { top: 38px; left: 24px; color: #08733e; font-size: 19px; }
  .sidebar-logo-home { top: 41px; right: 24px; color: #2f9a4b; font-size: 15px; }
  .sidebar-logo-text { top: 56px; left: 0; right: 0; color: #0a8f4d; font-size: 10px; line-height: 1; font-weight: 850; text-align: center; text-transform: uppercase; }
  .sidebar-official-logo strong { top: 66px; left: 0; right: 0; color: #dc2626; font-size: 24px; line-height: .9; font-weight: 900; text-align: center; }
  .nav-section {
  ...
  .gov-nav .nav-link.active {
    background: #0A8F4D !important;
    color: #fff !important;
    box-shadow: 0 12px 24px rgba(10, 143, 77, .22) !important;
  }
  .gov-nav .nav-link.active i {
    color: #fff !important;
  }
  .gov-nav .nav-link:hover:not(.active) {
    background: #e9f7ef !important;
    color: #0A8F4D !important;
  }
  body.sidebar-collapsed .gov-brand > div:not(.sidebar-official-logo),
  body.sidebar-collapsed .nav-section-title,
  body.sidebar-collapsed .gov-nav .nav-link span,
  body.sidebar-collapsed .gov-logout span {
    display: none !important;
  }
  body.sidebar-collapsed .sidebar-official-logo {
    width: 52px;
    height: 52px;
  }
  body.sidebar-collapsed .sidebar-logo-flag { top: 10px; width: 18px; height: 11px; font-size: 6px; }
  body.sidebar-collapsed .sidebar-logo-landmark { top: 23px; left: 14px; font-size: 11px; }
  body.sidebar-collapsed .sidebar-logo-home { top: 24px; right: 14px; font-size: 10px; }
  body.sidebar-collapsed .sidebar-logo-text { top: 34px; font-size: 7px; }
  body.sidebar-collapsed .sidebar-official-logo strong { top: 39px; font-size: 14px; }
  ...
  @media (max-width: 360px) {
    .gov-sidebar {
      width: min(232px, 90vw) !important;
...
HTML;
$html = str_replace('</head>', $sidebarCss . "\n</head>", $html);
echo $html;