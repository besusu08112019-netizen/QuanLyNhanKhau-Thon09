...
final class User extends BaseModel
{
    ...
    public function can(array $user, string $module, string $action): bool
    {
        ...
        if ($role === 'VIEWER') {
            return (in_array($module, ['dashboard','household','household_business','agriculture','livestock','finance','work_tasks','work_calendar','documents','photo_gallery','houses','public_assets','complaints','citizen','report','statistics','gis'], true) && $action === 'read') || ($module === 'notification' && in_array($action, ['read','update'], true));
        }
        ...
        if ($role === 'OFFICER') return (in_array($module, ['dashboard','household','household_business','agriculture','livestock','finance','work_tasks','work_calendar','documents','photo_gallery','houses','public_assets','complaints','citizen','movement','report'], true) && in_array($action, ['read','create','update','upload','export'], true)) || ($module === 'statistics' && $action === 'read') || ($module === 'notification' && in_array($action, ['read','update'], true)) || ($module === 'gis' && $action === 'read');
        return false;
    }
