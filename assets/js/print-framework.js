(() => {
  'use strict';

  const DEFAULT_UNIT = window.AppSettings?.unitName || 'Don vi hanh chinh';
  const AGENCY_PRIMARY = DEFAULT_UNIT;
  const AGENCY_SECONDARY = '';
  const NATIONAL_TITLE = 'C&#7896;NG H&#210;A X&#195; H&#7896;I CH&#7910; NGH&#296;A VI&#7878;T NAM';
  const NATIONAL_SUBTITLE = '&#272;&#7897;c l&#7853;p - T&#7921; do - H&#7841;nh ph&#250;c';
  const BLANK_DATE_LINE = '................................, ng&#224;y ..... th&#225;ng ..... n&#259;m ......';

  const TYPE_ORIENTATION = Object.freeze({
    household: 'portrait',
    population: 'landscape',
    citizen: 'landscape',
    summary: 'portrait',
    gis: 'landscape',
    'gis-located': 'landscape',
    'gis-unlocated': 'landscape',
    contributions: 'landscape',
    'contributions-list': 'landscape',
    'contributions-collection': 'landscape',
    'contributions-unpaid-list': 'landscape',
    'contributions-partial': 'landscape',
    'contributions-exempt': 'landscape',
    'contributions-by-contribution': 'landscape',
    'contributions-summary': 'portrait',
    'contributions-year-summary': 'portrait'
  });

  const FILTER_LABELS = Object.freeze({
    type: 'Lo\u1ea1i b\u00e1o c\u00e1o',
    report_type: 'Lo\u1ea1i b\u00e1o c\u00e1o',
    dateFrom: 'T\u1eeb ng\u00e0y',
    dateTo: '\u0110\u1ebfn ng\u00e0y',
    householdStatus: 'Tr\u1ea1ng th\u00e1i h\u1ed9',
    householdType: 'Di\u1ec7n h\u1ed9',
    gender: 'Gi\u1edbi t\u00ednh',
    residencyStatus: 'C\u01b0 tr\u00fa',
    presenceStatus: 'Hi\u1ec7n t\u1ea1i',
    lifeStatus: 'Tr\u1ea1ng th\u00e1i',
    ageFrom: 'Tu\u1ed5i t\u1eeb',
    ageTo: 'Tu\u1ed5i \u0111\u1ebfn',
    ethnicity: 'D\u00e2n t\u1ed9c',
    religion: 'T\u00f4n gi\u00e1o',
    occupation: 'Ngh\u1ec1 nghi\u1ec7p',
    search: 'T\u1eeb kh\u00f3a',
    year: 'N\u0103m',
    campaign_id: '\u0110\u1ee3t thu',
    campaignId: '\u0110\u1ee3t thu',
    payment_status: 'Tr\u1ea1ng th\u00e1i thanh to\u00e1n',
    paymentStatus: 'Tr\u1ea1ng th\u00e1i thanh to\u00e1n',
    contribution_name: 'Kho\u1ea3n thu',
    contributionName: 'Kho\u1ea3n thu',
    area_code: 'Khu v\u1ef1c',
    areaCode: 'Khu v\u1ef1c',
    party_member: '\u0110\u1ea3ng vi\u00ean',
    has_health_insurance: 'BHYT'
  });

  function esc(value) {
    return String(value ?? '').replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[char]));
  }

  function authToken() {
    try {
      return (window.App && App.token) || localStorage.getItem(tenantStorageKey('token')) || '';
    } catch (_) {
      return (window.App && App.token) || '';
    }
  }

  function fileNameFromHeader(value) {
    const header = String(value || '');
    const utf = header.match(/filename\*=UTF-8''([^;]+)/i);
    if (utf) return decodeURIComponent(utf[1].replace(/["]/g, ''));
    const plain = header.match(/filename="?([^";]+)"?/i);
    return plain ? plain[1] : '';
  }

  function defaultFileName(url, extension) {
    const fallback = 'bao_cao_' + new Date().toISOString().slice(0, 19).replace(/[-:T]/g, '');
    if (extension) return fallback + '.' + extension.replace(/^\./, '');
    if (/pdf/i.test(url)) return fallback + '.pdf';
    if (/excel|xlsx|xls/i.test(url)) return fallback + '.xlsx';
    return fallback;
  }

  async function downloadFile(url, options = {}) {
    const token = authToken();
    if (!token) throw new Error('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.');
    const response = await fetch(url, { headers: { Accept: '*/*', Authorization: 'Bearer ' + token }, cache: 'no-store' });
    const contentType = response.headers.get('Content-Type') || '';
    if (!response.ok || contentType.includes('application/json')) {
      const json = contentType.includes('application/json') ? await response.json().catch(() => null) : null;
      throw new Error(json?.error?.message || json?.message || 'Không xuất được file báo cáo.');
    }
    const blob = await response.blob();
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = options.fileName || fileNameFromHeader(response.headers.get('Content-Disposition')) || defaultFileName(url, options.extension);
    document.body.appendChild(link);
    link.click();
    setTimeout(() => URL.revokeObjectURL(link.href), 30000);
    link.remove();
    return { fileName: link.download, size: blob.size, contentType };
  }

  function nowText() {
    return new Date().toLocaleString('vi-VN');
  }

  function unitName(config = {}) {
    const settings = window.App?.settings || window.AppSettings || {};
    return config.unitName || config.meta?.unit_name || settings.unitName || [settings.hamletName, settings.communeName].filter(Boolean).join(' - ') || DEFAULT_UNIT;
  }

  function signatureTitle(value, fallback) {
    const text = String(value || fallback || '').replace(/[:：].*$/u, '').replace(/\.{2,}.*/u, '').trim();
    return text || fallback || '';
  }

  function normalizeRows(rows) {
    return Array.isArray(rows) ? rows : [];
  }

  function columnCount(config) {
    const headers = Array.isArray(config.headers) ? config.headers : [];
    const rows = normalizeRows(config.rows);
    return Math.max(headers.length, ...rows.slice(0, 10).map(row => Array.isArray(row) ? row.length : 0), 1);
  }

  const PORTRAIT_AVAILABLE_MM = 182;
  const TEXT_MM_PER_CHAR = 1.55;
  const COLUMN_PROFILES = Object.freeze({
    tiny: { min: 10, max: 14, wrapChars: 8 },
    narrow: { min: 16, max: 24, wrapChars: 11 },
    medium: { min: 24, max: 34, wrapChars: 16 },
    wide: { min: 34, max: 52, wrapChars: 22 },
    text: { min: 46, max: 70, wrapChars: 28 }
  });

  function normalizedText(value) {
    return String(value ?? '')
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase();
  }

  function textLength(value) {
    return Array.from(String(value ?? '').trim()).length;
  }

  function columnProfile(header) {
    const text = normalizedText(header);
    if (/^(stt|tt|#|no\.?|so tt)$/.test(text) || text.includes('stt')) return COLUMN_PROFILES.tiny;
    if (/(so luong|tong|nam sinh|nam|tuoi|gioi tinh|muc|ty le|dien tich|san luong|so tien|da nop|con no|id|ma )/.test(text)) return COLUMN_PROFILES.narrow;
    if (/(ngay|thang|thoi gian|han|trang thai|tinh trang|phan loai|loai|nhom|doi tuong|khu vuc|chi bo|to chuc)/.test(text)) return COLUMN_PROFILES.medium;
    if (/(ho va ten|ho ten|chu ho|ten ho|ten|nguoi|thanh vien|can bo|don vi)/.test(text)) return COLUMN_PROFILES.wide;
    if (/(dia chi|noi dung|ghi chu|ly do|mo ta|ket qua|hinh thuc|nguon nuoc|cong trinh|san pham|nganh nghe)/.test(text)) return COLUMN_PROFILES.text;
    return COLUMN_PROFILES.medium;
  }

  function orientationMetrics(config = {}) {
    const headers = Array.isArray(config.headers) && config.headers.length ? config.headers : ['Nội dung'];
    const rows = normalizeRows(config.rows).slice(0, 24);
    const cols = columnCount(config);
    let requiredWidth = 0;
    let headerWrapColumns = 0;
    let dataWrapColumns = 0;
    let constrainedTextColumns = 0;
    const columns = [];
    for (let i = 0; i < cols; i++) {
      const header = headers[i] ?? '';
      const profile = columnProfile(header);
      const headerLen = textLength(header);
      const sampleLen = Math.max(0, ...rows.map(row => textLength(Array.isArray(row) ? row[i] : '')));
      const weighted = Math.max(profile.min, Math.min(profile.max, Math.max(headerLen * TEXT_MM_PER_CHAR, sampleLen * 1.12)));
      const portraitEqualWidth = PORTRAIT_AVAILABLE_MM / Math.max(1, cols);
      const portraitChars = Math.max(4, Math.floor(Math.min(weighted, portraitEqualWidth) / TEXT_MM_PER_CHAR));
      const headerLines = Math.ceil(headerLen / Math.max(1, portraitChars));
      const dataLines = Math.ceil(sampleLen / Math.max(1, portraitChars));
      if (headerLines >= 3 || (headerLines >= 2 && headerLen >= 24)) headerWrapColumns++;
      if (dataLines >= 4 || (dataLines >= 3 && profile.min >= COLUMN_PROFILES.wide.min)) dataWrapColumns++;
      if (profile.min >= COLUMN_PROFILES.wide.min && portraitEqualWidth < profile.wrapChars * TEXT_MM_PER_CHAR) constrainedTextColumns++;
      requiredWidth += weighted;
      columns.push({ header, requiredWidth: weighted, headerLines, dataLines });
    }
    const excessiveHeaderWrap = headerWrapColumns >= 2 || columns.some(column => column.headerLines >= 4);
    const excessiveDataWrap = dataWrapColumns >= 2 || (dataWrapColumns >= 1 && constrainedTextColumns >= 2);
    const widthOverflow = requiredWidth > PORTRAIT_AVAILABLE_MM;
    return {
      columnCount: cols,
      requiredWidth,
      portraitAvailableWidth: PORTRAIT_AVAILABLE_MM,
      widthOverflow,
      excessiveHeaderWrap,
      excessiveDataWrap,
      needsLandscape: widthOverflow || excessiveHeaderWrap || excessiveDataWrap || cols >= 7,
      columns
    };
  }

  function orientation(config) {
    const forced = String(config.orientation || '').toLowerCase();
    if (forced === 'landscape') return forced;
    const metrics = orientationMetrics(config);
    if (metrics.needsLandscape) return 'landscape';
    if (forced === 'portrait') return forced;
    const type = String(config.type || config.reportType || config.filters?.type || config.filters?.report_type || '').toLowerCase();
    if (TYPE_ORIENTATION[type]) return TYPE_ORIENTATION[type];
    return 'portrait';
  }

  function densityClass(config) {
    const cols = columnCount(config);
    if (config.fontSize) return '';
    if (cols >= 12) return ' print-dense';
    if (cols >= 9) return ' print-compact';
    return '';
  }

  function objectLines(data, labels = FILTER_LABELS) {
    if (!data || typeof data !== 'object') return [];
    return Object.entries(data)
      .filter(([, value]) => value !== null && value !== undefined && String(value).trim() !== '')
      .map(([key, value]) => [labels[key] || key, value]);
  }

  function listHtml(className, rows) {
    if (!rows.length) return '';
    return '<div class="' + className + '">' + rows.map(([label, value]) => '<div><strong>' + esc(label) + ':</strong> ' + esc(value) + '</div>').join('') + '</div>';
  }

  function reportHeader(config) {
    const metaRows = [
      ['Ng\u01b0\u1eddi l\u1eadp', config.preparedBy || config.meta?.prepared_by || window.App?.user?.displayName || window.App?.user?.email || 'Ng\u01b0\u1eddi l\u1eadp']
    ];
    if (config.meta?.period_label) metaRows.unshift(['K\u1ef3 b\u00e1o c\u00e1o', config.meta.period_label]);
    return '<header class="print-header print-avoid-break">'
      + '<div class="print-masthead">'
      + '<div class="print-agency"><div class="print-agency-primary">' + AGENCY_PRIMARY + '</div><div class="print-agency-secondary">' + AGENCY_SECONDARY + '</div></div>'
      + (config.showNationalHeader === false ? '<div></div>' : '<div class="print-national"><p class="print-national-title">' + NATIONAL_TITLE + '</p><p class="print-national-subtitle">' + NATIONAL_SUBTITLE + '</p></div>')
      + '</div>'
      + '<h1 class="print-title">' + esc(config.title || 'B\u00e1o c\u00e1o') + '</h1>'
      + listHtml('print-meta', metaRows)
      + (config.showFilters === false ? '' : '<div class="print-section-title">\u0110i\u1ec1u ki\u1ec7n l\u1ecdc</div>' + (listHtml('print-filter-list', objectLines(config.filters)) || '<div class="print-filter-list"><div>Kh\u00f4ng \u00e1p d\u1ee5ng b\u1ed9 l\u1ecdc</div></div>'))
      + '</header>';
  }

  function reportTable(config) {
    const headers = Array.isArray(config.headers) && config.headers.length ? config.headers : ['N\u1ed9i dung'];
    const rows = normalizeRows(config.rows);
    const head = headers.map(header => '<th>' + esc(header) + '</th>').join('');
    const body = rows.length
      ? rows.map(row => '<tr>' + headers.map((_, index) => '<td>' + esc(Array.isArray(row) ? row[index] : '') + '</td>').join('') + '</tr>').join('')
      : '<tr><td class="print-empty" colspan="' + headers.length + '">Kh\u00f4ng c\u00f3 d\u1eef li\u1ec7u</td></tr>';
    return '<table class="print-table"><thead><tr>' + head + '</tr></thead><tbody>' + body + '</tbody></table>';
  }

  function summaryHtml(config) {
    if (config.showSummary === false) return '';
    const rows = objectLines(config.summary || defaultSummary(config));
    return rows.length ? '<section class="print-summary print-avoid-break"><div class="print-section-title">T\u1ed5ng h\u1ee3p cu\u1ed1i b\u00e1o c\u00e1o</div>' + listHtml('print-summary-list', rows) + '</section>' : '';
  }

  function defaultSummary(config) {
    const headers = (config.headers || []).map(item => String(item).toLowerCase());
    const rows = normalizeRows(config.rows);
    const summary = { 'T\u1ed5ng s\u1ed1 d\u00f2ng': Number(config.totalRows ?? rows.length).toLocaleString('vi-VN') };
    const genderIndex = headers.findIndex(header => header.includes('gi\u1edbi t\u00ednh'));
    if (genderIndex >= 0) {
      summary.Nam = rows.filter(row => String(row[genderIndex] || '').toLowerCase() === 'nam').length;
      summary['N\u1eef'] = rows.filter(row => String(row[genderIndex] || '').toLowerCase() === 'n\u1eef').length;
    }
    return summary;
  }

  function signatures(config) {
    if (config.showSignature === false) return '';
    const right = signatureTitle(config.signatureRight || config.meta?.approved_by, 'Tr\u01b0\u1edfng th\u00f4n');
    return '<section class="print-signature-block print-avoid-break"><div class="print-date-line">' + BLANK_DATE_LINE + '</div><div class="print-signatures"><div class="print-signature-box"><div class="print-sign-title">' + esc(right) + '</div><div class="print-sign-space"></div><div class="print-sign-name">................................</div></div></div></section>';
  }

  function footer(printedAt, config) {
    if (config.showFooter === false) return '';
    return '<footer class="print-footer"><span>Ng\u00e0y in: ' + esc(printedAt) + '</span><span class="print-footer-page"></span></footer>';
  }

  function inlineStyle(config, orient) {
    const paper = config.paperSize || 'A4';
    const page = '@page{size:' + paper + ' ' + orient + ';margin:' + (orient === 'landscape' ? '12mm 10mm 18mm' : '16mm 14mm 20mm') + '}';
    const vars = config.fontSize ? '.print-document{--print-font-size:' + esc(config.fontSize) + '}' : '';
    return '<style>' + page + vars + '</style>';
  }

  function renderHtml(config) {
    const orient = orientation(config);
    const printedAt = nowText();
    return '<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>' + esc(config.title || 'B\u00e1o c\u00e1o') + '</title><link rel="stylesheet" href="/assets/css/print.css">' + inlineStyle(config, orient) + '</head><body class="print-' + orient + '"><main class="print-document' + densityClass(config) + '">' + reportHeader(config) + reportTable(config) + summaryHtml(config) + signatures(config) + footer(printedAt, config) + '</main><script>window.onload=function(){setTimeout(function(){window.print();},60);};<\/script></body></html>';
  }

  function print(config = {}) {
    const popup = window.open('', '_blank', 'width=1120,height=780');
    if (!popup) {
      if (typeof window.showToast === 'function') window.showToast('Tr\u00ecnh duy\u1ec7t \u0111ang ch\u1eb7n c\u1eeda s\u1ed5 in', 'warning');
      return null;
    }
    popup.document.write(renderHtml(config));
    popup.document.close();
    return popup;
  }

  function fromTable(table, config = {}) {
    const headers = Array.from(table?.querySelectorAll?.('thead th') || []).map(th => th.textContent.trim()).filter(Boolean);
    const rows = Array.from(table?.querySelectorAll?.('tbody tr') || []).map(tr => Array.from(tr.children).map(td => td.textContent.trim()));
    return print(Object.assign({ headers, rows }, config));
  }

  function currentScreen(config = {}) {
    const screen = document.querySelector('.screen.active') || document;
    const table = screen.querySelector('table');
    if (table) return fromTable(table, Object.assign({ title: window.TenantAppPlatform?.navigation?.current?.()?.label || document.title }, config));
    return print(Object.assign({ title: document.title, headers: ['N\u1ed9i dung'], rows: [[screen.innerText.trim()]] }, config));
  }

  window.TenantAppPrint = Object.freeze({
    print,
    render: print,
    fromTable,
    currentScreen,
    orientation,
    orientationMetrics,
    renderHtml
  });

  window.TenantAppExport = Object.freeze({
    download: downloadFile,
    fileNameFromHeader
  });
})();
