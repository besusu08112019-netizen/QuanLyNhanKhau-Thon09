...
final class HouseholdController extends BaseController
{
    ...
    public function bulkDelete(): void { $user = $this->requirePermission('household', 'delete'); $ids = (array) $this->input('ids', []); $deleted = $this->households->bulkSoftDelete($ids, (int) $user['id']); $this->audit($user, 'household', 'delete', 'Xóa hàng loạt hộ gia đình', null, ['ids' => array_values(array_map('intval', $ids)), 'deleted' => $deleted]); $this->ok(['success' => $deleted, 'errors' => []]); }
}
