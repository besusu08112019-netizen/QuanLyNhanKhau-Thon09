...
const reportJs = read('assets/js/report.js');
...
assert.match(model, /assertLandFundTotals/);
assert.match(model, /\$expected = \$longTerm \+ \$publicUtility \+ \$leased/);
assert.match(model, /abs\(\$total - \$expected\) > 0\.0001/);
assert.match(model, /\$converted > \$longTerm \+ 0\.0001/);
assert.match(model, /'converted_area' => \[/);
assert.match(model, /business_note/);
const landFundChart = model.slice(model.indexOf('private function landFundChart'), model.indexOf('private function sumSelect'));
assert.match(landFundChart, /long_term_allocated_area_m2/);
assert.match(landFundChart, /public_utility_area_m2/);
assert.match(landFundChart, /leased_area_m2/);
assert.doesNotMatch(landFundChart, /converted_area_m2/, 'converted area must not be part of land fund chart');
...
assert.match(landJs, /const expectedTotal = longTermArea \+ publicUtilityArea \+ leasedArea/);
assert.match(landJs, /Math\.abs\(totalArea - expectedTotal\) > 0\.0001/);
assert.match(landJs, /convertedArea > longTermArea \+ 0\.0001/);
assert.match(landJs, /charts\.converted_area/);
...
assert.match(reportController, /business_note/);
assert.match(reportJs, /business_note/);
