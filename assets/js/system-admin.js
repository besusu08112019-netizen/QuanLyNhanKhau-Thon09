(() => {
  ...
  function loading(selector) { setHtml(selector, '<div class="system-admin-empty">Đang tải dữ liệu...</div>'); }
  ...
  async function createBackup(type) {
    const accepted = await confirmAction({
      ...
      message: type === 'full' ? 'Tạo backup toàn hệ thống? Hệ thống sẽ xuất SQL và ghi nhật ký thao tác.' : 'Tạo backup database ngay bây giờ?',
  ...
  async function revokeSession(id) {
    if (!(await confirmAction({ title: 'Xác nhận đăng xuất', message: 'Đăng xuất phiên này?', confirmLabel: 'Đăng xuất', tone: 'danger' }))) return;
    await apiPost(API + '/sessions/' + id + '/revoke'); notify('Đã đăng xuất phiên', 'success'); loadSessions();
  }
  async function revokeAllSessions() {
    if (!(await confirmAction({ title: 'Xác nhận đăng xuất', message: 'Đăng xuất tất cả phiên khác?', confirmLabel: 'Đăng xuất tất cả', tone: 'danger' }))) return;
    await apiPost(API + '/sessions/revoke-all'); notify('Đã đăng xuất các phiên khác', 'success'); loadSessions();
  }
  async function cleanup(target) {
    if (!(await confirmAction({ title: 'Xác nhận dọn dẹp', message: 'Dọn dẹp mục này? Dữ liệu người dùng sẽ không bị xóa.', confirmLabel: 'Dọn dẹp', tone: 'danger' }))) return;
    const data = await apiPost(API + '/cleanup', { target }); notify('Đã dọn ' + (data.label || ''), 'success'); loadMemory();
  }
