(function () {
  ...
  function shell() {
    ...
    host.innerHTML = [
      ...
      '<div class="agri-field agri-search-field"><label for="documentsSearch">Tìm kiếm</label><div class="module-search-input-wrap"><i class="fa-solid fa-magnifying-glass"></i><input id="documentsSearch" class="form-control" placeholder="Tiêu đề, số văn bản, đơn vị, người ký..."></div></div>',
      ...
      '<div class="agri-field"><label for="documentsYearFilter">Năm</label><select id="documentsYearFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="documentsPageSize">Hiển thị</label><select id="documentsPageSize" class="form-select"><option>20</option><option>50</option><option>100</option></select></div>',
  ...
  async function catalogs() {
    ...
    fill($('#documentsYearFilter'), state.catalogs.years, 'Tất cả');
  ...
  async function load() {
    ...
    try {
      ...
      toast(error.message || 'Không tải được văn bản', 'danger');
    } finally {
      state.loading = false;
    }
  }
  ...
  function renderRows(data = {}) {
    ...
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">Chưa có văn bản phù hợp bộ lọc.</td></tr>'; return; }
    tbody.innerHTML = rows.map(item => {
      const actions = [
        ...
        item.attachment_count ? `<a class="btn btn-sm btn-outline-success" href="${API}/${item.id}/download" target="_blank" rel="noopener" title="Tải xuống"><i class="fa-solid fa-download"></i></a>` : '',
        can('update') ? `<button class="btn btn-sm btn-outline-secondary" type="button" data-platform-action="documents.edit" data-id="${item.id}" title="Sửa"><i class="fa-solid fa-pen"></i></button>` : '',
        can('delete') ? `<button class="btn btn-sm btn-outline-danger" type="button" data-platform-action="documents.delete" data-id="${item.id}" title="Xóa"><i class="fa-solid fa-trash"></i></button>` : ''
      ].filter(Boolean).join(' ');
  ...
  async function openDetail(id) {
    ...
    const preview = pdf ? `<iframe src="${safe(pdf.preview_url)}" title="PDF preview" style="width:100%;height:520px;border:1px solid #dee2e6;border-radius:8px"></iframe>` : '<div class="text-muted border rounded p-3">File này không hỗ trợ preview trên trình duyệt. Vui lòng tải xuống để xem.</div>';
    ...
    $('#documentsDetailBody').innerHTML = `<div class="row g-3"><div class="col-md-5"><dl class="row mb-0"><dt class="col-sm-4">Số văn bản</dt><dd class="col-sm-8">${safe(text(item.document_number))}</dd><dt class="col-sm-4">Loại</dt><dd class="col-sm-8">${safe(text(item.category_name))}</dd><dt class="col-sm-4">Đơn vị</dt><dd class="col-sm-8">${safe(text(item.issuing_unit))}</dd><dt class="col-sm-4">Người ký</dt><dd class="col-sm-8">${safe(text(item.signer_name))}</dd><dt class="col-sm-4">Ban hành</dt><dd class="col-sm-8">${date(item.issued_date)}</dd><dt class="col-sm-4">Người tải</dt><dd class="col-sm-8">${safe(text(item.created_by_name))}</dd><dt class="col-sm-4">Mô tả</dt><dd class="col-sm-8">${safe(text(item.summary))}</dd></dl><h6 class="mt-3">File đính kèm</h6><div class="work-task-files">${files}</div></div><div class="col-md-7">${preview}</div></div>`;
  ...
  async function remove(id) {
    ...
    if (!window.confirm('Xóa văn bản này? File vật lý sẽ bị xóa khỏi uploads.')) return;
    await request(API + '/' + id, { method: 'DELETE' });
    toast('Đã xóa văn bản', 'success');
    await load();
  }
