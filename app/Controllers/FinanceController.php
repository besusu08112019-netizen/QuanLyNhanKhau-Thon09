...
final class FinanceController extends BaseController
{
    ...
    public function show(string $id): void
    {
        ...
        if (!$row) $this->fail('KhÃ´ng tÃ¬m tháº¥y phiáº¿u thu chi', 404);
        $this->ok($row);
    }
    ...
    public function store(): void
    {
        ...
        try {
            ...
            $this->audit($user, 'finance', 'create', 'ThÃªm phiáº¿u thu chi', $row['id'], ['after' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
    ...
    public function update(string $id): void
    {
        ...
        try {
            ...
            if (!$before) $this->fail('KhÃ´ng tÃ¬m tháº¥y phiáº¿u thu chi', 404);
            $row = $this->finance->upsert((array)$this->input(), (int)$user['id'], (int)$id);
            $this->audit($user, 'finance', 'update', 'Cáº­p nháº­t phiáº¿u thu chi', $id, ['before' => $before, 'after' => $row]);
            $this->ok($row);
        } catch (Throwable $e) {
            $this->fail($e->getMessage(), 422);
        }
    ...
    public function destroy(string $id): void
    {
        ...
        $this->audit($user, 'finance', 'delete', 'XÃ³a phiáº¿u thu chi', $id, ['before' => $before], 'WARN');
        $this->ok(['id' => (int)$id]);
    }
    ...
    public function uploadAttachment(string $id): void
    {
        ...
        if (!$this->finance->find((int)$id)) $this->fail('KhÃ´ng tÃ¬m tháº¥y phiáº¿u thu chi', 404);
        ...
        $this->audit($user, 'finance', 'upload', 'ÄÃ­nh kÃ¨m chá»©ng tá»« thu chi', $id, ['file' => $row]);
        $this->ok($row);
    }
    ...
    public function deleteAttachment(string $id, string $fileId): void
    {
        ...
        $this->audit($user, 'finance', 'delete_attachment', 'XÃ³a chá»©ng tá»« thu chi', $id, ['file' => $before]);
    ...
    public function exportPdf(): void
    {
        ...
        $pdf->addSignatureBlock('NgÆ°á»i láº­p biá»ƒu');
