(function () {
  ...
  function protectPopupElement(popup) {
    ...
    ['touchstart', 'touchend', 'pointerdown', 'pointerup'].forEach(type => {
      el.addEventListener(type, event => stopLeafletPropagation(event), { passive: true });
    });
  }
  ...
  function runPopupAction(action, row, event) {
    ...
    if (action === 'route') window.thon09GisRouteToHousehold(id);
    if (action === 'gallery') window.thon09GisOpenHouseholdGallery(id);
    if (action === 'relocate') window.thon09GisRelocateHousehold(id);
    if (action === 'provider') window.thon09GisOpenMapProvider(id, event.currentTarget.dataset.provider);
  }
  ...
  function bindPopupActions(popup, row) {
    ...
    el.querySelectorAll('[data-gis-popup-action]').forEach(button => {
      ...
      button.addEventListener('click', event => runPopupAction(button.dataset.gisPopupAction, row, event));
    });
  }
  ...
  function popupHtml(row) {
    ...
    return '<div class="gis-household-popup gis-smart-popup">' +
      ...
      '<div class="gis-smart-popup-actions">' +
        '<button class="btn btn-sm btn-primary" type="button" data-gis-popup-action="open"><i class="fa-solid fa-folder-open"></i> Hồ sơ số</button>' +
        '<button class="btn btn-sm btn-success" type="button" data-gis-popup-action="route"><i class="fa-solid fa-route"></i> Chỉ đường</button>' +
        '<button class="btn btn-sm btn-outline-secondary" type="button" data-gis-popup-action="gallery"><i class="fa-solid fa-images"></i> Xem ảnh</button>' +
        (phone ? '<a class="btn btn-sm btn-outline-primary" href="tel:' + escapeHtml(phone) + '"><i class="fa-solid fa-phone"></i> Gọi điện</a>' : '') +
        '<button class="btn btn-sm btn-outline-success" type="button" data-gis-popup-action="relocate"><i class="fa-solid fa-location-crosshairs"></i> GPS</button>' +
      '</div>' +
      '<div class="gis-smart-route-links"><button type="button" data-gis-popup-action="provider" data-provider="google">Google Maps</button><button type="button" data-gis-popup-action="provider" data-provider="apple">Apple Maps</button><button type="button" data-gis-popup-action="provider" data-provider="osm">OpenStreetMap</button></div>' +
    '</div>';
  }
