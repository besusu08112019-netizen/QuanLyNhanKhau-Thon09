...
if (layer.setStyle) layer.setStyle(gisAreaStyle(area, active, hasActive));
if (active && layer.bringToFront) layer.bringToFront();
if (layer._path) layer._path.classList.toggle('gis-area-polygon-active', Boolean(active));
...
if (area) focusGisArea(area.area_code);
}
...
const layer = App.gis.areaLayerMap?.get(gisAreaKey(area));
selectGisArea(area, layer, { scrollList: true });
const latLngs = gisPolygonLatLngs(area);
if (latLngs.length) { gisDebugTrace('map.fitBounds() focusGisArea', { areaCode }); App.gis.map.fitBounds(latLngs, { padding: [28, 28], maxZoom: 17 }); setTimeout(() => updateGisAreaActiveState(true), 120); return; }
const center = gisAreaCenter(area);
if (center) { App.gis.map.setView(center, Math.max(App.gis.map.getZoom(), 17)); setTimeout(() => updateGisAreaActiveState(true), 120); return; }
