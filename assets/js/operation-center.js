(function () {
  ...
  function apiGet(path, params, ttl) {
    ...
    return fetch(path + query, { headers, cache: 'no-store' }).then(r => r.json()).then(payload => {
      if (!payload || payload.ok === false) throw new Error(payload && payload.error && payload.error.message || 'KhÃ´ng táº£i Ä‘Æ°á»£c dá»¯ liá»‡u');
      return payload.data;
    });
  }
  function unwrap(payload) { return payload && payload.data ? payload.data : (payload || {}); }
  function loading(host, text) { if (host) host.innerHTML = '<div class="operation-empty">' + esc(text || 'Äang táº£i dá»¯ liá»‡u') + '</div>'; }
  function widgetError(host, error) { if (host) host.innerHTML = '<div class="operation-error">' + esc(error && error.message ? error.message : 'Widget táº¡m thá»i khÃ´ng táº£i Ä‘Æ°á»£c') + '</div>'; }
  ...
  function normalizeHeader(screen) {
    ...
    if (title) title.textContent = 'Trung tÃ¢m Ä‘iá»u hÃ nh';
    if (crumb) crumb.textContent = 'Trang chá»§ / Trung tÃ¢m Ä‘iá»u hÃ nh';
  }
  ...
  function loadAll() {
    bindStaticEvents();
    loadCommandCenter(); loadNotifications(); loadTasks(); loadTimeline(); loadAreaDashboard(); loadProgress(); loadLogs();
  }

  async function loadCommandCenter() {
    const host = qs('#operationCommandCenter'); loading(host);
    try {
      const data = unwrap(await apiGet(API + '/command-center', null, 10000));
      renderCommandCenter(data.widgets || []);
    } catch (error) { widgetError(host, error); }
  }

  function renderCommandCenter(widgets) {
    const host = qs('#operationCommandCenter'); if (!host) return;
    host.innerHTML = widgets.length ? '<div class="operation-command-grid">' + widgets.map(widget => '<article class="operation-command-widget"><div class="d-flex justify-content-between gap-2 align-items-center"><h5>' + esc(widget.label) + '</h5><button class="btn btn-sm btn-outline-primary" type="button" data-platform-action="operationCenter.openScreen" data-operation-screen="' + esc(widget.screen || 'dashboard') + '"><i class="fa-solid fa-arrow-up-right-from-square"></i></button></div><div class="operation-command-items">' + (widget.items || []).map(item => '<span><small>' + esc(item.label) + '</small><strong>' + num(item.value) + '</strong></span>').join('') + '</div></article>').join('') + '</div>' : empty('Chua co du lieu dashboard dieu hanh');
  }
  ...
  function renderNotifications(items) {
    ...
    host.innerHTML = items.length ? '<div class="operation-list">' + items.map(item => '<div class="operation-item operation-priority-' + esc(item.priority || 'low') + '"><span><strong>' + esc(item.label) + '</strong><small>' + esc(priorityLabel(item.priority)) + ' Â· ' + esc(item.status || 'new') + ' Â· ' + esc(formatTime(item.createdAt)) + '</small></span><div class="operation-actions"><span class="operation-badge">' + num(item.count) + '</span><button class="btn btn-sm btn-outline-primary" type="button" data-platform-action="operationCenter.openScreen" data-operation-screen="' + esc(item.screen || 'dashboard') + '">' + esc(item.action || 'Má»Ÿ') + '</button></div></div>').join('') + '</div>' : empty('KhÃ´ng cÃ³ cáº£nh bÃ¡o ná»•i báº­t');
  }
  ...
  function renderTasks() {
    ...
    host.innerHTML = rows.length ? '<div class="operation-list">' + rows.map(item => {
      ...
      return '<div class="operation-item operation-priority-' + esc(item.priority || 'low') + '"><span><strong>' + esc(item.label) + '</strong><small>' + esc(priorityLabel(item.priority)) + ' Â· ' + esc(statusLabel(status)) + '</small></span><div class="operation-actions"><span class="operation-badge">' + num(item.count) + '</span><button class="btn btn-sm btn-outline-warning" data-platform-action="operationCenter.taskStatus" data-operation-task="' + esc(item.key) + '" data-status="doing">Äang xá»­ lÃ½</button><button class="btn btn-sm btn-outline-success" data-platform-action="operationCenter.taskStatus" data-operation-task="' + esc(item.key) + '" data-status="done">HoÃ n thÃ nh</button><button class="btn btn-sm btn-outline-primary" data-platform-action="operationCenter.openScreen" data-operation-screen="' + esc(item.screen || 'dashboard') + '">Má»Ÿ</button></div></div>';
    }).join('') + '</div>' : empty('KhÃ´ng cÃ³ cÃ´ng viá»‡c phÃ¹ há»£p');
  }
  ...
  async function openQuickProfile(type, id) {
    ...
    } catch (error) { toast(error.message || 'KhÃ´ng má»Ÿ Ä‘Æ°á»£c há»“ sÆ¡ nhanh', 'danger'); }
  }
  ...
  function renderQuickProfile(data) {
    ...
    title.textContent = data.type === 'citizen' ? 'Há»“ sÆ¡ nhanh nhÃ¢n kháº©u' : 'Há»“ sÆ¡ nhanh há»™ gia Ä‘Ã¬nh';
    body.innerHTML = '<div class="operation-profile-grid"><section><dl>'
      + detailLine('MÃ£/Há» tÃªn', p.full_name || p.household_code || p.head_citizen_name)
      + detailLine('CCCD', p.identity_number)
      + detailLine('Äiá»‡n thoáº¡i', p.phone)
      + detailLine('Äá»‹a chá»‰', p.current_address || p.address || p.household_address)
      + detailLine('GPS', data.gps && data.gps.latitude ? data.gps.latitude + ', ' + data.gps.longitude : '')
      + '</dl></section><section><h6>ThÃ nh viÃªn há»™</h6><div class="operation-member-list">' + ((data.members || []).slice(0, 8).map(m => '<span>' + esc(m.full_name || '') + ' <small>' + esc(m.relationship || '') + '</small></span>').join('') || '<span>ChÆ°a cÃ³ dá»¯ liá»‡u</span>') + '</div></section><section><h6>Há»“ sÆ¡ sá»‘</h6><div class="operation-file-list">' + ((data.files || []).slice(0, 8).map(f => '<span>' + esc(f.original_name || f.file_name || 'Tá»‡p Ä‘Ã­nh kÃ¨m') + '</span>').join('') || '<span>ChÆ°a cÃ³ há»“ sÆ¡ sá»‘</span>') + '</div></section><section><h6>Nháº­t kÃ½</h6><div class="operation-file-list">' + ((data.timeline || []).slice(0, 6).map(t => '<span>' + esc(t.created_at || '') + ' Â· ' + esc(t.message || t.action || '') + '</span>').join('') || '<span>ChÆ°a cÃ³ nháº­t kÃ½</span>') + '</div></section></div><div class="mt-3"><button class="btn btn-primary" type="button" data-platform-action="operationCenter.openDetail" data-operation-detail="' + esc(data.type) + '" data-id="' + Number(data.id || 0) + '">Má»Ÿ chi tiáº¿t</button></div>';
  ...
  async function loadTimeline() {
    ...
    try {
      ...
      const items = data.items || [];
      host.innerHTML = items.length ? '<div class="operation-timeline">' + items.map(item => '<div class="operation-timeline-item"><strong>' + esc(item.title || item.action || '') + '</strong><small>' + esc(formatTime(item.time)) + ' Â· ' + esc(item.module || '') + (item.actor ? ' Â· ' + esc(item.actor) : '') + '</small></div>').join('') + '</div>' : empty('ChÆ°a cÃ³ timeline');
    } catch (error) { widgetError(host, error); }
  }
  ...
  async function loadAreaDashboard() {
    ...
    try {
      ...
      host.innerHTML = '<div class="operation-metric-grid">' + metric('Tá»•ng sá»‘ há»™', m.total_households) + metric('Tá»•ng nhÃ¢n kháº©u', m.total_citizens) + metric('Nam', m.male_count) + metric('Ná»¯', m.female_count) + metric('Tráº» em', m.children_count) + metric('NgÆ°á»i cao tuá»•i', m.elderly_count) + metric('Äáº£ng viÃªn', m.party_member_count) + metric('Há»™ nghÃ¨o', m.poor_households) + metric('Há»™ cáº­n nghÃ¨o', m.near_poor_households) + '</div><div class="operation-progress mt-3">' + progressRow('Tiáº¿n Ä‘á»™ GPS', data.gpsProgress) + progressRow('Tiáº¿n Ä‘á»™ há»“ sÆ¡ sá»‘', data.profileProgress) + '</div>';
    } catch (error) { widgetError(host, error); }
  }
  ...
  function syncAreaOptions(areas, current) {
    ...
    select.innerHTML = '<option value="">Táº¥t cáº£ khu vá»±c</option>' + areas.map(area => '<option value="' + esc(area.area_code || '') + '">' + esc(area.area_code || 'ChÆ°a phÃ¢n khu') + '</option>').join('');
  ...
  async function download(path, params, name) {
    ...
    if (!response.ok) { toast('KhÃ´ng xuáº¥t Ä‘Æ°á»£c file', 'danger'); return; }
  ...
  function priorityLabel(priority) { return ({ high: 'Æ¯u tiÃªn cao', medium: 'Æ¯u tiÃªn vá»«a', low: 'Theo dÃµi' })[priority] || 'Theo dÃµi'; }
  function statusLabel(status) { return ({ open: 'Cáº§n xá»­ lÃ½', doing: 'Äang xá»­ lÃ½', done: 'HoÃ n thÃ nh' })[status] || status; }
