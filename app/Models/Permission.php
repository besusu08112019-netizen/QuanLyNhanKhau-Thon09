...
final class Permission extends BaseModel
{
    public function matrix(): array
    {
        ...
        $modules = ['dashboard','household','household_business','agriculture','livestock','houses','citizen','movement','report','pdf','import','export','print','profile','file','gis','photo','video','gps','notification','user','permission','logs','settings','backup','system_admin'];
    ...
    private function defaultAllowed(string $role, string $module, string $action): bool
    {
        ...
        if ($role === 'VIEWER') return in_array($module, ['dashboard','household','household_business','agriculture','livestock','houses','citizen','report','gis'], true) && $action === 'read';
        return false;
    }
}
