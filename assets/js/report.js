(() => {
  ...
  async function printReport() {
    ...
    popup.document.write('<!doctype html><html><head><meta charset="utf-8"><title>' + esc(report.title || 'Báo cáo') + '</title><style>@page{size:A4;margin:14mm}body{font-family:Arial,sans-serif;color:#111}h1{text-align:center;font-size:20px}table{width:100%;border-collapse:collapse;font-size:12px}td,th{border:1px solid #555;padding:6px;vertical-align:top}th{background:#eef2f7}</style></head><body><h1>' + esc(report.title || 'Báo cáo') + '</h1><p>Ngày in: ' + new Date().toLocaleString('vi-VN') + '</p>' + reportTable(report) + '<script>window.onload=function(){window.print();};</script></body></html>');
