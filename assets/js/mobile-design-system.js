(function (window, document) {
  ...
  function cloneInteractiveCell(cell) {
    var shell = document.createElement('div');
    shell.className = 'mobile-card-actions';
    var buttons = Array.from(cell.querySelectorAll('button, .btn, a[href]')).slice(0, 3);
    buttons.forEach(function (source) {
      var button = document.createElement('button');
      button.type = 'button';
      button.className = 'mobile-card-action';
      button.dataset.mobileProxyTarget = ensureId(source);
      button.setAttribute('aria-label', source.getAttribute('aria-label') || source.getAttribute('title') || textOf(source) || 'Thao tac');
      button.title = button.getAttribute('aria-label');
      var icon = source.querySelector('i');
      if (icon) button.appendChild(icon.cloneNode(true));
      else {
        var normalized = normalizeLabel(textOf(source));
        var fallback = document.createElement('i');
        fallback.className = normalized.indexOf('xoa') >= 0
          ? 'fa-solid fa-trash'
          : normalized.indexOf('sua') >= 0
            ? 'fa-solid fa-pen'
            : 'fa-solid fa-eye';
    ...
    if (!buttons.length) {
      ...
      label.className = 'mobile-card-readonly';
      label.textContent = textOf(cell) || 'Chi xem';
      shell.appendChild(label);
    }
    return shell;
  }

  function cloneSummaryCell(cell) {
    var item = document.createElement('div');
    item.className = 'mobile-card-info';
    item.dataset.mobileField = cell.dataset.mobileField || '';
    item.dataset.mobileRole = cell.dataset.mobileRole || '';
    var label = document.createElement('span');
    ...
    value.querySelectorAll('button, a, input, select, textarea').forEach(function (node) {
      if (node.matches('button, a')) {
        var span = document.createElement('span');
        span.innerHTML = node.innerHTML || node.textContent || '';
  ...
  function buildCard(row) {
    ...
    if (colspan) {
      ...
      empty.textContent = textOf(colspan);
      return empty;
    }
    ...
    card.dataset.sourceRow = ensureId(row);

    var head = document.createElement('div');
    head.className = 'mobile-list-card-head';
    var titleEl = document.createElement('strong');
    titleEl.className = 'mobile-card-title';
    titleEl.textContent = title ? textOf(title) : 'Ban ghi';
    ...
    if (select) {
      var sourceInput = select.querySelector('input');
      var proxy = sourceInput.cloneNode(true);
      proxy.className = sourceInput.className;
      proxy.checked = sourceInput.checked;
      proxy.dataset.mobileProxyTarget = ensureId(sourceInput);
      proxy.addEventListener('change', function () {
        sourceInput.checked = proxy.checked;
  ...
  function enhanceTable(wrapper) {
    ...
    if (!surface) {
      ...
      surface.setAttribute('aria-label', 'Danh sach mobile');
    ...
    Array.from(tbody.children).forEach(function (row) {
      ...
      if (card) surface.appendChild(card);
    });
  }
