(function () {
  ...
  function guardMapClosePopup(m) {
    ...
    m.closePopup = function () {
      gisDebugLog('map.closePopup', { guarded: Boolean(state.openPopupId && Date.now() > state.allowPopupCloseUntil) });
  ...
  function stopLeafletEvent(event) {
    ...
    if (typeof L.DomEvent.preventDefault === 'function') L.DomEvent.preventDefault(original);
  }
  ...
  function protectPopupElement(popup) {
    ...
    ['touchstart', 'touchend', 'pointerdown', 'pointerup'].forEach(type => {
      el.addEventListener(type, event => stopLeafletEvent(event), { passive: false });
    });
    const closeButton = el.querySelector('.leaflet-popup-close-button');
    if (closeButton && !closeButton.__thon09PopupCloseTracked) {
      closeButton.__thon09PopupCloseTracked = true;
      ['click', 'touchend', 'pointerup'].forEach(type => {
        closeButton.addEventListener(type, () => { state.allowPopupCloseUntil = Date.now() + 1000; }, { passive: true });
      });
    }
  }

  function bindHouseholdPopup(marker, row) {
    const popup = L.popup({ closeButton: true, autoClose: true, closeOnClick: false, autoPan: true, keepInView: true }).setContent(popupHtml(row));
    popup.on('add', () => protectPopupElement(popup));
    ...
    marker.on('popupclose', () => {
      gisDebugLog('popup close', { id: row.id, renderingMarkers: state.renderingMarkers, reopeningPopup: state.reopeningPopup, allowPopupCloseUntil: state.allowPopupCloseUntil, now: Date.now() });
      if (state.renderingMarkers || state.reopeningPopup) return;
      const rowId = String(row.id || '');
      if (state.openPopupId !== rowId) return;
      if (Date.now() <= state.allowPopupCloseUntil) {
        state.openPopupId = '';
        return;
      }
      restoreOpenPopup();
  ...
  function restoreOpenPopup() {
    const id = String(state.openPopupId || '');
    if (!id) return;
    const marker = state.markers.get(id);
    if (!marker) return;
    state.reopeningPopup = true;
    setTimeout(() => {
      try {
        activateMarker(marker, marker.__thon09HouseholdRow || { id });
        marker.openPopup();
      } finally {
        setTimeout(() => { state.reopeningPopup = false; }, 80);
      }
    }, 0);
  }
  ...
  function popupHtml(row) {
    ...
    return '<div class="gis-household-popup gis-smart-popup">' +
      ...
      '<div class="gis-smart-popup-actions">' +
        '<button class="btn btn-sm btn-primary" type="button" onclick="thon09GisOpenHousehold(' + row.id + ')"><i class="fa-solid fa-folder-open"></i> Hồ sơ số</button>' +
        '<button class="btn btn-sm btn-success" type="button" onclick="thon09GisRouteToHousehold(' + row.id + ')"><i class="fa-solid fa-route"></i> Chỉ đường</button>' +
        '<button class="btn btn-sm btn-outline-secondary" type="button" onclick="thon09GisOpenHouseholdGallery(' + row.id + ')"><i class="fa-solid fa-images"></i> Xem ảnh</button>' +
        (phone ? '<a class="btn btn-sm btn-outline-primary" href="tel:' + escapeHtml(phone) + '"><i class="fa-solid fa-phone"></i> Gọi điện</a>' : '') +
