  ...
  async function renderPersonPolicySummary(citizenId) {
    ...
    try {
      ...
      if (items.length) {
        ...
        host.innerHTML = labels.length ? labels.map(label => '<span class="badge bg-info text-dark me-1 mb-1"><i class="fa-solid fa-check"></i> ' + esc(label) + '</span>').join('') : '<div class="text-muted">Chưa có diện chính sách.</div>';
      }
