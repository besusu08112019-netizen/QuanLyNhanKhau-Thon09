...
function gisEsriWorldImageryLayer(url, options) {
const layer = L.layerGroup(null, options);
layer.url = url;
layer.opts = options || {};
...
const size = typeof map.getSize === 'function' ? map.getSize() : { x: 1200, y: 800 };
const width = Math.max(320, Math.round(size.x || 1200));
const height = Math.max(240, Math.round(size.y || 800));
return url.replace(/\/tile\/\{z\}\/\{y\}\/\{x\}$/, '/export')
 + '?bbox=' + [b.getWest(), b.getSouth(), b.getEast(), b.getNorth()].map(value => Number(value).toFixed(7)).join(',')
 + '&bboxSR=4326&imageSR=3857&size=' + width + ',' + height
 + '&format=jpgpng&transparent=false&f=image';
};
const refresh = () => {
const map = attachedMap || layer._map;
if (!map || typeof L.imageOverlay !== 'function') return;
const bounds = map.getBounds();
const next = L.imageOverlay(exportUrl(map), bounds, { opacity: 1, interactive: false, crossOrigin: true });
pending = next;
next.on?.('load', () => {
if (pending !== next) return;
if (overlay && overlay !== next) layer.removeLayer(overlay);
overlay = next;
pending = null;
});
next.on?.('error', () => { if (pending === next) pending = null; });
layer.addLayer(next);
};
const schedule = () => {
...
layer.on?.('add', () => {
attachedMap = layer._map || attachedMap;
refresh();
attachedMap?.on?.('moveend zoomend resize', schedule);
});
layer.on?.('remove', () => {
if (timer) clearTimeout(timer);
attachedMap?.off?.('moveend zoomend resize', schedule);
pending = null;
overlay = null;
attachedMap = null;
layer.clearLayers?.();
...
esriWorldImagery: gisEsriWorldImageryLayer(blank, { maxZoom: 20, maxNativeZoom: 19, minZoom: 0, tileSize: 256, zoomOffset: 0, updateWhenIdle: false, updateWhenZooming: false, keepBuffer: 8, detectRetina: false, attribution: '' })
};
}
return {
...
esriWorldImagery: gisEsriWorldImageryLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { maxZoom: 20, maxNativeZoom: 19, minZoom: 0, tileSize: 256, zoomOffset: 0, attribution: 'Tiles &copy; Esri &mdash; Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community', updateWhenIdle: false, updateWhenZooming: false, keepBuffer: 8, detectRetina: false, crossOrigin: true })
};
}
