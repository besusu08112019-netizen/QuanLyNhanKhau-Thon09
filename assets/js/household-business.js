(() => {
  ...
  async function load() {
    ...
    try {
      ...
      if (rows) rows.innerHTML = items.length ? items.map((row, index) => rowHtml(row, start + index + 1)).join('') : '<tr><td colspan="13" class="text-center text-muted py-4">Chưa có hồ sơ sản xuất & kinh doanh</td></tr>';
      renderPager(data);
  ...
  function rowHtml(row, index) {
    ...
    const ocop = row.is_ocop ? '<span class="badge text-bg-success">' + esc(row.ocop_star ? row.ocop_star + ' sao' : 'Có') + '</span>' : '<span class="badge text-bg-light">Không</span>';
    return '<tr>'
      + '<td>' + index + '</td>'
      ...
      + '<td>' + esc(row.economic_type || '') + '</td>'
      + '<td>' + esc(row.business_scale || '') + '</td>'
  ...
  function actionButtons(row) {
    ...
    return [
      ...
      can('household_business', 'update') ? '<button class="btn btn-sm btn-outline-primary" type="button" onclick="window.openHouseholdBusinessForm(' + id + ')">Sửa</button>' : '',
      can('household_business', 'delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" onclick="window.deleteHouseholdBusiness(' + id + ')">Xóa</button>' : ''
  ...
  async function openForm(id = null) {
    ...
    $('#businessHouseholdAutocomplete').disabled = !!id;
    $('#businessHouseholdAutocomplete').value = '';
    $('#businessHouseholdSelected').textContent = '';
    hideSuggestions();
    await ensureCatalogs();
    renderExistingFiles([]);
  ...
  function selectHousehold(item) {
    ...
    form.elements.owner_name.value = item.head_citizen_name || '';
    ...
    $('#businessHouseholdAutocomplete').value = item.household_code + ' - ' + (item.head_citizen_name || '');
    $('#businessHouseholdSelected').textContent = item.address || '';
    hideSuggestions();
  }
  ...
  function setForm(form, row) {
    ...
    $('#businessHouseholdAutocomplete').value = row.household_code + ' - ' + (row.head_citizen_name || '');
    $('#businessHouseholdSelected').textContent = row.address || '';
    Object.entries({
  ...
  function detailHtml(row) {
    ...
    return '<article class="person-detail-card person-detail-dynamic"><section class="person-detail-hero"><div class="person-detail-identity"><span>Hồ sơ sản xuất/kinh doanh</span><h3>' + esc(row.business_name || row.household_code) + '</h3><div class="person-detail-codes"><strong>Mã hộ: ' + esc(row.household_code) + '</strong><strong>Chủ hộ: ' + esc(row.head_citizen_name) + '</strong></div></div></section><div class="person-detail-sections">'
      ...
      + section('Thông tin cơ sở', [['Tên cơ sở', row.business_name], ['Chủ cơ sở', row.owner_name], ['Loại hình', row.business_type_label], ['Ngành sản xuất', row.production_sector], ['Ngành kinh doanh', row.business_sector], ['Ngày bắt đầu', date(row.start_date)], ['Trạng thái', row.status_label]])
      + section('Thông tin kinh tế', [['Loại hình kinh tế', row.economic_type], ['Quy mô', row.business_scale], ['Sản phẩm chính', (row.main_products || []).join(', ')], ['Số lao động', num(row.worker_count)], ['Doanh thu năm', row.annual_revenue ? num(row.annual_revenue) : '']])
      + section('Thông tin OCOP', [['Tham gia', row.is_ocop ? 'Có' : 'Không'], ['Sản phẩm', row.ocop_product], ['Số sao', row.ocop_star ? row.ocop_star + ' sao' : '']])
      + section('Thông tin ATTP', [['Chứng nhận', row.food_safety_certified ? 'Có' : 'Không'], ['Số chứng nhận', row.food_safety_certificate_no], ['Ngày hết hạn', date(row.food_safety_expired_date)]])
      + section('Thông tin BHXH', [['Tham gia', row.social_insurance ? 'Có' : 'Không'], ['Lao động tham gia', row.social_insurance ? num(row.insured_workers) : '']])
      + fileGallery('Ảnh', files.filter(file => file.file_kind === 'IMAGE'), row.id)
      + fileGallery('Tài liệu', files.filter(file => file.file_kind === 'DOCUMENT'), row.id)
      + '</div><div class="mt-3 d-flex gap-2 flex-wrap"><button class="btn btn-outline-primary btn-sm" type="button" onclick="window.showHousehold && window.showHousehold(' + Number(row.household_id) + ')">Mở hồ sơ Hộ gia đình</button></div>'
