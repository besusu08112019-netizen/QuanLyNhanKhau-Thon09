...
<head>
  ...
  <link rel="stylesheet" href="assets/css/app.css?v=20260629-login-reference-1">
</head>
<body>
  <div id="toastHost" class="toast-container position-fixed top-0 end-0 p-3"></div>

  <main id="loginView" class="login-view">
    <section class="login-shell login-shell-reference" aria-label="Đăng nhập hệ thống quản lý hành chính Thôn 09">
        ...
        <section class="login-panel">
          <div class="login-brand" aria-label="Thôn 09 xã Hồng Phong">
            <div class="login-logo login-logo-emblem" aria-hidden="true">
              ...
              <strong>09</strong>
              <small>Đoàn kết - Dân chủ - Phát triển</small>
            </div>
            <div class="login-title-block">
              <p>Hệ thống</p>
              <h1><span>Quản lý hành chính</span><strong>Thôn 09</strong><span>Xã Hồng Phong</span></h1>
              <div class="title-ornament"><span></span><i class="fa-solid fa-star"></i><span></span></div>
            </div>
          </div>
          <form id="loginForm" class="login-form" novalidate>
            ...
            <button class="btn login-submit" type="submit"><i class="fa-solid fa-arrow-right-to-bracket" aria-hidden="true"></i><span>Đăng nhập</span></button>
          </form>
      ...
      <aside class="login-column login-column-intro" aria-label="Giới thiệu Thôn 09">
        <div class="login-top-meta">
          ...
          <span><i class="fa-solid fa-cloud-sun"></i><strong>28°C</strong> Nhiều mây</span>
        </div>
        <div class="intro-content">
          <article class="history-card">
            <h2>Giới thiệu Thôn 09 - Xã Hồng Phong</h2>
            <div class="history-divider"><span></span><i class="fa-solid fa-star"></i><span></span></div>
            ...
            <div id="loginHistoryText" class="history-text" data-setting-key="hamletHistory">Thôn 09 thuộc xã Hồng Phong, được hình thành trong quá trình sắp xếp, kiện toàn đơn vị dân cư nhằm đáp ứng yêu cầu phát triển kinh tế - xã hội, nâng cao hiệu quả quản lý nhà nước và xây dựng đời sống văn hóa tại cơ sở. Trải qua nhiều giai đoạn phát triển, nhân dân Thôn 09 luôn đoàn kết, cần cù lao động, tích cực phát triển kinh tế, xây dựng nếp sống văn minh, giữ gìn và phát huy bản sắc văn hóa truyền thống.</div>
          </article>
          <section class="login-info-card" aria-label="Thông tin cơ bản">
            <div class="info-heading"><span><i class="fa-solid fa-users"></i></span><h3>Thông tin cơ bản</h3></div>
            <div class="login-stats" id="loginStats" aria-live="polite">
              ...
              <div class="public-stat stat-temp"><i class="fa-solid fa-archway"></i><span>Tạm trú</span><strong data-stat="temporary_count">0</strong><small>người</small></div>
              <div class="public-stat stat-away"><i class="fa-solid fa-person-walking-luggage"></i><span>Tạm vắng</span><strong data-stat="away_count">0</strong><small>người</small></div>
          ...
          <section class="login-address-card" aria-label="Thông tin địa chỉ">
            <div class="address-left"><i class="fa-solid fa-location-dot"></i><div><h3>Thông tin địa chỉ</h3><dl><div><dt>Thôn:</dt><dd>Thôn 09</dd></div><div><dt>Xã:</dt><dd>Hồng Phong</dd></div><div><dt>Huyện:</dt><dd>Nho Quan</dd></div><div><dt>Tỉnh:</dt><dd>Ninh Bình</dd></div></dl></div></div>
            <div class="address-right"><p><i class="fa-solid fa-building-columns"></i><strong>Nhà văn hóa Thôn 09</strong><span>Thôn 09, Xã Hồng Phong, Huyện Nho Quan, Tỉnh Ninh Bình</span></p><p><i class="fa-solid fa-phone"></i><strong>Trưởng thôn:</strong><span>................................</span></p><p><i class="fa-solid fa-phone-volume"></i><strong>Số điện thoại:</strong><span>................................</span></p></div>
          </section>
        </div>
        <div class="login-slogan"><span></span><i class="fa-solid fa-star"></i><strong>Đoàn kết - Dân chủ - Phát triển</strong><i class="fa-solid fa-star"></i><span></span></div>
      </aside>
    </section>
  </main>

  <div id="appView" class="app-shell d-none">
    <aside class="sidebar"><div class="sidebar-brand"><span class="state-mark small-mark">09</span><div><strong>Thôn 09</strong><small>xã Hồng Phong</small></div></div><nav class="nav flex-column gap-1"><button class="nav-link active" data-screen="dashboard">Tổng quan</button><button class="nav-link" data-screen="households">Hộ dân</button><button class="nav-link" data-screen="persons">Nhân khẩu</button><button class="nav-link" data-screen="reports">Báo cáo</button></nav></aside>

    <section class="main-area">
      <header class="topbar"><button id="sidebarToggle" class="btn btn-outline-secondary btn-sm d-lg-none" type="button">Menu</button><div><h2 id="screenTitle" class="h5 mb-0">Tổng quan</h2><small class="text-muted">Quản Lý Nhân Khẩu Thôn 09 xã Hồng Phong</small></div><div class="ms-auto text-end"><div id="currentUser" class="small text-muted"></div><button id="logoutBtn" class="btn btn-link btn-sm text-decoration-none px-0">Đăng xuất</button></div></header>
      ...
      <section id="dashboardScreen" class="screen active">
        <form id="dashboardFilters" class="content-card dashboard-filter mb-3"><div class="row g-3 align-items-end"><div class="col-md-3"><label class="form-label">Từ ngày</label><input name="dateFrom" type="date" class="form-control"></div><div class="col-md-3"><label class="form-label">Đến ngày</label><input name="dateTo" type="date" class="form-control"></div><div class="col-md-2"><label class="form-label">Trạng thái hộ</label><select name="householdStatus" class="form-select"><option value="">Tất cả</option><option value="ACTIVE">Đang hoạt động</option></select></div><div class="col-md-2"><label class="form-label">Thường trú</label><select name="residencyStatus" class="form-select"><option value="">Tất cả</option><option value="PERMANENT">Thường trú</option><option value="TEMPORARY">Tạm trú</option></select></div><div class="col-md-2"><label class="form-label">Hiện tại</label><select name="presenceStatus" class="form-select"><option value="">Tất cả</option><option value="AT_HOME">Ở nhà</option><option value="AWAY">Đi vắng</option></select></div><div class="col-12 d-flex justify-content-end gap-2"><button id="dashboardResetBtn" class="btn btn-light" type="button">Xóa lọc</button><button class="btn btn-primary" type="submit">Lọc dữ liệu</button></div></div></form>
        <div class="row g-3" id="dashboardCards"></div><div class="row g-3 mt-1"><div class="col-lg-3"><div class="content-card"><h3 class="section-title">Dân số theo giới tính</h3><div id="genderChart" class="chart-list"></div></div></div><div class="col-lg-3"><div class="content-card"><h3 class="section-title">Dân số theo độ tuổi</h3><div id="ageChart" class="chart-list"></div></div></div><div class="col-lg-3"><div class="content-card"><h3 class="section-title">Tình trạng hộ</h3><div id="householdChart" class="chart-list"></div></div></div><div class="col-lg-3"><div class="content-card"><h3 class="section-title">Cư trú</h3><div id="residencyChart" class="chart-list"></div></div></div></div>
      </section>

      <section id="householdsScreen" class="screen"><div class="toolbar"><input id="householdSearch" class="form-control" placeholder="Tìm mã hộ, chủ hộ, địa chỉ, điện thoại"><select id="householdPageSize" class="form-select w-auto"><option>20</option><option>50</option><option>100</option></select><button id="householdAddBtn" class="btn btn-primary">Thêm hộ dân</button><button id="householdBulkDeleteBtn" class="btn btn-outline-danger">Xóa đã chọn</button></div><div class="content-card table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th><input type="checkbox" id="householdCheckAll"></th><th>Mã hộ</th><th>Chủ hộ</th><th>Địa chỉ</th><th>Ở nhà</th><th>Đi vắng</th><th>Diện hộ</th><th></th></tr></thead><tbody id="householdRows"></tbody></table></div><div id="householdPager" class="pager"></div></section>

      <section id="personsScreen" class="screen"><div class="toolbar"><input id="personSearch" class="form-control" placeholder="Tìm họ tên, mã nhân khẩu, CCCD, điện thoại, mã hộ"><input id="personHouseholdFilter" class="form-control" placeholder="Mã hộ" style="max-width:180px"><select id="personPageSize" class="form-select w-auto"><option>20</option><option>50</option><option>100</option></select><button id="personAddBtn" class="btn btn-primary">Thêm nhân khẩu</button><button id="personBulkDeleteBtn" class="btn btn-outline-danger">Xóa đã chọn</button></div><div class="content-card table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th><input type="checkbox" id="personCheckAll"></th><th>Mã hộ</th><th>Mã nhân khẩu</th><th>Họ tên</th><th>Ngày sinh</th><th>CCCD</th><th>Thường trú</th><th>Hiện tại</th><th></th></tr></thead><tbody id="personRows"></tbody></table></div><div id="personPager" class="pager"></div></section>
  ...
  <script src="assets/js/app.js?v=20260629-login-reference-1"></script>
...
<script id="thon09-dashboard-final-force">
(function(){function run(){try{if(window.App&&localStorage.getItem('thon09_token'))window.App.token=localStorage.getItem('thon09_token');if(typeof window.loadDashboard==='function'&&document.querySelector('#appView')&&!document.querySelector('#appView').classList.contains('d-none'))window.loadDashboard();}catch(e){console.error(e);}}setTimeout(run,500);setTimeout(run,1500);setTimeout(run,3500);})();
</script>
