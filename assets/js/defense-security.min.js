(() => {
'use strict';
const state = { tab: 'nvqs', page: 1, pageSize: 20, sort: '', direction: 'ASC', catalogs: null, metric: '', metricLabel: '' };
...
actions.register('defenseSecurity.metric', ctx => { state.metric = ctx.dataset.metric || ''; state.metricLabel = ctx.dataset.label || ctx.querySelector('span')?.textContent.trim() || ''; state.page = 1; switchTab(ctx.dataset.tab || 'nvqs', false, true); loadList(); });
...
['ChÆ°a cáº­p nháº­t sÆ¡ tuyá»ƒn', n.preliminary_missing, 'fa-clipboard-question', 'nvqs', 'preliminary_missing'],
['ChÆ°a cáº­p nháº­t khÃ¡m tuyá»ƒn', n.medical_missing, 'fa-stethoscope', 'nvqs', 'medical_missing'],
...
['Tá»•ng dÃ¢n quÃ¢n', m.total, 'fa-people-group', 'militia', 'total'],
['DÃ¢n quÃ¢n Ä‘ang tham gia', m.active, 'fa-user-check', 'militia', 'active'],
['Tá»•ng ANTT cÆ¡ sá»Ÿ', s.total, 'fa-shield-halved', 'security_force', 'total'],
['ANTT Ä‘ang hoáº¡t Ä‘á»™ng', s.active, 'fa-user-shield', 'security_force', 'active']
];
$('#defenseSecurityDashboard').innerHTML = cards.map(([label, value, icon, tab, metric]) => '<article class="content-card rural-water-kpi" role="button" tabindex="0" data-platform-action="defenseSecurity.metric" data-tab="' + esc(tab) + '" data-metric="' + esc(metric) + '" data-label="' + esc(label) + '"><i class="fa-solid ' + icon + '"></i><span>' + esc(label) + '</span><strong>' + esc(value) + '</strong></article>').join('');
}
...
$('#defenseTotalCount').textContent = 'Tá»•ng sá»‘: ' + fmt(data.total || 0) + (state.metric ? ' ngÆ°á»i' : ' há»“ sÆ¡');
$('#defenseListTitle').textContent = state.metric ? ('Danh sÃ¡ch: ' + (state.metricLabel || state.metric)) : (state.tab === 'nvqs' ? 'Danh sÃ¡ch nghÄ©a vá»¥ quÃ¢n sá»±' : (state.tab === 'militia' ? 'Danh sÃ¡ch dÃ¢n quÃ¢n tá»± vá»‡' : 'Danh sÃ¡ch ANTT cÆ¡ sá»Ÿ'));
renderHead();
...
function renderHead() {
const nvqs = ['STT','MÃ£ NK','Há» tÃªn','NgÃ y sinh','Tuá»•i','Giá»›i tÃ­nh','MÃ£ há»™','Chá»§ há»™','Äá»‹a chá»‰','NÄƒm','Tráº¡ng thÃ¡i','LÃ½ do thuá»™c KPI','Thao tÃ¡c'];
const militia = ['STT','MÃ£ NK','Há» tÃªn','NgÃ y sinh','Tuá»•i','Giá»›i tÃ­nh','MÃ£ há»™','Chá»§ há»™','Äá»‹a chá»‰','Loáº¡i','Chá»©c vá»¥','Tráº¡ng thÃ¡i','LÃ½ do thuá»™c KPI','Thao tÃ¡c'];
const security = ['STT','MÃ£ NK','Há» tÃªn','NgÃ y sinh','Tuá»•i','Giá»›i tÃ­nh','MÃ£ há»™','Chá»§ há»™','Äá»‹a chá»‰','Tá»• ANTT','Chá»©c vá»¥','Tráº¡ng thÃ¡i','LÃ½ do thuá»™c KPI','Thao tÃ¡c'];
...
const citizenId = row.citizen_id || row.id || 0;
const view = '<button class="btn btn-sm btn-outline-secondary" data-platform-action="persons.detail" data-id="' + esc(citizenId) + '" title="Xem nhÃ¢n kháº©u"><i class="fa-solid fa-eye"></i></button>';
const edit = row.id ? '<button class="btn btn-sm btn-outline-primary" data-platform-action="defenseSecurity.edit" data-id="' + esc(row.id) + '" title="Cáº­p nháº­t nghiá»‡p vá»¥"><i class="fa-solid fa-pen"></i></button>' : '<button class="btn btn-sm btn-outline-primary" data-platform-action="defenseSecurity.createForCitizen" data-citizen-id="' + esc(citizenId) + '" data-label="' + esc(row.full_name) + '" data-address="' + esc(row.address) + '" title="Cáº­p nháº­t nghiá»‡p vá»¥"><i class="fa-solid fa-pen-to-square"></i></button>';
const del = row.id ? ' <button class="btn btn-sm btn-outline-danger" data-platform-action="defenseSecurity.delete" data-id="' + esc(row.id) + '" title="XÃ³a"><i class="fa-solid fa-trash"></i></button>' : '';
const actions = '<td class="text-end">' + view + ' ' + edit + del + '</td>';
const age = ageInStatsYear(row.date_of_birth, row.recruitment_year || $('#defenseYearFilter')?.value);
const base = '<td>' + stt + '</td><td>' + esc(row.citizen_code) + '</td><td><button class="btn btn-link p-0 person-name-link" data-platform-action="persons.detail" data-id="' + esc(citizenId) + '">' + esc(row.full_name) + '</button></td><td>' + esc(formatDate(row.date_of_birth)) + '</td><td>' + esc(age) + '</td><td>' + esc(row.gender) + '</td><td>' + esc(row.household_code) + '</td><td>' + esc(row.head_citizen_name) + '</td><td>' + esc(row.address || row.area_code) + '</td>';
if (state.tab === 'nvqs') return '<tr>' + base + '<td>' + esc(row.recruitment_year) + '</td><td>' + esc([row.registered_status_label, row.preliminary_status_label, row.medical_exam_status_label, row.eligibility_status_label, row.selection_status_label].filter(Boolean).join(' / ')) + '</td><td>' + esc(row.kpi_reason || '') + '</td>' + actions + '</tr>';
if (state.tab === 'militia') return '<tr>' + base + '<td>' + esc(row.militia_type_label) + '</td><td>' + esc(row.position_name || row.unit_name) + '</td><td>' + esc(row.participation_status_label) + '</td><td>' + esc(row.kpi_reason || '') + '</td>' + actions + '</tr>';
return '<tr>' + base + '<td>' + esc(row.team_name) + '</td><td>' + esc(row.position_label) + '</td><td>' + esc(row.participation_status_label) + '</td><td>' + esc(row.kpi_reason || '') + '</td>' + actions + '</tr>';
}
...
function switchTab(tab, reload = true, keepMetric = false) {
state.tab = tab || 'nvqs';
state.page = 1;
if (!keepMetric) { state.metric = ''; state.metricLabel = ''; }
...
if (!id) return;
const endpoint = state.tab === 'nvqs' ? '/nvqs/' : (state.tab === 'militia' ? '/militia/' : '/security-force/');
try { await openForm(await api(API + endpoint + encodeURIComponent(id))); } catch (error) { toast(error.message, 'danger'); }
}
function bindForm() {
...
return '<button type="button" class="list-group-item list-group-item-action" data-defense-citizen-option data-citizen-id="' + esc(row.id) + '" data-label="' + esc(label) + '" data-detail="' + esc(detail) + '"><strong>' + esc(label) + '</strong>' + (detail ? '<br><small>' + esc(detail) + '</small>' : '') + '</button>';
}).join('');
box.querySelectorAll('[data-defense-citizen-option]').forEach(button => { const handler = event => { event.preventDefault(); event.stopImmediatePropagation?.(); selectCitizen(button); }; button.onpointerdown = handler; button.onmousedown = handler; button.onclick = handler; });
...
function ageInStatsYear(value, statsYear) {
if (!value) return '';
const year = Number(statsYear || new Date().getFullYear());
