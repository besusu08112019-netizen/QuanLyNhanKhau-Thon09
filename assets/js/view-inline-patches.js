...
if(!window.Thon09Print){showMessage('Print Framework is not ready','warning');return;}
var popup=window.Thon09Print.render({
  title: printData.title||report.title||'Báo cáo',
  type: reportType(),
  paperSize: 'A4',
...
if(!popup)showMessage('Trình duyệt đang chặn cửa sổ in. Vui lòng cho phép popup.','warning');
}catch(e){showMessage(e.message||'Không in được báo cáo.','danger');}
}
function lockReportTypes(){var select=q('#reportTypeSelect');if(!select)return;var value=select.value||'summary';var options=[['summary','Báo cáo tổng hợp'],['population','Báo cáo nhân khẩu'],['household','Báo cáo hộ gia đình'],['public-assets','Cong trinh cong cong - Danh sach'],['public-assets-located','Cong trinh cong cong - Da co GPS'],['public-assets-missing-gps','Cong trinh cong cong - Chua co GPS'],['public-assets-inventory','Cong trinh cong cong - Kiem ke tai san'],['houses','Nha o va cong trinh - Danh sach'],['houses-degraded','Nha o xuong cap'],['houses-temporary','Nha tam'],['houses-fire-risk','Nha nguy co PCCC'],['houses-missing-gps','Nha chua co GPS'],['household-business-production','Ho san xuat'],['household-business-trade','Ho kinh doanh'],['household-business-sector','Ho SXKD theo nganh nghe'],['household-business-status','Ho SXKD theo trang thai'],['agriculture','San xuat nong nghiep - Danh sach'],['agriculture-producers','Chu the san xuat nong nghiep'],['agriculture-area','Dien tich san xuat nong nghiep'],['agriculture-crop','Cay trong'],['agriculture-season','Mua vu'],['agriculture-production','San luong nong nghiep'],['agriculture-damage','Thiet hai nong nghiep'],['livestock','Vat nuoi - Danh sach'],['livestock-by-type','Vat nuoi theo loai'],['livestock-vaccinated','Vat nuoi da tiem phong'],['livestock-unvaccinated','Vat nuoi chua tiem phong'],['livestock-disease','Vat nuoi co dich benh'],['vehicles','Xe co - Danh sach'],['vehicles-by-type','Xe co theo loai'],['vehicles-missing-plate','Xe chua co bien so'],['vehicles-expired-inspection','Xe het han kiem dinh'],['vehicles-expired-insurance','Xe het han bao hiem'],['contributions-list','Dong gop ho - Danh sach'],['contributions-collection','Dong gop ho - Thu tien'],['contributions-unpaid-list','Dong gop ho - Chua nop'],['contributions-partial','Dong gop ho - Nop mot phan'],['contributions-exempt','Dong gop ho - Mien giam'],['contributions-summary','Dong gop ho - Tong hop'],['contributions-year-summary','Dong gop ho - Tong hop nam'],['contributions-by-contribution','Dong gop ho - Theo khoan thu'],['gis','GIS - Ho gia dinh'],['gis-located','GIS - Da dinh vi'],['gis-unlocated','GIS - Chua dinh vi'],['digital-profile','Ho so so'],['profile-complete','Ho so hoan chinh'],['profile-missing-photo','Ho so thieu anh'],['profile-missing-documents','Ho so thieu giay to'],['profile-incomplete','Ho so chua hoan thien'],['temporary_residence','Báo cáo tạm trú'],['temporary_absence','Báo cáo tạm vắng'],['migration','Báo cáo biến động'],['health_insurance','Báo cáo Bảo hiểm y tế'],['health-insurance-missing','Danh sách chưa tham gia BHYT'],['health-insurance-expiring','Danh sách BHYT sắp hết hạn (30 ngày)'],['health-insurance-expired','Danh sách BHYT đã hết hạn'],['health-insurance-household','Thống kê BHYT theo hộ'],['health-insurance-area','Thống kê BHYT theo khu vực'],['party_member','Báo cáo Đảng viên'],['meritorious_person','Báo cáo người có công'],['disabled_person','Báo cáo người khuyết tật'],['age','Báo cáo theo độ tuổi'],['gender','Báo cáo theo giới tính'],['youth_union_member','Báo cáo Đoàn viên'],['poor-households','Báo cáo hộ nghèo'],['near-poor-households','Báo cáo hộ cận nghèo'],['labor','Báo cáo lao động'],['elderly','Báo cáo người cao tuổi'],['children','Báo cáo trẻ em']];var html=options.map(function(item){return '<option value="'+item[0]+'">'+item[1]+'</option>';}).join('');if(select.innerHTML.replace(/\s+/g,' ').trim()!==html.replace(/\s+/g,' ').trim())select.innerHTML=html;if(!Array.prototype.some.call(select.options,function(o){return o.value===value;}))value='summary';select.value=value;}
...
function renderLoaderError(screen,loader){var target=targetFor(screen);if(!target||target.id!==screen+'Screen'||target.dataset.loaderErrorRendered==='1')return;target.dataset.loaderErrorRendered='1';var host=target.querySelector('.module-placeholder-screen, .content-card, .dashboard-kpi-grid, .agri-kpi-grid')||target;host.innerHTML='<div class="empty-state text-muted py-4 text-center">Không tải được module. Vui lòng tải lại trang hoặc kiểm tra file JavaScript: '+loader+'</div>';}
...
function setSidebarSectionOpen(section,open,persist){if(!section)return;var toggle=section.querySelector(':scope > .nav-section-title, :scope > .sidebar-accordion-toggle');section.classList.toggle('is-open',!!open);section.classList.toggle('is-collapsed',!open);if(toggle){toggle.setAttribute('aria-expanded',open?'true':'false');if(persist&&toggle.dataset.sidebarGroupToggle){try{localStorage.setItem('thon09_sidebar_group_'+toggle.dataset.sidebarGroupToggle,open?'1':'0');}catch(error){}}}}
function registerNavigationPatchActions(){var actions=window.Thon09Platform&&window.Thon09Platform.actions;if(window.__thon09NavigationPatchActionsRegistered||!actions||typeof actions.register!=='function')return;window.__thon09NavigationPatchActionsRegistered=true;actions.register('navigation.sidebarGroup.toggle',function(context){var section=context.target&&context.target.closest&&context.target.closest('.nav-section');setSidebarSectionOpen(section,!(section&&section.classList.contains('is-open')),true);});}
