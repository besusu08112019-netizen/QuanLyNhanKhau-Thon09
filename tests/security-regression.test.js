...

{
  ...
  assert.match(auth, /LOGIN_MAX_FAILURES\) \{/);
  assert.match(auth, /\$this->fail\([^;]+,\s*429\)/);
...

{
  ...
  assert.match(tenantGuard, /statusForHost\(PortalContext::host\(\)\)/);
  ...
  assert.match(tenantGuard, /http_response_code\(423\)/);
  ...
  assert.match(appView, /href="\/assets\/css\/app\.min\.css(?:\?v=[^"]+)?"/);
  assert.match(appView, /src="\/assets\/js\/pwa\.min\.js(?:\?v=[^"]+)?"/);
...

{
  ...
  assert.match(sprint10, /Super Admin/);
  assert.match(sprint10, /showToast\([^;]+,\s*'warning'\)/);
  assert.match(sprint10, /window\.resetUserPassword = resetUserPassword10/);
  assert.match(sprint10, /users\.resetPassword/);
  assert.ok(sprint10.indexOf("const row = await api('/api/users/' + id);") < sprint10.indexOf('const password = prompt('), 'sprint10 must check target role before prompting for reset password');
  const sprint8 = read('assets/js/sprint8.js');
  assert.match(sprint8, /function patchUserManagementUi/);
  assert.match(sprint8, /SUPER_ADMIN/);
  assert.match(sprint8, /row\.role === 'SUPER_ADMIN' \? 'ADMIN' : row\.role/);
}
...

{
  ...
  assert.match(deploy, /EXPECTED_PWA_VERSION:\s*tenant-pwa-v20260911-qpan-kpi-adaptive-1/);
  assert.match(deploy, /hashlib\.sha256\(body\)\.hexdigest\(\)/);
  assert.doesNotMatch(deploy, /server-dir:\s*\.\.\//);
  assert.doesNotMatch(deploy, /server-dir:\s*\/\s*$/m);
  assert.doesNotMatch(deploy, /Deploy to Thon 09 via FTPS/);
  assert.doesNotMatch(deploy, /Deploy to Thon 10 via FTPS/);
  assert.doesNotMatch(deploy, /server-dir:\s*\.\.\/public_html\//);
...

{
  ...
  assert.match(policyAlerts, /const SUMMARY_SCREENS = new Set\(\[[^\]]*'dashboard'[^\]]*'dashboardPopulation'[^\]]*'persons'[^\]]*'households'[^\]]*\]\)/);
...

{
  ...
  assert.doesNotMatch(artifact, /'\.htaccess'/);
  assert.doesNotMatch(artifact, /'uploads\/\.htaccess'/);
  const artifactValidator = read('tools/validate-production-artifact.js');
  assert.doesNotMatch(artifactValidator, /'\.htaccess'/);
  assert.doesNotMatch(artifactValidator, /'uploads\/\.htaccess'/);
}
...

{
  ...
  assert.match(repository, /PLATFORM_SETTINGS_SCHEMA_NOT_READY/);
  assert.doesNotMatch(repository, /CREATE TABLE IF NOT EXISTS platform_settings/);
...

{
  ...
  assert.match(read('views/control-center.php'), /general\.copyright/);
  assert.match(adminPanel, /name="copyright"/);
  const appJs = read('assets/js/app.utf8.min.js');
