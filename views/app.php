...
<body>
  ...
  <div id="appView" class="app-shell d-none">
    <aside class="sidebar">
      <div class="sidebar-brand">
        ...
        <div><strong>Thôn 09</strong><small>xã Hồng Phong</small></div>
      </div>
    ...
    <section class="main-area">
      <header class="topbar">
        ...
        <div><h2 id="screenTitle" class="h5 mb-0">Tổng quan</h2><small class="text-muted">Quản Lý Nhân Khẩu Thôn 09 xã Hồng Phong</small></div>
        <div class="ms-auto text-end"><div id="currentUser" class="small text-muted"></div><button id="logoutBtn" class="btn btn-link btn-sm text-decoration-none px-0">Đăng xuất</button></div>
      </header>
      ...
      <section id="dashboardScreen" class="screen active">
        <form id="dashboardFilters" class="content-card dashboard-filter mb-3">
          <div class="row g-3 align-items-end">
            <div class="col-md-3"><label class="form-label">Từ ngày</label><input name="dateFrom" type="date" class="form-control"></div>
            <div class="col-md-3"><label class="form-label">Đến ngày</label><input name="dateTo" type="date" class="form-control"></div>
            <div class="col-md-2"><label class="form-label">Trạng thái hộ</label><select name="householdStatus" class="form-select"><option value="">Tất cả</option><option value="ACTIVE">Đang hoạt động</option></select></div>
            <div class="col-md-2"><label class="form-label">Thường trú</label><select name="residencyStatus" class="form-select"><option value="">Tất cả</option><option value="PERMANENT">Thường trú</option><option value="TEMPORARY">Tạm trú</option></select></div>
            ...
            <div class="col-12 d-flex justify-content-end gap-2"><button id="dashboardResetBtn" class="btn btn-light" type="button">Xóa lọc</button><button class="btn btn-primary" type="submit">Lọc dữ liệu</button></div>
          </div>
        </form>
        <div class="row g-3" id="dashboardCards"></div>
        <div class="row g-3 mt-1">
          <div class="col-lg-3"><div class="content-card"><h3 class="section-title">Giới tính</h3><div id="genderChart" class="chart-list"></div></div></div>
          <div class="col-lg-3"><div class="content-card"><h3 class="section-title">Độ tuổi</h3><div id="ageChart" class="chart-list"></div></div></div>
          <div class="col-lg-3"><div class="content-card"><h3 class="section-title">Tình trạng hộ</h3><div id="householdChart" class="chart-list"></div></div></div>
          <div class="col-lg-3"><div class="content-card"><h3 class="section-title">Cư trú</h3><div id="residencyChart" class="chart-list"></div></div></div>
        </div>
      </section>
      ...
      <section id="householdsScreen" class="screen">
        ...
        <div class="content-card table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th><input type="checkbox" id="householdCheckAll"></th><th>Mã hộ</th><th>Chủ hộ</th><th>Địa chỉ</th><th>Ở nhà</th><th>Đi vắng</th><th>Diện hộ</th><th></th></tr></thead><tbody id="householdRows"></tbody></table></div>
      ...
      <section id="personsScreen" class="screen">
        ...
        <div class="content-card table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th><input type="checkbox" id="personCheckAll"></th><th>Mã hộ</th><th>Mã nhân khẩu</th><th>Họ tên</th><th>Ngày sinh</th><th>CCCD</th><th>Thường trú</th><th>Hiện tại</th><th></th></tr></thead><tbody id="personRows"></tbody></table></div>
        <div id="personPager" class="pager"></div>
      </section>

      <section id="reportsScreen" class="screen"><div class="content-card"><h3 class="section-title">Báo cáo</h3><p class="text-muted mb-0">Báo cáo, Excel, PDF và in phiếu sẽ được chuyển trong các sprint tiếp theo.</p></div></section>
    </section>
  </div>

  <div class="modal fade" id="householdModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-lg modal-dialog-scrollable"><form class="modal-content" id="householdForm">
  ...
  <div class="modal fade" id="personModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-scrollable"><form class="modal-content" id="personForm">
