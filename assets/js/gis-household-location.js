(function () {
  ...
  function bindHouseholdPopup(marker, row) {
    marker.bindPopup(popupHtml(row), { closeButton: true, autoClose: true, closeOnClick: false, autoPan: true, keepInView: true });
    marker.on('click', event => {
      if (event && event.originalEvent && window.L && L.DomEvent) L.DomEvent.stopPropagation(event.originalEvent);
    ...
    marker.on('popupclose', () => {
      ...
      if (state.openPopupId === String(row.id || '')) state.openPopupId = '';
    });
  ...
  function activateMarker(marker, row) {
    ...
    marker.setIcon(markerIcon(true, (marker.__thon09HouseholdRow || row || {}).__thumbnailObjectUrl || ''));
  }
