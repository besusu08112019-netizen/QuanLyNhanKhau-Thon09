(() => {
  ...
  function renderHouseholdMembers() {
    ...
    document.querySelector('#householdMemberRows').innerHTML = pageItems.map(row => '<tr ondblclick="openCitizenProfile(' + Number(row.id || 0) + ')"><td>' + escapeHtml(row.citizen_code || '') + '</td><td>' + (window.renderCitizenProfileLink ? window.renderCitizenProfileLink(row, row.full_name || '') : escapeHtml(row.full_name || '')) + '</td><td>' + formatDate(row.date_of_birth) + '</td><td>' + escapeHtml(row.identity_number || '') + '</td><td>' + escapeHtml(row.phone || '') + '</td><td>' + residencyLabel(row.residency_status) + '</td><td>' + presenceLabel(row.presence_status) + '</td><td class="text-end"><button class="btn btn-sm btn-outline-primary" onclick="openCitizenProfile(' + row.id + ')">Xem chi tiết</button></td></tr>').join('') || emptyRow(8, 'Không có thành viên phù hợp');
