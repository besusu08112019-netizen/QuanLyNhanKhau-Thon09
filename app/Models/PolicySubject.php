...
SQL);
    ...
    public function searchCitizens(string $query, int $limit = 12): array
    {
        ...
        $rows = $this->fetchAll(
            'SELECT c.id, c.citizen_code, c.full_name, c.gender, c.date_of_birth, h.household_code, h.address, h.area_code
             ...
             WHERE c.status <> "DELETED" AND h.status NOT IN ("DELETED","ENDED","MERGED","TRANSFERRED_OUT","MOVED_OUT","INACTIVE")
               ...
               AND (LOWER(c.citizen_code) LIKE :q OR LOWER(c.full_name) LIKE :q OR LOWER(COALESCE(c.identity_number,"")) LIKE :q OR LOWER(h.household_code) LIKE :q)
    ...
    {
        return 'SELECT cpr.*, pst.code AS policy_type_code, pst.name AS policy_type_name, c.citizen_code, c.full_name, c.gender, c.date_of_birth, h.household_code, h.address AS household_address, h.area_code,
