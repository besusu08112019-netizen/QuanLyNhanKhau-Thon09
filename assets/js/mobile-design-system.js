(function (window, document) {
  ...
  var uid = 0;
  var excludedTitleFields = new Set(['stt', 'so-thu-tu', 'thu-tu', 'checkbox', 'select', 'actions', 'thao-tac']);
  var titleMetadata = {
    businessHouseholds: ['ten co so', 'chu ho', 'nguoi dai dien', 'ho kinh doanh'],
    agriculture: ['ma thua', 'chu su dung', 'nguoi san xuat', 'khu dong'],
    livestock: ['chu ho', 'ma ho', 'loai vat nuoi'],
    vehicles: ['bien so', 'chu so huu', 'chu ho', 'loai xe'],
    contributions: ['ten ho', 'chu ho', 'khoan dong gop'],
    households: ['chu ho', 'ma ho', 'dia chi'],
    persons: ['ho va ten', 'nhan khau', 'cong dan', 'cccd'],
    publicAssets: ['ten cong trinh', 'ten tai san', 'ma cong trinh'],
    houses: ['chu ho', 'ma nha', 'dia chi']
  };
  ...
  function inferRole(label, index, cell) {
    ...
    if (label === 'stt' || label === 'so thu tu' || label === '#') return '';
  ...
  function screenIdForRow(row) {
    var screen = row && row.closest ? row.closest('.screen[id]') : null;
    return screen ? screen.id.replace(/Screen$/, '') : '';
  }

  function cellLabel(cell) {
    return normalizeLabel(cell ? cell.getAttribute('data-label') || '' : '');
  }

  function isTitleCandidate(cell) {
    if (!cell || isEmpty(cell)) return false;
    if (cell.querySelector('input, button, .btn, a[href]')) return false;
    if (cell.dataset.mobileRole === 'actions' || cell.dataset.mobileRole === 'select') return false;
    var label = cellLabel(cell);
    var field = cell.dataset.mobileField || inferField(label);
    if (excludedTitleFields.has(field) || excludedTitleFields.has(label)) return false;
    return true;
  }

  function scoreTitleCandidate(cell, index, config) {
    if (!isTitleCandidate(cell)) return -1;
    var label = cellLabel(cell);
    var field = cell.dataset.mobileField || '';
    var role = cell.dataset.mobileRole || '';
    var score = 0;
    if (config && config.indexOf(label) >= 0) score += 100 - config.indexOf(label);
    if (role === 'title') score += 80;
    if (field === 'name' || field === 'asset-name') score += 60;
    if (label.indexOf('ten') >= 0 || label.indexOf('chu ho') >= 0 || label.indexOf('chu so huu') >= 0 || label.indexOf('nguoi') >= 0) score += 40;
    if (role === 'header-meta' || field === 'code') score += 15;
    score -= index;
    return score;
  }

  function pickTitle(cells, row) {
    var config = titleMetadata[screenIdForRow(row)] || [];
    var scored = cells
      .map(function (cell, index) { return { cell: cell, score: scoreTitleCandidate(cell, index, config) }; })
      .filter(function (item) { return item.score >= 0; })
      .sort(function (a, b) { return b.score - a.score; });
    return scored.length ? scored[0].cell : null;
  }
