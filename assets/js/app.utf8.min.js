...
host.innerHTML = filtered.map(area => '<article class="gis-area-item" data-gis-area-id="' + Number(area.id) + '" aria-selected="' + (String(area.id) === String(App.gis.selectedAreaId || '') ? 'true' : 'false') + '"><button class="gis-area-main" type="button" onclick="focusGisArea(\'' + gisEscape(area.area_code) + '\')"><span><b>' + gisEscape(area.name) + '</b><small>' + gisEscape(area.area_code) + '</small></span><em>' + gisNumber(area.stats?.households) + ' h\u1ed9 / ' + gisNumber(area.stats?.citizens) + ' NK</em></button><div class="gis-area-row-actions">' + (canAccess('gis','update') ? '<button class="btn btn-sm btn-outline-primary" type="button" onclick="editGisArea(' + Number(area.id) + ')"><i class="fa-solid fa-pen-to-square"></i></button>' : '') + (canAccess('gis','delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" onclick="deleteGisArea(' + Number(area.id) + ')"><i class="fa-solid fa-trash"></i></button>' : '') + '</div></article>').join('') || '<div class="text-muted small py-2">Ch\u01b0a c\u00f3 khu v\u1ef1c b\u1ea3n \u0111\u1ed3</div>';
updateGisAreaActiveState(false);
}
...
const latLngs = gisPolygonLatLngs(area);
if (latLngs.length) { gisDebugTrace('map.fitBounds() focusGisArea', { areaCode }); App.gis.map.fitBounds(latLngs, { padding: [28, 28], maxZoom: 17 }); return; }
const center = gisAreaCenter(area);
if (center) { App.gis.map.setView(center, Math.max(App.gis.map.getZoom(), 17)); return; }
