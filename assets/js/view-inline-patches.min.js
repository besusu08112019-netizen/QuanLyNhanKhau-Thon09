...
var timeOptionalTypes = new Set(['summary','population','citizen','household','party_member','party','youth_union_member','meritorious_person','disabled_person','disability','age','gender','labor','elderly','children','poor-households','poor','near-poor-households','near_poor']);
...
function lockReportTypes(){var select=q('#reportTypeSelect');if(!select)return;var value=select.value||'summary';var options=[['summary','Báo cáo tổng hợp'],['population','Báo cáo nhân khẩu'],['household','Báo cáo hộ gia đình'],['temporary_residence','Báo cáo tạm trú'],['temporary_absence','Báo cáo tạm vắng'],['migration','Báo cáo biến động'],['party_member','Báo cáo Đảng viên'],['meritorious_person','Báo cáo người có công'],['disabled_person','Báo cáo người khuyết tật'],['age','Báo cáo theo độ tuổi'],['gender','Báo cáo theo giới tính'],['youth_union_member','Báo cáo Đoàn viên'],['poor-households','Báo cáo hộ nghèo'],['near-poor-households','Báo cáo hộ cận nghèo'],['labor','Báo cáo lao động'],['elderly','Báo cáo người cao tuổi'],['children','Báo cáo trẻ em']];var html=options.map(function(item){return '<option value="'+item[0]+'">'+item[1]+'</option>';}).join('');if(select.innerHTML.replace(/\s+/g,' ').trim()!==html.replace(/\s+/g,' ').trim())select.innerHTML=html;if(!Array.prototype.some.call(select.options,function(o){return o.value===value;}))value='summary';select.value=value;}
...
var labels={dashboard:'Dashboard',gis:'Bản đồ địa bàn',households:'Quản lý hộ gia đình',persons:'Quản lý nhân khẩu',reports:'Báo cáo thống kê',temporaryResidence:'Tạm trú',temporaryAbsence:'Tạm vắng',movements:'Biến động nhân khẩu',import:'Import dữ liệu',export:'Xuất Excel',exportExcel:'Xuất Excel',printForms:'In biểu mẫu',users:'Quản lý tài khoản',permissions:'Phân quyền',logs:'Nhật ký hệ thống',backups:'Sao lưu dữ liệu',restore:'Khôi phục dữ liệu',settings:'Cấu hình hệ thống',appearance:'Cấu hình giao diện'};
...
function repairScreenState(screen,requested){screen=normalizeScreen(screen);var target=document.getElementById(screen+'Screen')||document.getElementById('dashboardScreen');if(!target)return;document.querySelectorAll('.screen').forEach(function(el){el.classList.toggle('active',el===target);});document.querySelectorAll('.sidebar .nav-link').forEach(function(btn){btn.classList.toggle('active',btn.dataset.screen===screen||btn.dataset.screen===requested);});document.querySelectorAll('.mobile-bottom-nav [data-mobile-screen]').forEach(function(btn){var active=btn.dataset.mobileScreen===screen||btn.dataset.mobileScreen===requested;btn.classList.toggle('active',active);btn.setAttribute('aria-current',active?'page':'false');});var label=labels[screen]||labels[requested]||'Dashboard';var title=document.getElementById('screenTitle');if(title)title.textContent=label;var breadcrumb=document.getElementById('breadcrumbTrail');if(breadcrumb)breadcrumb.textContent='Trang chủ / '+label;try{localStorage.setItem('thon09_screen',screen);}catch(error){}}
...
document.addEventListener('click',function(event){var button=event.target.closest&&event.target.closest('.sidebar .nav-link[data-screen]');if(!button||button.classList.contains('gov-logout'))return;event.preventDefault();window.switchScreen(button.dataset.screen);},true);
}
