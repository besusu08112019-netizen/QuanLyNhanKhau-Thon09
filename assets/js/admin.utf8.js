(() => {
  ...
  function installPersonTableRenderFix() {
    ...
    window.personRow = function personRow(row = {}) {
      ...
      const residenceClass = row.presence_status === 'MOVED_OUT' || row.residency_status === 'TRANSFERRED_OUT' ? 'person-badge-muted' : (row.presence_status === 'AWAY' ? 'person-badge-away' : (row.residency_status === 'TEMPORARY' ? 'person-badge-temp' : 'person-badge-home'));
      const residenceText = row.presence_status === 'MOVED_OUT' || row.residency_status === 'TRANSFERRED_OUT' ? 'Đã chuyển đi' : (row.presence_status === 'AWAY' ? 'Tạm vắng' : (typeof residencyLabel === 'function' ? residencyLabel(row.residency_status) : (row.residency_status || 'Thường trú')));
      ...
      return '<tr>'
        ...
        + '<td><strong>' + safe(row.household_code || '') + '</strong>' + (row.head_citizen_name ? '<div class="small text-muted">Chủ hộ: ' + safe(row.head_citizen_name) + '</div>' : '') + '</td>'
