...
function repairScreenState(screen,requested){screen=normalizeScreen(screen);var target=document.getElementById(screen+'Screen')||document.getElementById('dashboardScreen');if(!target)return;document.querySelectorAll('.screen').forEach(function(el){el.classList.toggle('active',el===target);});document.querySelectorAll('.sidebar .nav-link').forEach(function(btn){btn.classList.toggle('active',btn.dataset.screen===screen||btn.dataset.screen===requested);btn.setAttribute('aria-current',(btn.dataset.screen===screen||btn.dataset.screen===requested)?'page':'false');});syncDashboardTree(screen,requested);document.querySelectorAll('.mobile-bottom-nav [data-mobile-screen]').forEach(function(btn){var active=btn.dataset.mobileScreen===screen||btn.dataset.mobileScreen===requested;btn.classList.toggle('active',active);btn.setAttribute('aria-current',active?'page':'false');});var label=labels[screen]||labels[requested]||'Dashboard';var title=document.getElementById('screenTitle');if(title)title.textContent=label;var breadcrumb=document.getElementById('breadcrumbTrail');if(breadcrumb)breadcrumb.textContent='Trang chủ / '+label;try{localStorage.setItem('thon09_screen',screen);}catch(error){}}
