(function () {
  ...
  function shell() {
    ...
    host.innerHTML = [
      '<section id="financeDashboard" class="agri-kpi-grid" aria-label="Thá»‘ng kÃª thu chi"></section>',
      '<section class="agri-filter-card" aria-label="Bá»™ lá»c thu chi"><div class="agri-filter-row">',
      '<div class="agri-field agri-search-field"><label for="financeSearch">TÃ¬m kiáº¿m</label><div class="module-search-input-wrap"><i class="fa-solid fa-magnifying-glass"></i><input id="financeSearch" class="form-control" placeholder="MÃ£ phiáº¿u, sá»‘ chá»©ng tá»«, ngÆ°á»i ná»™p/nháº­n, ná»™i dung..."></div></div>',
      '<div class="agri-field"><label for="financeTypeFilter">Loáº¡i</label><select id="financeTypeFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="financeFundFilter">Quá»¹</label><select id="financeFundFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="financeCategoryFilter">Danh má»¥c</label><select id="financeCategoryFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="financeStatusFilter">Tráº¡ng thÃ¡i</label><select id="financeStatusFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="financeDateFrom">Tá»« ngÃ y</label><input id="financeDateFrom" class="form-control" type="date"></div>',
      '<div class="agri-field"><label for="financeDateTo">Äáº¿n ngÃ y</label><input id="financeDateTo" class="form-control" type="date"></div>',
      '<div class="agri-field module-page-size-field"><label for="financePageSize">Hiá»ƒn thá»‹</label><select id="financePageSize" class="form-select"><option>20</option><option>50</option><option>100</option></select></div>',
      ...
      '</div></section>',
      '<section class="module-list-card household-list-card"><div class="module-list-head"><div><h3>Sá»• thu chi</h3><span id="financeTotalCount">Tá»•ng sá»‘: 0 phiáº¿u</span></div><div class="d-flex gap-2 flex-wrap"><button class="btn btn-success" type="button" data-platform-action="finance.create" data-type="INCOME"><i class="fa-solid fa-plus"></i> Phiáº¿u thu</button><button class="btn btn-primary" type="button" data-platform-action="finance.create" data-type="EXPENSE"><i class="fa-solid fa-minus"></i> Phiáº¿u chi</button></div></div><div id="financeLoadState" class="text-muted small px-3 pb-2 d-none"></div><div class="table-responsive"><table class="table module-table align-middle mb-0"><thead><tr><th data-platform-action="finance.sort" data-finance-sort="transaction_code">MÃ£ phiáº¿u</th><th data-platform-action="finance.sort" data-finance-sort="transaction_date">NgÃ y</th><th data-platform-action="finance.sort" data-finance-sort="transaction_type">Loáº¡i</th><th data-platform-action="finance.sort" data-finance-sort="fund">Quá»¹</th><th data-platform-action="finance.sort" data-finance-sort="category">Danh má»¥c</th><th data-platform-action="finance.sort" data-finance-sort="amount">Sá»‘ tiá»n</th><th>Chá»©ng tá»«</th><th data-platform-action="finance.sort" data-finance-sort="status">Tráº¡ng thÃ¡i</th><th class="text-end">Thao tÃ¡c</th></tr></thead><tbody id="financeRows"></tbody></table></div><div id="financePager" class="pager module-pager"></div></section>',
      formModal(),
      detailModal()
    ].join('');
    bind();
  ...
  async function catalogs() {
    ...
    fill($('#financeTypeFilter'), state.catalogs.types, 'Táº¥t cáº£');
    fill($('#financeTypeInput'), state.catalogs.types, '');
    fill($('#financeFundFilter'), state.catalogs.funds, 'Táº¥t cáº£');
    fill($('#financeFundInput'), state.catalogs.funds, 'Chá»n quá»¹');
    fill($('#financeStatusFilter'), state.catalogs.statuses, 'Táº¥t cáº£');
  ...
  function fillCategoryFilters() {
    fill($('#financeCategoryFilter'), categoriesFor(state.transaction_type), 'Táº¥t cáº£');
  }

  function fillCategoryInputs() {
    const current = $('#financeCategoryInput')?.value || '';
    fill($('#financeCategoryInput'), categoriesFor($('#financeTypeInput')?.value || ''), 'Chá»n danh má»¥c');
  ...
  function setLoading(on, message = '') {
    ...
    el.textContent = on ? (message || 'Äang táº£i dá»¯ liá»‡u...') : message;
  }
  async function load() {
    ...
    try {
      ...
      renderPager({});
      setLoading(false, error.message || 'KhÃ´ng táº£i Ä‘Æ°á»£c thu chi');
      toast(error.message || 'KhÃ´ng táº£i Ä‘Æ°á»£c thu chi', 'danger');
    }
  }

  function renderDashboard(data = {}) {
    const m = data.metrics || {};
    const cards = [['Tá»•ng thu', 'fa-arrow-trend-up', m.total_income || 0, 'success'], ['Tá»•ng chi', 'fa-arrow-trend-down', m.total_expense || 0, 'danger'], ['ChÃªnh lá»‡ch', 'fa-scale-balanced', m.balance || 0, Number(m.balance || 0) >= 0 ? 'primary' : 'warning'], ['Sá»‘ phiáº¿u', 'fa-receipt', m.total || 0, 'secondary']];
  ...
  function renderRows(data = {}) {
    ...
    $('#financeTotalCount').textContent = `Tá»•ng sá»‘: ${money(data.total || 0)} phiáº¿u`;
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted py-4">ChÆ°a cÃ³ phiáº¿u thu chi phÃ¹ há»£p bá»™ lá»c.</td></tr>'; return; }
  ...
  async function openForm(id = null, type = 'INCOME') {
    ...
    if (!form) return toast('KhÃ´ng má»Ÿ Ä‘Æ°á»£c form thu chi', 'danger');
  ...
  async function save(form) {
    ...
    closeModal('financeModal'); toast('ÄÃ£ lÆ°u phiáº¿u thu chi', 'success'); await load();
  }
  ...
  async function openDetail(id) {
    ...
    const files = (item.attachments || []).map(file => `<div class="work-task-file"><i class="fa-solid ${file.file_kind === 'PDF' ? 'fa-file-pdf' : file.file_kind === 'IMAGE' ? 'fa-file-image' : 'fa-file-lines'}"></i><a href="${safe(file.preview_url)}" target="_blank" rel="noopener">${safe(file.original_name)}</a><small>${safe(file.file_kind)}</small>${can('delete') ? `<button class="btn btn-sm btn-outline-danger" type="button" data-platform-action="finance.attachment.delete" data-id="${file.id}"><i class="fa-solid fa-trash"></i></button>` : ''}</div>`).join('') || '<div class="text-muted">ChÆ°a cÃ³ chá»©ng tá»« Ä‘Ã­nh kÃ¨m</div>';
    $('#financeDetailBody').innerHTML = `<div class="row g-3"><div class="col-md-7"><dl class="row mb-0"><dt class="col-sm-3">Loáº¡i</dt><dd class="col-sm-9">${safe(item.type_label)}</dd><dt class="col-sm-3">Sá»‘ tiá»n</dt><dd class="col-sm-9 fw-semibold">${money(item.amount)}</dd><dt class="col-sm-3">Quá»¹</dt><dd class="col-sm-9">${safe(text(item.fund_name))}</dd><dt class="col-sm-3">Danh má»¥c</dt><dd class="col-sm-9">${safe(text(item.category_name))}</dd><dt class="col-sm-3">NgÆ°á»i ná»™p</dt><dd class="col-sm-9">${safe(text(item.payer_name))}</dd><dt class="col-sm-3">NgÆ°á»i nháº­n</dt><dd class="col-sm-9">${safe(text(item.receiver_name))}</dd><dt class="col-sm-3">Sá»‘ chá»©ng tá»«</dt><dd class="col-sm-9">${safe(text(item.receipt_number))}</dd><dt class="col-sm-3">Ná»™i dung</dt><dd class="col-sm-9">${safe(text(item.description))}</dd></dl></div><div class="col-md-5"><h6>Chá»©ng tá»« Ä‘Ã­nh kÃ¨m</h6><div class="work-task-files">${files}</div></div></div>`;
    openModal('financeDetailModal');
  }

  async function deleteAttachment(fileId) { if (!state.current?.id || !fileId) return; if (!window.confirm('XÃ³a chá»©ng tá»« nÃ y?')) return; await request(API + '/' + state.current.id + '/attachments/' + fileId, { method: 'DELETE' }); await openDetail(state.current.id); }
  async function remove(id) { if (!can('delete')) return toast('KhÃ´ng cÃ³ quyá»n xÃ³a', 'warning'); if (!window.confirm('XÃ³a phiáº¿u thu chi nÃ y?')) return; await request(API + '/' + id, { method: 'DELETE' }); toast('ÄÃ£ xÃ³a phiáº¿u thu chi', 'success'); await load(); }
