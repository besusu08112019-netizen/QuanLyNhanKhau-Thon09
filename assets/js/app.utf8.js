...
function updateGisMarkers(items) {
  ...
  items.forEach(item => {
    ...
    App.gis.markerGroup.addLayer(marker);
  });
...
function updateGisGps(position) {
  ...
  if (!watch || !position?.coords || !App.gis.map) return;
...
function stopGisGps() {
  ...
  watch.marker?.remove(); watch.circle?.remove(); watch.panel?.remove(); App.gis.gpsWatch = null;
}
