  ...
  async function loadHouseholdsWithCategory() {
    try {
      ...
      tbody.innerHTML = items.map(row => '<tr>' +
        ...
        '<td class="text-end"><button class="btn btn-sm btn-outline-secondary" onclick="showHousehold(' + row.id + ')">Xem</button>' + (canUpdateHousehold ? ' <button class="btn btn-sm btn-outline-primary" onclick="openHouseholdForm(' + row.id + ')">Sửa</button>' : '') + (canDeleteHousehold ? ' <button class="btn btn-sm btn-outline-danger" onclick="deleteHousehold(' + row.id + ')">Xóa</button>' : '') + '</td>' +
