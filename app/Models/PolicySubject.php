...
final class PolicySubject extends BaseModel
{
    ...
    public const DEFAULT_TYPES = [
        ...
        'SOCIAL_ASSISTANCE' => 'Đối tượng bảo trợ xã hội',
        'DISABLED_PERSON' => 'Người khuyết tật',
    ];

    private const REQUIRED_SCHEMA = [
        'policy_subject_types' => ['id','village_id','code','name','description','display_order','is_active','created_at','updated_at','deleted_at','created_by','updated_by','deleted_by'],
        'citizen_policy_records' => ['id','village_id','citizen_id','policy_type_id','benefit_level','decision_number','decision_date','issuing_authority','benefit_start_date','benefit_end_date','status','note','created_at','updated_at','deleted_at','created_by','updated_by','deleted_by'],
        'policy_subject_attachments' => ['id','village_id','record_id','file_type','original_name','file_path','mime_type','file_size','uploaded_by','created_at','deleted_at','deleted_by'],
        'policy_subject_change_logs' => ['id','village_id','record_id','citizen_id','policy_type_id','action','before_json','after_json','actor_user_id','ip_address','user_agent','created_at'],
    ];
    ...
    private function assertSchemaReady(): void
    {
        foreach (self::REQUIRED_SCHEMA as $table => $columns) {
            if (!$this->tableExists($table)) {
                throw new RuntimeException('Policy Subject schema is not provisioned: missing table ' . $table);
            }
            foreach ($columns as $column) {
                if (!$this->columnExists($table, $column)) {
                    throw new RuntimeException('Policy Subject schema is not provisioned: missing column ' . $table . '.' . $column);
                }
            }
        }
    }
    ...
    public function saveType(array $data, int $userId, ?int $id = null): array
    {
        ...
        $params = [
            ...
            'name' => $this->required($data['name'] ?? null, 'Tên loại đối tượng'),
    ...
    public function deleteType(int $id, int $userId): void
    {
        ...
        if ($used > 0) throw new RuntimeException('Không thể xóa loại đối tượng đã có hồ sơ');
    ...
    {
        if (!$this->db->inTransaction()) $this->assertSchemaReady();
    ...
    public function updateRecord(int $id, array $data, int $userId, array $requestMeta = []): array
    {
        ...
        if (!$before) throw new RuntimeException('Không tìm thấy hồ sơ chính sách');
    ...
    public function report(array $filters): array
    {
        ...
        return [
            'title' => 'Báo cáo đối tượng chính sách',
            'headers' => ['STT','Mã nhân khẩu','Họ tên','Mã hộ','Khu','Giới tính','Tuổi','Loại đối tượng','Mức hưởng','Số quyết định','Ngày quyết định','Cơ quan ban hành','Bắt đầu hưởng','Kết thúc','Trạng thái','Ghi chú'],
            'rows' => $rows,
            'items' => $items,
    ...
    public function addAttachment(int $recordId, array $file, string $fileType, int $userId, array $requestMeta = []): array
    {
        ...
        if (!$record) throw new RuntimeException('Không tìm thấy hồ sơ chính sách');
    ...
    public function deleteAttachment(int $id, int $userId, array $requestMeta = []): void
    {
        ...
        if (!$before) throw new RuntimeException('Không tìm thấy file đính kèm');
    ...
    private function recordParams(array $data, int $userId, ?array $existing = null): array
    {
        ...
        return [
            ...
            'benefit_start_date' => $this->dateOrFail($data['benefit_start_date'] ?? $data['benefitStartDate'] ?? $existing['benefit_start_date'] ?? null, 'Ngày bắt đầu hưởng'),
    ...
    private function closeCurrentRecord(int $citizenId, int $typeId, string $newStartDate, int $userId, array $requestMeta): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($endDate < (string) $row['benefit_start_date']) {
                if ($newStartDate !== (string) $row['benefit_start_date']) throw new RuntimeException('Ngày bắt đầu mới phải sau ngày bắt đầu của hồ sơ hiện tại');
    ...
    private function validateCitizen(int $citizenId): void
    {
        ...
        if (!$row) throw new RuntimeException('Không tìm thấy nhân khẩu');
    }

    private function validateType(int $typeId): void
    {
        if ($typeId <= 0 || !$this->findType($typeId)) throw new RuntimeException('Loại đối tượng không hợp lệ');
    }

    private function storeUpload(array $file): array
    {
        if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK || empty($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) throw new RuntimeException('File upload không hợp lệ');
        if ((int) ($file['size'] ?? 0) <= 0 || (int) ($file['size'] ?? 0) > 20 * 1024 * 1024) throw new RuntimeException('File tối đa 20MB');
        ...
        if (!isset($allowed[$extension]) || !in_array($mime, $allowed[$extension], true)) throw new RuntimeException('Định dạng file chưa được hỗ trợ');
        $dir = dirname(__DIR__, 2) . '/uploads/policy-subjects/' . date('Y/m');
        if (!is_dir($dir) && !mkdir($dir, 0755, true) && !is_dir($dir)) throw new RuntimeException('Không tạo được thư mục upload');
        $stored = bin2hex(random_bytes(16)) . '.' . $extension;
        $path = $dir . '/' . $stored;
        if (!move_uploaded_file($file['tmp_name'], $path)) throw new RuntimeException('Không lưu được file upload');
    ...
    private function code(mixed $value): string
    {
        ...
        if ($code === '') throw new RuntimeException('Mã loại đối tượng là bắt buộc');
