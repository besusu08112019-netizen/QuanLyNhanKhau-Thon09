...
const useCluster = Boolean(window.L && typeof L.markerClusterGroup === 'function');
const group = useCluster ? L.markerClusterGroup(gisMarkerClusterOptions(map)) : L.layerGroup();
const flatGroup = L.layerGroup();
const markersById = new Map();
let coordinateBuckets = new Map();
let markerLoadCycles = 0;
let mapZoomListenerBound = false;
let clusterListenerCount = 0;
let visibleMode = '';
let layerVisible = true;
function rebuildCoordinateBuckets() {
coordinateBuckets = new Map();
markersById.forEach((marker, id) => {
const key = gisMarkerCoordinateKey(marker);
if (!key) return;
if (!coordinateBuckets.has(key)) coordinateBuckets.set(key, []);
coordinateBuckets.get(key).push(id);
});
}
function refresh() {
gisApplyDuplicateMarkerOffsets(Array.from(markersById.values()));
rebuildCoordinateBuckets();
if (gisHasOpenPopup()) return;
renderVisibleMarkers();
if (visibleMode === 'cluster' && group && typeof group.refreshClusters === 'function') group.refreshClusters();
}
function shouldUseFlatMarkers() {
const zoom = Number(map && typeof map.getZoom === 'function' ? map.getZoom() : 0);
return useCluster && zoom >= Math.max(0, gisMapMaxZoom(map) - 1);
}
...
const list = Array.from(markersById.values());
gisApplyDuplicateMarkerOffsets(list);
rebuildCoordinateBuckets();
if (!layerVisible) {
if (map?.hasLayer?.(group) && typeof map.removeLayer === 'function') map.removeLayer(group);
if (map?.hasLayer?.(flatGroup) && typeof map.removeLayer === 'function') map.removeLayer(flatGroup);
if (typeof group.clearLayers === 'function') group.clearLayers();
if (typeof flatGroup.clearLayers === 'function') flatGroup.clearLayers();
visibleMode = 'hidden';
return;
}
const useFlat = shouldUseFlatMarkers();
if (useFlat) {
if (map?.hasLayer?.(group) && typeof map.removeLayer === 'function') map.removeLayer(group);
if (typeof group.clearLayers === 'function') group.clearLayers();
addMarkersToLayer(flatGroup, list);
if (flatGroup && typeof flatGroup.addTo === 'function' && map && !map.hasLayer?.(flatGroup)) flatGroup.addTo(map);
visibleMode = 'flat';
return;
}
if (map?.hasLayer?.(flatGroup) && typeof map.removeLayer === 'function') map.removeLayer(flatGroup);
if (typeof flatGroup.clearLayers === 'function') flatGroup.clearLayers();
addMarkersToLayer(group, list);
if (group && typeof group.addTo === 'function' && map && !map.hasLayer?.(group)) group.addTo(map);
visibleMode = useCluster ? 'cluster' : 'flat';
if (visibleMode === 'cluster' && group && typeof group.refreshClusters === 'function') group.refreshClusters();
}
function bindClusterClick() {
if (!useCluster || !group || group.__thon09GisClusterClickBound || typeof group.on !== 'function') return;
group.__thon09GisClusterClickBound = true;
...
clusterListenerCount += 1;
}
function bindListeners() {
if (map && !mapZoomListenerBound && typeof map.on === 'function') {
...
return {
group,
getLayer() { return group; },
getFlatLayer() { return flatGroup; },
attach() { bindListeners(); renderVisibleMarkers(); return group; },
setVisible(visible) { layerVisible = visible !== false; renderVisibleMarkers(); },
replace(markers) {
...
if (id) markersById.set(id, marker);
});
gisDebugTrace('cluster.manager.replace()', { nextMarkerCount: list.length, duplicateCoordinateBuckets: this.debugState().duplicateCoordinateBuckets });
gisApplyDuplicateMarkerOffsets(list);
rebuildCoordinateBuckets();
renderVisibleMarkers();
},
...
debugState() {
rebuildCoordinateBuckets();
const duplicateBuckets = Array.from(coordinateBuckets.values()).filter(ids => ids.length > 1);
return { useCluster, visibleMode, layerVisible, markerCount: markersById.size, flatLayerCount: typeof flatGroup.getLayers === 'function' ? flatGroup.getLayers().length : 0, clusterLayerCount: typeof group.getLayers === 'function' ? group.getLayers().length : 0, coordinateBucketCount: coordinateBuckets.size, duplicateCoordinateBuckets: duplicateBuckets.length, listenerCount: clusterListenerCount, markerLoadCycles, options: group?.options || {} };
}
};
}
...
markers: {
loadViewport: loadGisAllMarkers,
...
draw: startGisDraw,
save: saveGisArea,
cancel: cancelGisDraw,
redo: redoGisDraw,
undoPoint: undoGisDrawPoint,
edit: editGisArea,
delete: deleteGisArea
}
};
}
function initGisMap() {
...
gisWatchMapDom();
if (App.gis.map) { setTimeout(() => { gisDebugTrace('map.invalidateSize() existing map'); App.gis.map.invalidateSize(); }, 80); return; }
...
map.on('zoomend', () => gisRefreshMarkerClusters());
map.on('popupopen', () => { console.log('popupopen'); gisDebugTrace('popupopen'); });
map.on('popupclose', () => { console.trace('popupclose'); });
map.on('movestart', () => { console.trace('movestart'); });
map.on('moveend', () => { console.trace('moveend'); });
map.on('zoomstart', () => { console.trace('zoomstart'); });
map.on('zoomend', () => { console.trace('zoomend'); });
...
setTimeout(() => { gisDebugTrace('map.invalidateSize() initial map'); map.invalidateSize(); }, 200);
}
...
setTimeout(() => { gisDebugTrace('map.invalidateSize() after marker fit'); map.invalidateSize(); }, 80);
return;
}
...
setTimeout(() => updateGisAreaActiveState(Boolean(options.scrollList)), 80);
}
...
console.log('GIS draw created GeoJSON:', layer.toGeoJSON?.());
if (layer.editing?.enable) layer.editing.enable();
attachGisAreaEditEvents(layer);
...
gisDebugTrace('renderMap/renderGisAreas()', { fit, areaCount: (data.areas || []).length });
const map = App.gis.map;
const group = App.gis.layerGroup;
if (!map || !group) return;
gisDebugTrace('areaLayer.clearLayers()', { areaCount: (data.areas || []).length });
...
if (areaKey) App.gis.areaLayerMap.set(areaKey, polygon);
polygon.bindPopup(gisAreaPopup(area));
if ((data.areas || []).length <= 30) polygon.bindTooltip(gisTooltip(area), { permanent: true, direction: 'center', className: gisAreaTooltipClass(areaKey === String(App.gis.selectedAreaId || '')) });
polygon.on('click', () => selectGisArea(area, polygon, { scrollList: true }));
polygon.addTo(group);
latLngs.forEach(p => bounds.push(p));
});
if (draftLayer && !group.hasLayer(draftLayer)) { group.addLayer(draftLayer); styleGisDraftLayer(draftLayer); }
if (fit && bounds.length) { gisDebugTrace('map.fitBounds() renderGisAreas', { boundsCount: bounds.length }); map.fitBounds(bounds, { padding: [24, 24] }); }
renderGisAreaList(data.areas || [], $('#gisSearch')?.value || '');
setTimeout(() => { gisDebugTrace('map.invalidateSize() renderGisAreas'); map.invalidateSize(); }, 120);
updateGisAreaActiveState(false);
}
...
const latLngs = gisPolygonLatLngs(area);
if (latLngs.length) { gisDebugTrace('map.fitBounds() focusGisArea', { areaKey }); App.gis.map.fitBounds(latLngs, { padding: [28, 28], maxZoom: 17 }); updateGisAreaActiveState(true); setTimeout(() => updateGisAreaActiveState(true), 80); setTimeout(() => updateGisAreaActiveState(true), 220); return; }
const center = gisAreaCenter(area);
if (center) { App.gis.map.setView(center, Math.max(App.gis.map.getZoom(), 17)); updateGisAreaActiveState(true); setTimeout(() => updateGisAreaActiveState(true), 80); setTimeout(() => updateGisAreaActiveState(true), 220); return; }
