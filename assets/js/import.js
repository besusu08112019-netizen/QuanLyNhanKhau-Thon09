(() => {
  ...
  function injectImportScreen() {
    ...
    main.insertAdjacentHTML('beforeend', `<section id="importScreen" class="screen"><form id="importForm" class="content-card mb-3"><div class="row g-3 align-items-end"><div class="col-md-3"><label class="form-label">Loại dữ liệu</label><select name="type" class="form-select"><option value="household">Hộ dân</option><option value="person">Nhân khẩu</option></select></div><div class="col-md-3"><label class="form-label">Khi trùng mã hộ</label><select name="mode" class="form-select"><option value="skip">Bỏ qua</option><option value="update">Cập nhật</option></select></div><div class="col-md-4"><label class="form-label">File dữ liệu</label><input name="file" type="file" class="form-control" accept=".csv,.xlsx" required></div><div class="col-md-2 d-grid gap-2"><button id="importPreviewBtn" class="btn btn-outline-primary" type="button">Kiểm tra</button><button id="importRunBtn" class="btn btn-primary" type="button">Import</button></div><div class="col-12 d-flex flex-wrap gap-2"><a class="btn btn-sm btn-light" href="sample-data/import_household_template.csv" download>Tải mẫu hộ dân</a><a class="btn btn-sm btn-light" href="sample-data/import_person_template.csv" download>Tải mẫu nhân khẩu</a></div></div></form><div class="row g-3 mb-3"><div class="col-md-4"><div class="metric-card"><div class="metric-label">Tổng số dòng</div><div id="importTotal" class="metric-value">0</div></div></div><div class="col-md-4"><div class="metric-card"><div class="metric-label">Hợp lệ</div><div id="importValid" class="metric-value">0</div></div></div><div class="col-md-4"><div class="metric-card"><div class="metric-label">Lỗi</div><div id="importFailed" class="metric-value">0</div></div></div></div><div class="content-card"><div class="d-flex justify-content-between align-items-center mb-3"><h3 class="section-title mb-0">Kết quả import</h3><span id="importStatus" class="text-muted small"></span></div><div id="importResult" class="table-responsive"><p class="text-muted mb-0">Chọn file CSV/XLSX và bấm Kiểm tra để xem trước dữ liệu.</p></div></div></section>`);
  ...
  function renderImportResult(data, processed) {
    ...
    const message = processed ? `Thành công ${number(data.success || 0)}, bỏ qua ${number(data.skipped || 0)}, lỗi ${number(data.failed || 0)}` : `Hợp lệ ${number(data.valid || 0)}, lỗi ${number(data.failed || 0)}`;
    ...
    document.querySelector('#importResult').innerHTML = `<table class="table table-sm table-bordered align-middle mb-0"><thead><tr><th style="width:120px">Dòng</th><th>Nội dung lỗi</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
