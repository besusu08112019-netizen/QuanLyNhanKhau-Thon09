...
const options = Object.assign({}, source?.options || source?.opts || {});
...
syncClusterLayer() {
this.markersById.forEach(marker => this.ensureLayerHas(this.group, marker));
}
syncFlatLayer() {
...
if (id) this.markersById.set(id, marker);
});
...
const marker = L.marker([Number(item.latitude), Number(item.longitude)], { title: item.household_code || item.head_citizen_name || 'H\u1ed9 gia \u0111\u00ecnh', pane: 'gisMarkerPane', bubblingMouseEvents: false });
