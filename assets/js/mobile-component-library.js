(function (window, document) {
  ...
  var scheduled = false;
  var gisDataSyncScheduled = false;
  var moduleShellEnabled = true;
  var filterStates = Object.create(null);
  var adapterReadyPromises = Object.create(null);
  ...
  function applyFilterFieldValues(root, state) {
    Array.from(root.querySelectorAll('[data-app-v2-filter-field]')).forEach(function (control) {
      var key = control.getAttribute('data-app-v2-filter-field') || control.name || '';
      if (!key) return;
      control.value = state && state[key] != null ? state[key] : '';
    });
  }
  ...
  function dashboardData() {
    ...
    return {
      ...
      stats: [
        ...
        { label: 'Tạm trú', value: number(temporary), icon: 'fa-location-dot' },
        { label: 'Tạm vắng', value: number(away), icon: 'fa-person-walking-arrow-right' },
  ...
  var MODULE_DASHBOARD_META = {
    ...
    dashboardPopulation: {
      ...
      kpis: [
        ...
        { key: 'maleCitizens', label: 'Nam', unit: 'người', icon: 'fa-mars', meta: 'Nhân khẩu nam' },
        { key: 'femaleCitizens', label: 'Nữ', unit: 'người', icon: 'fa-venus', meta: 'Nhân khẩu nữ' },
        { key: 'children', label: 'Trẻ em', unit: 'người', icon: 'fa-child-reaching', meta: 'Dưới 16 tuổi' },
        { key: 'elderly', label: 'Người cao tuổi', unit: 'người', icon: 'fa-person-cane', meta: 'Từ 60 tuổi' },
        { key: 'temporaryResidents', label: 'Tạm trú', unit: 'người', icon: 'fa-location-dot' },
        { key: 'temporaryAbsences', label: 'Tạm vắng', unit: 'người', icon: 'fa-person-walking-arrow-right' },
  ...
  var MODULE_SCREEN_META = {
    householdsScreen: {
      ...
      primaryAction: { label: 'Thêm hộ', icon: 'fa-plus', proxy: '#householdAddBtn, [data-platform-action="households.create"]' },
    ...
    personsScreen: {
      ...
      desktopFilter: { searchSelector: '#personSearch', statusSelector: '#personLifeStatusFilter', stateKey: 'persons', loaderName: 'loadPersons' },
    ...
    partyMembersScreen: {
      ...
      search: 'Tìm họ tên, số thẻ đảng viên, chi bộ...',
      titleLabels: ['Họ tên', 'Họ và tên', 'Ho ten', 'Ho va ten'],
      summaryLabels: ['Số thẻ đảng viên', 'Chi bộ', 'Chức vụ', 'Loại', 'Tình trạng', 'Ngày vào Đảng', 'So the dang vien', 'Chi bo', 'Chuc vu', 'Loai', 'Tinh trang', 'Ngay vao Dang'],
      metaLabels: ['Số thẻ đảng viên', 'Chi bộ', 'Chức vụ', 'Tình trạng', 'So the dang vien', 'Chi bo', 'Chuc vu', 'Tinh trang'],
    ...
    temporaryResidenceScreen: {
      ...
      primaryAction: { label: 'Nhân khẩu', icon: 'fa-id-card', action: 'persons' },
      nav: [{ label: 'Nhân khẩu', icon: 'fa-users', action: 'persons' }, { label: 'Tạm vắng', icon: 'fa-person-walking-arrow-right', action: 'temporaryAbsence' }]
    },
    temporaryAbsenceScreen: {
      ...
      primaryAction: { label: 'Nhân khẩu', icon: 'fa-id-card', action: 'persons' },
      nav: [{ label: 'Nhân khẩu', icon: 'fa-users', action: 'persons' }, { label: 'Tạm trú', icon: 'fa-location-dot', action: 'temporaryResidence' }]
    },
    movementsScreen: {
      title: 'Bien dong',
      overviewMode: 'compact',
      ...
      primaryAction: { label: 'Thêm biến động', icon: 'fa-plus', proxy: '#movementAddBtn, [data-platform-action="admin.movement.add"]' },
      nav: [{ label: 'Nhân khẩu', icon: 'fa-id-card', action: 'persons' }, { label: 'Báo cáo', icon: 'fa-chart-pie', action: 'reports' }]
    },
    importScreen: {
      title: 'Import',
      eyebrow: 'Dữ liệu',
      ...
      nav: [{ label: 'Xuất Excel', icon: 'fa-file-export', action: 'exportExcel' }, { label: 'Báo cáo', icon: 'fa-chart-pie', action: 'reports' }]
    },
    exportExcelScreen: {
      title: 'Xuất Excel',
      eyebrow: 'Dữ liệu',
      ...
      primaryAction: { label: 'Báo cáo tổng hợp', icon: 'fa-file-excel', proxy: '[data-platform-action="admin.report.export"]' },
    ...
    printFormsScreen: {
      ...
      nav: [{ label: 'Xuất Excel', icon: 'fa-file-excel', action: 'exportExcel' }, { label: 'Báo cáo', icon: 'fa-chart-pie', action: 'reports' }]
    },
    systemAdminScreen: {
      ...
      primaryAction: { label: 'Làm mới', icon: 'fa-rotate-right', proxy: '[data-system-refresh], [data-platform-action="systemAdmin.refresh"]' },
      nav: [{ label: 'Tài khoản', icon: 'fa-user-shield', action: 'users' }, { label: 'Sao luu', icon: 'fa-database', action: 'backups' }]
    },
    usersScreen: {
      title: 'Tài khoản',
      eyebrow: 'Quan tri',
      icon: 'fa-user-shield',
      subtitle: 'Quản lý người dùng, vai trò và trạng thái tài khoản',
      search: 'Tim email, ten hien thi, vai tro...',
      primaryAction: { label: 'Thêm tài khoản', icon: 'fa-user-plus', proxy: '#userAddBtn, [data-platform-action="users.create"]' },
    ...
    permissionsScreen: {
      ...
      nav: [{ label: 'Tài khoản', icon: 'fa-user-shield', action: 'users' }, { label: 'Cấu hình', icon: 'fa-gear', action: 'settings' }]
    },
    logsScreen: {
      title: 'Nhat ky',
      eyebrow: 'Quan tri',
      ...
      primaryAction: { label: 'Làm mới', icon: 'fa-rotate-right', action: 'logs' },
    ...
    settingsScreen: {
      ...
      eyebrow: 'Quan tri',
      icon: 'fa-gear',
      subtitle: 'Cấu hình thong tin he thong, don vi va bao cao',
    ...
    appearanceScreen: {
      ...
      eyebrow: 'Quan tri',
      icon: 'fa-palette',
      subtitle: 'Quản lý logo, ảnh nền và nội dung hiển thị',
  ...
  var MODULE_LIST_META = {
    ...
    temporaryAbsenceScreen: { label: 'Tạm vắng', unit: 'nhân khẩu' },
    ...
    livestockScreen: { label: 'Danh sách cơ sở chăn nuôi', unit: 'cơ sở', totalSelector: '#livestockTotalCount' },
  ...
  function readKpis(screen, moduleMeta) {
    ...
    var allCards = source ? Array.from(source.children) : [];
    var useAllKpis = !!(moduleMeta && moduleMeta.useAllKpis);
    var limit = useAllKpis ? allCards.length : Number(moduleMeta && moduleMeta.kpiLimit || 4);
    if (!Number.isFinite(limit) || limit < 1) limit = 4;
    var cards = useAllKpis ? allCards : allCards.slice(0, limit);
    ...
    var kpis = cards.map(function (card, index) {
      ...
      return {
        label: label || kpiMeta.label || 'T?ng quan ' + (index + 1),
        value: withKpiUnit(value || '0', note || kpiMeta.unit),
        meta: kpiMeta.meta || note || 'Theo d? li?u ' + ((screen && screen.getAttribute('data-module-dashboard')) || 'dashboard'),
        icon: firstFaIcon(card, kpiMeta.icon || (moduleMeta && moduleMeta.icon)),
        platformAction: card.getAttribute('data-kpi-action') || card.getAttribute('data-platform-action') || '',
        type: card.getAttribute('data-kpi-alert-type') || card.getAttribute('data-type') || '',
        status: card.getAttribute('data-kpi-alert-status') || card.getAttribute('data-status') || ''
      };
    });
    if (screen && screen.id === 'dashboardGisScreen') kpis = mergeGisDashboardKpis(kpis, moduleMeta);
    return kpis.slice(0, limit);
  }

  function gisMarkerRecords() {
    var cache = window.App && window.App.gis && window.App.gis.markerCache;
    if (!cache || typeof cache.forEach !== 'function') return [];
    var records = [];
    cache.forEach(function (entry, id) {
      var row = (entry && entry.data) || (entry && entry.row) || {};
      var lat = Number(row.latitude);
      var lng = Number(row.longitude);
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) return;
      var code = cleanLabel(row.household_code || row.householdCode || ('GIS #' + id));
      var head = cleanLabel(row.head_citizen_name || row.headCitizenName || '');
      var area = cleanLabel(row.area_code || row.areaCode || '');
      var accuracy = row.location_accuracy == null || row.location_accuracy === '' ? '' : cleanLabel(row.location_accuracy) + 'm';
      records.push({
        title: code,
        meta: head || 'Hộ đã định vị GIS',
        summaryFields: [
          { label: 'Chủ hộ', value: head },
          { label: 'Khu vực', value: area },
          { label: 'Tọa độ', value: lat.toFixed(8) + ', ' + lng.toFixed(8) },
          { label: 'Độ chính xác', value: accuracy }
        ].filter(function (field) { return field.value; }),
        searchFields: [
          { label: 'Mã hộ', value: code },
          { label: 'Chủ hộ', value: head },
          { label: 'Khu vực', value: area }
        ].filter(function (field) { return field.value; }),
        searchText: [code, head, area, lat, lng].join(' '),
        icon: 'fa-location-dot',
        action: 'gis',
        badges: [{ label: 'Đã định vị', tone: 'success' }],
        details: [
          { label: 'Latitude', value: lat.toFixed(8) },
          { label: 'Longitude', value: lng.toFixed(8) }
        ],
        primaryProxy: ''
      });
    });
  ...
  function gisSummaryValue(labelPattern) {
    var cards = Array.from(document.querySelectorAll('#gisSummaryCards > *'));
    for (var i = 0; i < cards.length; i += 1) {
      var label = normalizeSearchText(text(cards[i].querySelector('span')));
      if (labelPattern.test(label)) {
        var value = parseCountText(text(cards[i].querySelector('b, strong')));
  ...
  function mergeGisDashboardKpis(kpis, moduleMeta) {
    var markerCount = gisMarkerRecords().length;
    if (!markerCount) return kpis;
    var totalHouseholds = gisSummaryValue(/tong ho/) || markerCount;
    var located = gisSummaryValue(/da dinh vi/) || markerCount;
    var unlocated = gisSummaryValue(/chua dinh vi/);
    var coverage = totalHouseholds > 0 ? Math.round((located * 1000) / totalHouseholds) / 10 : 0;
    var fallback = [
      { label: 'Hộ đã định vị', value: number(located) + ' hộ', meta: 'Có tọa độ bản đồ', icon: 'fa-location-dot' },
      { label: 'Chưa định vị', value: number(unlocated) + ' hộ', meta: 'Cần bổ sung tọa độ', icon: 'fa-map-pin' },
      { label: 'Marker bản đồ', value: number(markerCount) + ' marker', meta: 'Điểm đang hiển thị', icon: 'fa-location-crosshairs' },
      { label: 'Tỷ lệ phủ GIS', value: number(coverage) + '%', meta: 'Mức hoàn thiện định vị', icon: 'fa-chart-pie' }
    ];
    if (!kpis.length || !kpis.some(function (item) { return parseCountText(item.value) > 0; })) return fallback;
    var normalized = kpis.slice();
    fallback.forEach(function (item, index) {
      if (!normalized[index] || !parseCountText(normalized[index].value)) normalized[index] = item;
    });
    return normalized.length ? normalized : ((moduleMeta && moduleMeta.kpis) || fallback).map(function (item, index) { return fallback[index] || item; });
  }
  ...
  function renderModuleDashboard(screen) {
    ...
    applyFilterFieldValues(filterSheet, filterState);
  ...
  function moduleKeyFromScreen(screen) {
    return screen && screen.id ? screen.id.replace(/Screen$/, '') : '';
  }

  function mobileFilterAdapter(screen, meta) {
    var registry = window.TenantAppMobileFilterAdapters || window.TenantAppModuleFilterAdapters || {};
    var keys = [
      screen && screen.id,
      meta && meta.filterAdapter,
      moduleKeyFromScreen(screen)
    ].filter(Boolean);
    for (var i = 0; i < keys.length; i += 1) {
      if (registry[keys[i]]) return registry[keys[i]];
    }
    return null;
  }

  function adapterDefinition(adapter) {
    if (!adapter || typeof adapter.getDefinition !== 'function') return null;
    var definition = adapter.getDefinition() || {};
    if (Array.isArray(definition)) definition = { fields: definition };
    definition.fields = Array.isArray(definition.fields) ? definition.fields : [];
    return definition;
  }

  function adapterFilterState(adapter) {
    if (!adapter || typeof adapter.getState !== 'function') return {};
    return adapter.getState() || {};
  }

  function ensureAdapterReady(screen, adapter) {
    if (!adapter || typeof adapter.ensureReady !== 'function') return;
    if (typeof adapter.isReady === 'function' && adapter.isReady()) return;
    var key = (screen && screen.id) || 'module';
    if (adapterReadyPromises[key]) return;
    var result = adapter.ensureReady();
    if (!result || typeof result.then !== 'function') return;
    adapterReadyPromises[key] = result.catch(function () {
      return null;
    }).finally(function () {
      delete adapterReadyPromises[key];
      schedule();
    });
  }

  function applyAdapterFilter(adapter, state, host, done) {
    if (!adapter) return false;
    if (host) host.setAttribute('data-app-v2-adapter-filtering', 'true');
    var result;
    if (typeof adapter.applyFilters === 'function') {
      result = adapter.applyFilters(state || {});
    } else {
      Object.keys(state || {}).forEach(function (key) {
        if (typeof adapter.setFilter === 'function') adapter.setFilter(key, state[key]);
      });
      result = typeof adapter.reload === 'function' ? adapter.reload() : null;
    }
    Promise.resolve(result).finally(function () {
      if (host) host.removeAttribute('data-app-v2-adapter-filtering');
  ...
  function adapterGoToPage(adapter, page, host) {
    if (!adapter || typeof adapter.setPage !== 'function') return false;
    if (host) host.setAttribute('data-app-v2-list-loading', 'true');
    Promise.resolve(adapter.setPage(page)).finally(function () {
  ...
  function countRecords(screen) {
    if (screen && screen.id === 'gisScreen') return gisMarkerRecords().length;
  ...
  function clickPagerButton(pager, page, direction) {
    ...
    var pattern = direction < 0 ? /(truoc|trước|prev|previous)/i : /(sau|next)/i;
  ...
  function splitCellFields(cell, label, value) {
    ...
    addressParts.forEach(function (part) {
      fields.push({ label: 'Địa chỉ', value: part });
    });
    return fields;
  }
  ...
  function sourceRecords(screen, meta) {
    if (screen && screen.id === 'gisScreen') return gisMarkerRecords();
    var rows = sourceRows(screen);
    if (rows.length) {
  ...
  function buildModuleOverview(screen, meta, listMeta, total) {
    if (screen && screen.id === 'livestockScreen') return buildLivestockOverview(screen, meta);
  ...
  function livestockDashboardMetric(filter, fallback) {
    var cards = Array.from(document.querySelectorAll('#livestockMiniDashboard [data-filter]'));
    var card = cards.find(function (item) {
      return item.getAttribute('data-filter') === filter;
    });
    var value = parseCountText(card && text(card.querySelector('strong')));
    var unit = cleanLabel(card && text(card.querySelector('em')));
    return {
  ...
  function buildLivestockOverview(screen, meta) {
    var section = AppSection({ title: 'Tóm tắt', meta: 'Theo KPI desktop' });
    var grid = el('div', 'app-v2-grid app-v2-module-overview-grid');
    [
      livestockDashboardMetric('all', { label: 'Tổng vật nuôi', unit: 'con', icon: 'fa-paw', note: 'SUM(quantity) bản ghi hợp lệ' }),
      livestockDashboardMetric('facilities', { label: 'Cơ sở chăn nuôi', unit: 'cơ sở', icon: 'fa-warehouse', note: 'COUNT(DISTINCT facility)' }),
      livestockDashboardMetric('households', { label: 'Hộ chăn nuôi', unit: 'hộ', icon: 'fa-house-chimney', note: 'COUNT(DISTINCT household)' }),
      livestockDashboardMetric('pig', { label: 'Tổng đàn lợn', unit: 'con', icon: 'fa-bacon', note: 'SUM(quantity) nhóm lợn' })
    ].forEach(function (item) {
      grid.appendChild(AppSummaryCard({
        label: item.label,
        value: number(item.value) + (item.unit ? ' ' + item.unit : ''),
        note: item.note,
        icon: item.icon || (meta && meta.icon)
      }));
    });
    append(section, [grid]);
    return section;
  ...
  function renderModuleScreen(screen) {
    ...
    var adapter = mobileFilterAdapter(screen, meta);
    ensureAdapterReady(screen, adapter);
    var definition = adapterDefinition(adapter);
    var adapterBacked = !!(adapter && definition && definition.fields.length);
    var filterState = adapterBacked ? adapterFilterState(adapter) : (filterStates[hostId] || desktopFilterState(meta));
    ...
    var activeClientState = adapterBacked ? {} : (meta.desktopFilter ? { status: filterState.status && !meta.desktopFilter.statusSelector ? filterState.status : '' } : filterState);
    var filteredRecords = renderFilteredRecords(recordList, records, activeClientState);
    ...
    paginationHost.addEventListener('click', function (event) {
      ...
      if (adapterGoToPage(adapter, Number(button.getAttribute('data-app-v2-page')), host)) return;
    ...
    var filterSheet = AppFilterSheet({
      ...
      fields: adapterBacked ? definition.fields : [
    ...
    filterSheet.addEventListener('app-v2-filter-change', function (event) {
      filterStates[hostId] = event.detail.state || {};
      if (applyAdapterFilter(adapter, filterStates[hostId], host, function () {
  ...
  function isMobileOrTablet() {
    var layout = window.TenantAppPlatform && window.TenantAppPlatform.layout;
    var device = layout && typeof layout.device === 'function' ? layout.device() : (window.TenantRuntime && window.TenantRuntime.device);
    return !!(device && (device.isMobile || device.isTablet));
  }

  function removeMobileShell() {
    document.documentElement.classList.remove('app-v2-adaptive');
    var dashboard = document.getElementById('appMobileDashboard');
    if (dashboard) dashboard.remove();
    document.querySelectorAll('.app-v2-module-dashboard, .app-v2-module-screen, [data-app-v2-global-fab]').forEach(function (node) {
      node.remove();
    });
  }
  ...
  function enhance() {
    ...
    document.documentElement.classList.add('app-v2-adaptive');
    renderDashboard();
  ...
  function isGisDataUrl(url) {
    var value = String(url || '');
    return /\/api\/gis\/households(?:[?#]|$)/.test(value) || /\/api\/dashboard\/gis(?:[?#]|$)/.test(value);
  }

  function renderGisAdaptiveViews() {
    var active = activeScreen();
    if (active && active.id === 'gisScreen') renderModuleScreen(active);
    if (active && active.id === 'dashboardGisScreen') renderModuleDashboard(active);
  }

  function requestGisDataSync(url) {
    if (gisDataSyncScheduled) return;
    gisDataSyncScheduled = true;
    window.setTimeout(function () {
      gisDataSyncScheduled = false;
      document.dispatchEvent(new CustomEvent('tenant:gis-data-updated', { detail: { url: String(url || '') } }));
    }, 0);
  }

  function syncAfterGisApiRequests() {
    if (typeof window.api !== 'function' || window.api.__appV2GisSynced) return;
    var baseApi = window.api;
    var syncedApi = function (url) {
      var result = baseApi.apply(this, arguments);
      if (!isGisDataUrl(url) || !result || typeof result.then !== 'function') return result;
      return result.then(function (data) {
        requestGisDataSync(url);
        return data;
      });
    };
    syncedApi.__appV2GisSynced = true;
    window.api = syncedApi;
  }
  ...
  function dispatchProxy(selector) {
    ...
    if (proxyTarget && typeof proxyTarget.dispatchEvent === 'function') {
      var actionKey = proxyTarget.getAttribute('data-platform-action') || '';
      var actions = window.TenantAppPlatform && window.TenantAppPlatform.actions;
      if (actionKey && actions && typeof actions.dispatch === 'function') {
        actions.dispatch(actionKey, {
          target: proxyTarget,
          key: actionKey,
          dataset: Object.assign({}, proxyTarget.dataset || {})
        });
        return true;
      }
  ...
  document.addEventListener('tenant:gis-data-updated', renderGisAdaptiveViews);
