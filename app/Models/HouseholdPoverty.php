...
final class HouseholdPoverty extends BaseModel
{
    ...
    public const POVERTY_TYPES = [
        'NONE' => 'KhÃ´ng thuá»™c diá»‡n',
        'NEAR_POOR' => 'Há»™ cáº­n nghÃ¨o',
        'MEDIUM' => 'Há»™ trung bÃ¬nh',
        'POOR' => 'Há»™ nghÃ¨o',
    ];
...
SQL);
    ...
    {
        $this->ensureSchema();
        if ($id && !$this->findPeriod($id)) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y giai Ä‘oáº¡n');
        ...
        if ($duplicate) {
            ...
            throw new RuntimeException('TÃªn giai Ä‘oáº¡n Ä‘Ã£ tá»“n táº¡i');
        }
    ...
    public function deletePeriod(int $id, int $userId): void
    {
        ...
        if (!$period) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y giai Ä‘oáº¡n');
        ...
        if ($activeRecords > 0) throw new RuntimeException('KhÃ´ng thá»ƒ xÃ³a giai Ä‘oáº¡n Ä‘Ã£ cÃ³ lá»‹ch sá»­ há»™');
    ...
    public function updateRecord(int $id, array $data, int $userId, array $requestMeta = []): array
    {
        ...
        if (!$before) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y báº£n ghi há»™ nghÃ¨o/cáº­n nghÃ¨o');
    ...
    public function report(array $filters): array
    {
        ...
        return [
            'title' => 'BÃ¡o cÃ¡o há»™ nghÃ¨o / há»™ cáº­n nghÃ¨o',
            'headers' => ['STT','MÃ£ há»™','Chá»§ há»™','Khu','Giai Ä‘oáº¡n','Loáº¡i há»™','Tá»« ngÃ y','Äáº¿n ngÃ y','Quyáº¿t Ä‘á»‹nh','Ghi chÃº'],
            'rows' => $rows,
            'items' => $items,
            'totalRows' => count($rows),
            'summary' => [
                'Tá»•ng há»™ nghÃ¨o' => $dashboard['metrics']['poor'],
                'Tá»•ng há»™ cáº­n nghÃ¨o' => $dashboard['metrics']['near_poor'],
                'Tá»•ng há»™ trung bÃ¬nh' => $dashboard['metrics']['medium'],
                'Tá»· lá»‡ há»™ nghÃ¨o' => $dashboard['metrics']['poor_rate'] . '%',
                'Tá»· lá»‡ há»™ cáº­n nghÃ¨o' => $dashboard['metrics']['near_poor_rate'] . '%',
                'Tá»· lá»‡ há»™ trung bÃ¬nh' => $dashboard['metrics']['medium_rate'] . '%',
                'So vá»›i nÄƒm trÆ°á»›c - há»™ nghÃ¨o' => $dashboard['metrics']['poor'] - $previous['metrics']['poor'],
                'So vá»›i nÄƒm trÆ°á»›c - há»™ cáº­n nghÃ¨o' => $dashboard['metrics']['near_poor'] - $previous['metrics']['near_poor'],
                'So vá»›i nÄƒm trÆ°á»›c - há»™ trung bÃ¬nh' => $dashboard['metrics']['medium'] - $previous['metrics']['medium'],
            ],
    ...
    private function periodParams(array $data, int $userId): array
    {
        ...
        return [
            'name' => $this->required($data['name'] ?? null, 'TÃªn giai Ä‘oáº¡n'),
            'start_date' => $this->dateOrFail($data['start_date'] ?? $data['startDate'] ?? null, 'NgÃ y báº¯t Ä‘áº§u'),
            'end_date' => $this->dateOrFail($data['end_date'] ?? $data['endDate'] ?? null, 'NgÃ y káº¿t thÃºc'),
    ...
    private function recordParams(array $data, int $userId, ?array $existing = null): array
    {
        ...
        if (!isset(self::POVERTY_TYPES[$type])) throw new RuntimeException('Loáº¡i há»™ khÃ´ng há»£p lá»‡');
        ...
        return [
            ...
            'poverty_type' => $type,
            'effective_from' => $this->dateOrFail($data['effective_from'] ?? $data['effectiveFrom'] ?? $existing['effective_from'] ?? null, 'NgÃ y báº¯t Ä‘áº§u'),
        ...
        foreach ($rows as $row) {
            ...
            if ($effectiveTo < (string) $row['effective_from']) {
                if ($newEffectiveFrom !== (string) $row['effective_from']) {
                    throw new RuntimeException('NgÃ y báº¯t Ä‘áº§u má»›i pháº£i sau ngÃ y báº¯t Ä‘áº§u cá»§a báº£n ghi hiá»‡u lá»±c hiá»‡n táº¡i');
                }
    ...
    {
        if ($end < $start) throw new RuntimeException('NgÃ y káº¿t thÃºc giai Ä‘oáº¡n pháº£i sau ngÃ y báº¯t Ä‘áº§u');
    }

    private function validateRecordDates(string $from, ?string $to, array $period): void
    {
        if ($to !== null && $to < $from) throw new RuntimeException('NgÃ y káº¿t thÃºc pháº£i sau ngÃ y báº¯t Ä‘áº§u');
        if ($from < (string) $period['start_date'] || $from > (string) $period['end_date']) throw new RuntimeException('NgÃ y báº¯t Ä‘áº§u pháº£i náº±m trong giai Ä‘oáº¡n');
        if ($to !== null && ($to < (string) $period['start_date'] || $to > (string) $period['end_date'])) throw new RuntimeException('NgÃ y káº¿t thÃºc pháº£i náº±m trong giai Ä‘oáº¡n');
    }
