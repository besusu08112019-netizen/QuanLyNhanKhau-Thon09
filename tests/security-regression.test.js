...

{
  ...
  assert.ok(deploy.indexOf('Upload production artifact for audit') < deploy.indexOf('Create rollback production env'), 'audit artifact must be uploaded before production .env is created');
  assert.ok(deploy.indexOf('Create rollback production env') < deploy.indexOf('Deploy to hosting via FTPS'), 'production .env must be created only immediately before FTPS deploy');
  assert.match(deploy, /PLATFORM_ADMIN_ENABLED=false/);
  assert.match(deploy, /PLATFORM_DEFAULT_PORTAL=PUBLIC/);
