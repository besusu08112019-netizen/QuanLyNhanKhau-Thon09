...
function lockReportTypes(){if(window.__TenantAppSmartReportingBound)return;var select=q('#reportTypeSelect');if(!select)return;var value=select.value||'summary';var options=[['summary','Báo cáo tổng hợp'],['population','Báo cáo nhân khẩu'],['household','Báo cáo hộ gia đình'],['settled-elsewhere-households','Báo cáo hộ dân sinh sống ổn định ở nơi khác'],['public-assets','Công trình công cộng - Danh sách'],['public-assets-located','Công trình công cộng - Đã có GPS'],['public-assets-missing-gps','Công trình công cộng - Chưa có GPS'],['public-assets-inventory','Công trình công cộng - Kiểm kê tài sản'],['houses','Nhà ở và công trình - Danh sách'],['houses-degraded','Nhà ở xuống cấp'],['houses-temporary','Nhà tạm'],['houses-fire-risk','Nhà nguy cơ PCCC'],['houses-missing-gps','Nhà chưa có GPS'],['household-business-production','Hộ sản xuất'],['household-business-trade','Hộ kinh doanh'],['household-business-sector','Hộ SXKD theo ngành nghề'],['household-business-status','Hộ SXKD theo trạng thái'],['agriculture','Sản xuất nông nghiệp - Danh sách'],['agriculture-producers','Chủ thể sản xuất nông nghiệp'],['agriculture-area','Diện tích sản xuất nông nghiệp'],['agriculture-crop','Cây trồng'],['agriculture-season','Mùa vụ'],['agriculture-production','Sản lượng nông nghiệp'],['agriculture-damage','Thiệt hại nông nghiệp'],['livestock','Vật nuôi - Danh sách'],['livestock-by-type','Vật nuôi theo loại'],['livestock-vaccinated','Vật nuôi đã tiêm phòng'],['livestock-unvaccinated','Vật nuôi chưa tiêm phòng'],['livestock-disease','Vật nuôi có dịch bệnh'],['livestock-pig-farms','Danh sách trang trại lợn'],['livestock-pig-sow','Danh sách hộ nuôi lợn nái'],['livestock-pig-meat','Danh sách hộ nuôi lợn thịt'],['livestock-pig-sow-and-meat','Hộ vừa nuôi lợn nái vừa nuôi lợn thịt'],['vehicles','Xe cộ - Danh sách'],['vehicles-by-type','Xe cộ theo loại'],['vehicles-missing-plate','Xe chưa có biển số'],['vehicles-expired-inspection','Xe hết hạn kiểm định'],['vehicles-expired-insurance','Xe hết hạn bảo hiểm'],['contributions-list','Đóng góp hộ - Danh sách'],['contributions-collection','Đóng góp hộ - Thu tiền'],['contributions-unpaid-list','Đóng góp hộ - Chưa nộp'],['contributions-partial','Đóng góp hộ - Nộp một phần'],['contributions-exempt','Đóng góp hộ - Miễn giảm'],['contributions-summary','Đóng góp hộ - Tổng hợp'],['contributions-year-summary','Đóng góp hộ - Tổng hợp năm'],['contributions-by-contribution','Đóng góp hộ - Theo khoản thu'],['gis','GIS - Hộ gia đình'],['gis-located','GIS - Đã định vị'],['gis-unlocated','GIS - Chưa định vị'],['digital-profile','Hồ sơ số'],['profile-complete','Hồ sơ hoàn chỉnh'],['profile-missing-photo','Hồ sơ thiếu ảnh'],['profile-missing-documents','Hồ sơ thiếu giấy tờ'],['profile-incomplete','Hồ sơ chưa hoàn thiện'],['temporary_residence','Báo cáo tạm trú'],['temporary_absence','Báo cáo tạm vắng'],['migration','Báo cáo biến động'],['health_insurance','Báo cáo Bảo hiểm y tế'],['health-insurance-missing','Danh sách chưa tham gia BHYT'],['health-insurance-expiring','Danh sách BHYT sắp hết hạn (30 ngay)'],['health-insurance-expired','Danh sách BHYT đã hết hạn'],['health-insurance-household','Thống kê BHYT theo hộ'],['health-insurance-area','Thống kê BHYT theo khu vực'],['party_member','Báo cáo Đảng viên'],['meritorious_person','Báo cáo người có công'],['disabled_person','Báo cáo người khuyết tật'],['age','Báo cáo theo độ tuổi'],['gender','Báo cáo theo giới tính'],['youth_union_member','Báo cáo Đoàn viên'],['poor-households','Báo cáo hộ nghèo'],['near-poor-households','Báo cáo hộ cận nghèo'],['labor','Báo cáo lao động'],['elderly','Báo cáo người cao tuổi'],['children','Báo cáo trẻ em']];Array.prototype.forEach.call(select.options,function(option){if(!options.some(function(item){return item[0]===option.value;}))options.push([option.value,option.textContent||option.value]);});var html=options.map(function(item){return '<option value="'+item[0]+'">'+item[1]+'</option>';}).join('');if(select.innerHTML.replace(/\s+/g,' ').trim()!==html.replace(/\s+/g,' ').trim())select.innerHTML=html;if(!Array.prototype.some.call(select.options,function(o){return o.value===value;})){select.appendChild(new Option(value,value));}select.value=value;}
...
function applyResidence(p,value){if(value==='PERMANENT'||value==='TEMPORARY')p.set('residencyStatus',value);else if(value==='AWAY')p.set('presenceStatus','AWAY');else if(value==='MOVED_OUT')p.set('presenceStatus','MOVED_OUT');}
...
function explicitRoutePath(){if(!window.location)return false;var path=(window.location.pathname||'/').replace(/\/+$/,'')||'/';return path!=='/'&&path!=='/dashboard';}
function routeScreen(){var platform=window.TenantAppPlatform,router=platform&&platform.router;if(!router||typeof router.resolve!=='function'||!window.location)return '';try{var path=(window.location.pathname||'/')+(window.location.search||'');var state=router.resolve(path);var screen=state&&state.screenId||'';return screen&&(targetFor(screen)||platformModule(screen))?screen:'';}catch(error){return '';}}
function currentPath(){if(!window.location)return '/';return (window.location.pathname||'/')+(window.location.search||'');}
function canonicalPathFor(screen){var module=platformModule(screen);var path=module&&module.path||'';if(!path&&screen==='dashboard')path='/';return path||'';}
function samePath(left,right){function clean(value){return String(value||'/').replace(/\/+$/,'')||'/';}return clean(left)===clean(right);}
function syncUrl(screen,mode){if(!window.history||!window.history.pushState)return false;var path=canonicalPathFor(screen);if(!path)return false;var current=currentPath();if(samePath(current,path))return false;var state={screen:screen,path:path};try{if(mode==='push')window.history.pushState(state,'',path);else if(mode==='replace')window.history.replaceState(state,'',path);else return false;writeLog('syncUrl',{screen:screen,mode:mode,path:path,previousPath:current});return true;}catch(error){writeLog('syncUrlError',{screen:screen,mode:mode,path:path,message:error&&error.message||String(error)});return false;}}
function currentScreen(){return platformScreen()||(window.App&&window.App.screen)||storedScreen()||'dashboard';}
function initialScreen(){return routeScreen()||platformScreen()||(explicitRoutePath()?'dashboard':(storedScreen()||(window.App&&window.App.screen)||'dashboard'));}
...
function targetRouteState(screen,requested,options){var platform=window.TenantAppPlatform,router=platform&&platform.router,config=options||{},intent=config.intent||null,module=platformModule(requested)||platformModule(screen),state=null;if(intent&&(intent.moduleKey||intent.route)){state=Object.assign({},intent);}else{try{state=router&&typeof router.resolve==='function'&&router.resolve(currentPath())||null;}catch(error){state=null;}}if(state&&state.screenId&&state.screenId!==screen){var stateModule=platformModule(state.moduleKey||state.screenId);if(!(stateModule&&stateModule.screenId===screen))state=null;}return Object.assign({screenId:screen,moduleKey:module&&module.moduleKey||'',route:module&&module.path||''},state||{}, {screenId:screen});}
function activeRouteState(screen,requested,explicitState){if(explicitState)return explicitState;var platform=window.TenantAppPlatform,nav=platform&&platform.navigation,router=platform&&platform.router,state=null;try{state=router&&typeof router.resolve==='function'&&router.resolve(currentPath())||null;}catch(error){}if(!state||!state.moduleKey){try{state=nav&&typeof nav.current==='function'&&nav.current()||state;}catch(error){}}var module=platformModule(requested)||platformModule(screen);return Object.assign({screenId:screen,moduleKey:module&&module.moduleKey||'',route:module&&module.path||''},state||{});}
function cleanRoute(value){return String(value||'/').split('#')[0].split('?')[0].replace(/\/+$/,'')||'/';}
function navRoute(btn){var route=btn.dataset&&btn.dataset.route||'';if(!route&&btn.getAttribute)route=btn.getAttribute('href')||'';return route&&route.charAt(0)==='/'?route:'';}
function navButtonActive(btn,state,screen,requested,mobile){var navScreen=mobile?btn.dataset.mobileScreen:btn.dataset.screen;var moduleKey=btn.dataset&&(btn.dataset.module||btn.dataset.moduleKey)||'';var route=navRoute(btn);if(moduleKey&&state.moduleKey&&moduleKey===state.moduleKey)return true;if(route&&state.route)return cleanRoute(route)===cleanRoute(state.route);if(moduleKey&&state.moduleKey)return false;return navScreen===screen||navScreen===requested;}
function syncActiveNavigation(screen,requested,state){var activeState=activeRouteState(screen,requested,state);document.querySelectorAll('.sidebar .nav-link[data-screen]').forEach(function(btn){var active=navButtonActive(btn,activeState,screen,requested,false);btn.classList.toggle('active',active);btn.setAttribute('aria-current',active?'page':'false');});syncSidebarAccordion(screen,requested);document.querySelectorAll('.mobile-bottom-nav [data-mobile-screen]').forEach(function(btn){var active=navButtonActive(btn,activeState,screen,requested,true);btn.classList.toggle('active',active);btn.setAttribute('aria-current',active?'page':'false');if(active)centerMobile(btn);});}
...
function displayScreen(screen,requested,options){var target=targetFor(screen);var loader=loaderFor(screen);if(!target&&loader&&typeof window[loader]==='function'){try{window[loader]();}catch(error){writeLog('displayLoaderError',{screen:screen,loader:loader,message:error&&error.message||String(error)});}target=targetFor(screen);}if(!target&&loader){writeLog('displayWaitForLoader',{screen:screen,loader:loader});ensureModuleAsset(screen,loader).then(function(){displayScreen(screen,requested,options);}).catch(function(){renderLoaderError(screen,loader);});return false;}if(!target){writeLog('displayScreen',{screen:screen,found:false});return false;}if(typeof window.TenantAppApplyModuleDisplayOrder==='function')window.TenantAppApplyModuleDisplayOrder();var routeState=targetRouteState(screen,requested,options);setAppState(screen,requested);hideOtherScreens(target);syncActiveNavigation(screen,requested,routeState);updateHeader(screen,requested);closeMobileShell();render(screen);writeLog('displayScreen',Object.assign({screen:screen,targetScreen:target.id,currentScreen:currentScreen(),activeRouteState:routeState},domState()));return true;}
function navigate(screen,event,options){options=options||{};if(event&&event.defaultPrevented)return false;log=[];var requested=screen;var routed=routeScreen();var normalized=normalize(screen);if(!event&&!options.historyRestore&&explicitRoutePath()&&routed&&routed!==normalized)normalized=routed;var module=platformModule(normalized);if(!targetFor(normalized)&&!module)normalized='dashboard';if(event&&typeof event.preventDefault==='function')event.preventDefault();if(event&&typeof event.stopPropagation==='function')event.stopPropagation();writeLog('Click menu',{menuKey:requested,moduleKey:normalized,targetScreen:normalized+'Screen',currentScreen:currentScreen(),eventTarget:event&&event.target&&event.target.tagName,eventCurrentTarget:event&&event.currentTarget&&event.currentTarget.tagName,historyMode:options.historyMode||''});var displayed=displayScreen(normalized,requested,options);if(displayed){var mode=options.historyMode||'';if(!mode&&event)mode='push';if(mode)syncUrl(normalized,mode);}return displayed;}
...
window.addEventListener('popstate',function(){var screen=routeScreen();if(screen)navigate(screen,null,{historyRestore:true});});
function initialHistoryMode(screen){if(!explicitRoutePath())return '';var path=currentPath();if(screen==='communityOrganizations'&&samePath(path,'/community-organizations'))return 'replace';return '';}
function startInitialNavigation(attempt){var screen=initialScreen();if(screen==='dashboard'&&explicitRoutePath()&&!routeScreen()&&attempt<20){setTimeout(function(){startInitialNavigation(attempt+1);},50);return;}navigate(screen,null,{historyMode:initialHistoryMode(screen)});}
setTimeout(function(){startInitialNavigation(0);},0);
}
