...
if (role === 'VIEWER') return ['dashboard','household','household_business','citizen','report','gis','public_assets'].includes(module) && action === 'read';
if (['user','permission','logs','settings','backup','system_admin'].includes(module)) return false;
if (role === 'OFFICER') return (['dashboard','household','household_business','citizen','movement','report','public_assets'].includes(module) && ['read','create','update'].includes(action)) || (module === 'gis' && action === 'read');
return false;
}
...
sheet.innerHTML = '<div class="mobile-filter-sheet-head"><strong>' + escapeHtml(config.label) + '</strong><button class="mobile-filter-close" type="button" aria-label="ÄÃ³ng bá»™ lá»c"><i class="fa-solid fa-xmark"></i></button></div><div class="mobile-filter-sheet-body"></div>';
...
toggle.innerHTML = '<i class="fa-solid fa-sliders"></i><span>Bá»™ lá»c</span>';
...
publicAssets: 'C\u00f4ng tr\u00ecnh c\u00f4ng c\u1ed9ng',
...
{ key: 'temporary_count', label: 'Táº¡m trÃº', unit: 'ngÆ°á»i', icon: 'fa-location-dot', tone: 'cyan' },
{ key: 'away_count', label: 'Táº¡m váº¯ng', unit: 'ngÆ°á»i', icon: 'fa-person-walking-arrow-right', tone: 'purple' },
{ key: 'poor_households', label: 'Há»™ nghÃ¨o', unit: 'há»™', icon: 'fa-hand-holding-heart', tone: 'orange' },
{ key: 'near_poor_households', label: 'Há»™ cáº­n nghÃ¨o', unit: 'há»™', icon: 'fa-hands-holding', tone: 'pink' }
];
...
canUpdateHousehold ? '<button class="btn btn-sm btn-outline-primary" onclick="openHouseholdForm(' + row.id + ')">Sá»­a</button>' : '',
canDeleteHousehold ? '<button class="btn btn-sm btn-outline-danger" onclick="deleteHousehold(' + row.id + ')">XÃ³a</button>' : ''
]) +
...
canAccess('citizen', 'update') ? '<button class="btn btn-sm person-row-btn person-row-edit" onclick="openPersonForm(' + row.id + ')">Sá»­a</button>' : '',
canAccess('citizen', 'delete') ? '<button class="btn btn-sm btn-outline-danger" onclick="deletePerson(' + row.id + ')">XÃ³a</button>' : ''
])
+ '</tr>';
}
...
$('#detailBody').innerHTML = householdPhotoDetailHtml(row) + details([['MÃ£ há»™', row.household_code], ['Chá»§ há»™', row.head_citizen_name], ['Äá»‹a chá»‰', row.address], ['Sá»‘ Ä‘iá»‡n thoáº¡i', row.phone], ['á»ž nhÃ ', row.at_home_count || 0], ['Äi váº¯ng', row.away_count || 0], ['Diá»‡n há»™', stripTags(householdBadges(row))], ['Ghi chÃº', row.note]]) + memberTable(members.items || []);
...
? '<button class="btn btn-link household-member-link" type="button" title="Xem há»“ sÆ¡ nhÃ¢n kháº©u" aria-label="Xem há»“ sÆ¡ nhÃ¢n kháº©u ' + name + '" onclick="openCitizenProfile(' + citizenId + ')">' + name + '</button>'
: name;
...
const buttons = [];
if (citizenId) {
buttons.push('<button class="btn btn-sm btn-outline-primary household-member-action" type="button" title="Xem há»“ sÆ¡ nhÃ¢n kháº©u" aria-label="Xem há»“ sÆ¡ nhÃ¢n kháº©u" onclick="openCitizenProfile(' + citizenId + ')"><i class="fa-solid fa-id-card"></i></button>');
if (memberHasDigitalProfile(row)) buttons.push('<button class="btn btn-sm btn-outline-secondary household-member-action" type="button" title="Há»“ sÆ¡ sá»‘" aria-label="Há»“ sÆ¡ sá»‘" onclick="openCitizenProfile(' + citizenId + ')"><i class="fa-solid fa-folder-open"></i></button>');
}
...
+ (canAccess('gis', 'update') ? '<button class="btn btn-sm btn-outline-primary" onclick="editGisArea(' + Number(area.id) + ')">S\u1eeda ranh gi\u1edbi</button>' : '')
+ (canAccess('gis', 'delete') ? '<button class="btn btn-sm btn-outline-danger" onclick="deleteGisArea(' + Number(area.id) + ')">X\u00f3a ranh gi\u1edbi</button>' : '')
+ (canAccess('gis', 'export') ? '<button class="btn btn-sm btn-outline-secondary" onclick="exportGisPdf()">Xu\u1ea5t PDF</button>' : '') + '</div></div>';
}
...
gisDebugTrace('cluster.refreshClusters()');
...
if (typeof App.gis.markerGroup.addLayers === 'function') { gisDebugTrace('cluster.addLayers()', { nextMarkerCount: nextMarkers.length }); App.gis.markerGroup.addLayers(nextMarkers); }
...
return '<a class="btn btn-sm btn-success" target="_blank" rel="noopener" data-gis-popup-action="route" href="' + gisEscape(url) + '"><i class="fa-brands fa-google"></i> Chá»‰ Ä‘Æ°á»ng báº±ng Google Maps</a>';
}
...
return '<article class="gis-area-item' + (active ? ' is-active' : '') + '" data-gis-area-id="' + gisEscape(areaKey) + '" aria-selected="' + (active ? 'true' : 'false') + '"><button class="gis-area-main" type="button"><span><b>' + gisEscape(area.name) + '</b><small>' + gisEscape(area.area_code) + '</small></span><em>' + gisNumber(area.stats?.households) + ' h\u1ed9 / ' + gisNumber(area.stats?.citizens) + ' NK</em></button><div class="gis-area-row-actions">' + (canAccess('gis','update') ? '<button class="btn btn-sm btn-outline-primary" type="button" onclick="editGisArea(' + Number(area.id ?? area.area_id) + ')"><i class="fa-solid fa-pen-to-square"></i></button>' : '') + (canAccess('gis','delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" onclick="deleteGisArea(' + Number(area.id ?? area.area_id) + ')"><i class="fa-solid fa-trash"></i></button>' : '') + '</div></article>';
...
if (!navigator.geolocation || !window.isSecureContext) return showToast('GPS chá»‰ hoáº¡t Ä‘á»™ng trÃªn HTTPS hoáº·c localhost.', 'warning');
if (!App.gis.selectedHouseholdId) return showToast('Vui lÃ²ng má»Ÿ marker há»™ cáº§n lÆ°u GPS trÆ°á»›c.', 'warning');
stopGisGps(false);
...
updateGisGpsPanel('Äang láº¥y tÃ­n hiá»‡u GPS...', 'Äang chá»...', true);
...
panel.innerHTML = '<div class="d-flex justify-content-between gap-2"><strong>Äá»‹nh vá»‹ GPS</strong><button class="btn btn-sm btn-outline-secondary" type="button" data-gis-gps-close>ÄÃ³ng</button></div><div class="gis-gps-grid"><span>Tráº¡ng thÃ¡i</span><b data-gis-gps-status>Äang láº¥y tÃ­n hiá»‡u GPS...</b><span>Sai sá»‘</span><b data-gis-gps-accuracy>Äang chá»...</b></div><div class="small text-muted" data-gis-gps-note>KhÃ´ng lÆ°u vá»‹ trÃ­ náº¿u sai sá»‘ trÃªn 10 m.</div><div class="d-flex gap-2"><button class="btn btn-outline-success btn-sm" type="button" data-gis-gps-retry>Äá»‹nh vá»‹ láº¡i</button></div>';
...
if (!watch.marker) watch.marker = L.marker(bestLatLng, { title: 'Vá»‹ trÃ­ GPS tá»‘t nháº¥t' }).addTo(App.gis.map);
...
updateGisGpsPanel('GPS chÃ­nh xÃ¡c - Ä‘ang lÆ°u...', best.accuracy.toFixed(1) + ' m', true);
...
updateGisGpsPanel('Äá»§ chÃ­nh xÃ¡c - Ä‘ang lÆ°u...', best.accuracy.toFixed(1) + ' m', true);
saveGisGpsForSelectedHousehold(best);
return;
}
updateGisGpsPanel(best.accuracy <= GIS_GPS_MAX_ACCEPTABLE_ACCURACY ? 'Äang cáº£i thiá»‡n...' : 'Äang láº¥y tÃ­n hiá»‡u GPS...', best.accuracy.toFixed(1) + ' m', true);
}
function handleGisGpsError(error) {
const message = error?.message || 'KhÃ´ng láº¥y Ä‘Æ°á»£c GPS hiá»‡n táº¡i';
updateGisGpsPanel('KhÃ´ng láº¥y Ä‘Æ°á»£c GPS', message, false);
...
updateGisGpsPanel('KhÃ´ng thá»ƒ láº¥y GPS chÃ­nh xÃ¡c', best ? best.accuracy.toFixed(1) + ' m' : 'KhÃ´ng cÃ³ tÃ­n hiá»‡u', false);
showToast('KhÃ´ng thá»ƒ láº¥y GPS chÃ­nh xÃ¡c. Vui lÃ²ng di chuyá»ƒn ra nÆ¡i thoÃ¡ng hoáº·c báº­t GPS chÃ­nh xÃ¡c cao rá»“i thá»­ láº¡i.', 'warning');
}
...
if (!gps || !id) return showToast('Vui lÃ²ng má»Ÿ marker há»™ cáº§n lÆ°u GPS trÆ°á»›c.', 'warning');
...
if (!Number.isFinite(savedLat) || !Number.isFinite(savedLng)) throw new Error('Dá»¯ liá»‡u GPS chÆ°a Ä‘Æ°á»£c ghi vÃ o database.');
const cached = App.gis.markerCache.get(String(id));
...
if (Math.abs(markerLat - savedLat) > 0.000001 || Math.abs(markerLng - savedLng) > 0.000001) throw new Error('Marker chÆ°a cáº­p nháº­t Ä‘Ãºng vá»‹ trÃ­ vá»«a lÆ°u.');
}
updateGisGpsPanel('GPS chÃ­nh xÃ¡c', Number(saved.location_accuracy || gps.accuracy).toFixed(1) + ' m', false);
showToast('Äá»‹nh vá»‹ thÃ nh cÃ´ng');
setTimeout(() => stopGisGps(true), 800);
} catch (error) {
watch.saving = false;
updateGisGpsPanel('KhÃ´ng lÆ°u Ä‘Æ°á»£c GPS', gps.accuracy.toFixed(1) + ' m', false);
showToast(error.message || 'KhÃ´ng lÆ°u Ä‘Æ°á»£c vá»‹ trÃ­ GPS.', 'danger');
}
}
