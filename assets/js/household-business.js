(() => {
  ...
  function detailHtml(row) {
    ...
    const updatedAt = latestActivityDate(activities) || row.updated_at || row.created_at;
    const primary = primaryActivity(activities);
    const typeValues = uniqueValues(activities.map(activity => activity.business_type_label || activity.business_type));
    return '<article class="business-detail-dashboard">'
      + '<section class="business-detail-grid business-detail-grid-top">'
      + dashboardInfoCard('Thông tin hộ', 'fa-house-user', [
        dashboardInfoRow('Mã hộ', row.household_code),
        dashboardInfoRow('Chủ hộ', row.head_citizen_name),
        dashboardInfoRow('Điện thoại', row.phone || 'Chưa cập nhật'),
        dashboardInfoRow('Địa chỉ', row.address || 'Chưa cập nhật')
      ].join(''))
      + dashboardInfoCard('Trạng thái chung', 'fa-signal', [
        dashboardStatusRow('Trạng thái', statusBadge(isActive ? 'ACTIVE' : '', isActive ? 'Đang hoạt động' : 'Chưa hoạt động')),
        dashboardStatusRow('GPS', flagBadge(hasGps, hasGps ? 'Có GPS' : 'Không GPS')),
        dashboardStatusRow('OCOP', flagBadge(hasOcop, hasOcop ? 'Có' : 'Không')),
        dashboardStatusRow('ATTP', flagBadge(hasFoodSafety, hasFoodSafety ? 'Có' : 'Không')),
        dashboardStatusRow('BHXH', flagBadge(hasSocialInsurance, hasSocialInsurance ? 'Có' : 'Không')),
        dashboardStatusRow('Ngày cập nhật', '<strong>' + esc(date(updatedAt) || 'Chưa cập nhật') + '</strong>')
      ].join(''))
      + '</section>'
      + '<section class="business-detail-grid">'
      + dashboardInfoCard('Thông tin hoạt động', 'fa-briefcase', primary ? [
        dashboardInfoRow('Loại hình hoạt động chính', primary.business_type_label || primary.business_type),
        dashboardInfoRow('Ngành nghề chính', primary.sector_label || primary.economic_type || primary.production_sector || primary.business_sector),
        dashboardInfoRow('Quy mô', primary.business_scale),
        dashboardInfoRow('Lao động', num(primary.worker_count || 0) + ' người'),
        dashboardInfoRow('Ngày bắt đầu', date(primary.start_date)),
        dashboardInfoRow('Ghi chú', primary.note)
      ].join('') : '<div class="business-empty-state">Chưa có thông tin hoạt động.</div>')
      + businessTypeCard(typeValues, activities)
      + '</section>'
      + '<section class="business-detail-card business-detail-card-wide">'
      + '<div class="business-detail-card-head"><div><i class="fa-solid fa-store"></i><h4>Danh sách hoạt động</h4></div>' + addButton + '</div>'
      + '<div class="business-detail-activity-list">' + (activities.length ? activities.map((activity, index) => dashboardActivityRow(activity, index)).join('') : '<div class="business-empty-state">Chưa có hoạt động sản xuất/kinh doanh.</div>') + '</div>'
      + '</section>'
      + '<section class="business-detail-grid">'
      + documentsCard(activities)
      + notesCard(activities)
      + '</section>'
      + '</article>';
  }
