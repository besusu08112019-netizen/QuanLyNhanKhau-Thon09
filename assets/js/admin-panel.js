(() => {
  ...
  async function printAdminReport(type) {
    ...
    win.document.write(`<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>${escapeHtml(report.title)}</title><style>@page{size:A4;margin:14mm}body{font-family:"Segoe UI",Roboto,Arial,"Noto Sans",sans-serif}table{width:100%;border-collapse:collapse}th,td{border:1px solid #555;padding:6px;font-size:12px}</style></head><body><h2>${escapeHtml(report.title)}</h2>${reportTableHtml(report)}<script>window.print();<\/script></body></html>`);
    win.document.close();
  }
