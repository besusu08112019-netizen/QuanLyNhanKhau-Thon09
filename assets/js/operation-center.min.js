...
return '<div class="operation-item operation-priority-' + esc(item.priority || 'low') + '"><span><strong>' + esc(item.label) + '</strong><small>' + esc(priorityLabel(item.priority)) + ' Â· ' + esc(statusLabel(status)) + '</small></span><div class="operation-actions"><span class="operation-badge">' + num(item.count) + '</span><button class="btn btn-sm btn-outline-warning" data-operation-task="' + esc(item.key) + '" data-status="doing">Äang xá»­ lÃ½</button><button class="btn btn-sm btn-outline-success" data-operation-task="' + esc(item.key) + '" data-status="done">HoÃ n thÃ nh</button><button class="btn btn-sm btn-outline-primary" data-operation-screen="' + esc(item.screen || 'dashboard') + '">Má»Ÿ</button></div></div>';
...
host.innerHTML = items.length ? items.map(item => '<button type="button" data-profile-type="' + esc(item.type) + '" data-profile-id="' + Number(item.id || 0) + '"><strong>' + esc(item.title) + '</strong><span>' + esc(item.subtitle || item.meta || '') + '</span></button>').join('') : '<div class="operation-empty">KhÃ´ng tÃ¬m tháº¥y káº¿t quáº£</div>';
