...
final class TenantMigrationService
{
    ...
    public function targetedMigrationPlan(PDO $pdo, string $migrationId): array
    {
        $migration = $this->findMigrationOrFail($migrationId);
        $audit = $this->audit($pdo, true);
        $applied = $this->appliedMigrations($pdo);
        $isDone = (($applied[$migration['id']]['status'] ?? '') === 'DONE');
        $compatibility = $audit['livestockMigrationCompatibility'] ?? ['status' => 'BLOCKED', 'reasons' => ['Compatibility audit unavailable.']];
        $plan = $this->livestockFacilitiesPlan($pdo, (string) $migration['id'], $audit, $compatibility);
        $plan['alreadyRecorded'] = $isDone;
        $plan['willRun'] = !$isDone && in_array((string) ($compatibility['status'] ?? ''), ['SAFE', 'NO_LIVESTOCK_DATA'], true);
        if ($isDone || ($compatibility['status'] ?? '') === 'ALREADY_APPLIED') {
            $plan['willRun'] = false;
            $plan['schemaUpgradeOnly'] = false;
        }
        return $plan;
    }

    public function applyOnlyMigration(PDO $pdo, string $migrationId, string $tenantLabel = 'tenant', bool $dryRun = false): array
    {
        $migration = $this->findMigrationOrFail($migrationId);
        $plan = $this->targetedMigrationPlan($pdo, (string) $migration['id']);
        $status = (string) (($plan['compatibility']['status'] ?? '') ?: 'BLOCKED');
        if (($plan['alreadyRecorded'] ?? false) || $status === 'ALREADY_APPLIED') {
            return ['tenant' => $tenantLabel, 'appVersion' => $this->currentAppVersion(), 'schemaVersion' => $this->latestAppliedVersion($pdo), 'results' => [['id' => $migration['id'], 'status' => 'SKIPPED', 'reason' => $status]]];
        }
        if (!in_array($status, ['SAFE', 'NO_LIVESTOCK_DATA'], true)) {
            throw new RuntimeException($tenantLabel . ': targeted migration ' . $migration['id'] . ' blocked: ' . implode('; ', $plan['blockedReasons'] ?? ['Compatibility check failed.']));
        }
        if ($dryRun) {
            return ['tenant' => $tenantLabel, 'appVersion' => $this->currentAppVersion(), 'schemaVersion' => $this->latestAppliedVersion($pdo), 'results' => [['id' => $migration['id'], 'status' => 'PENDING', 'plan' => $plan]]];
        }
        ...
        try {
            $statements = $this->executeSqlFile($pdo, (string) $migration['path']);
            $this->markDone($pdo, $migration, $statements, (int) round((microtime(true) - $started) * 1000));
            return ['tenant' => $tenantLabel, 'appVersion' => $this->currentAppVersion(), 'schemaVersion' => (string) $migration['id'], 'results' => [['id' => $migration['id'], 'status' => 'DONE', 'statements' => $statements]]];
        } catch (Throwable $e) {
            $this->markFailed($pdo, $migration, $e);
            throw new RuntimeException($tenantLabel . ': targeted migration ' . $migration['id'] . ' failed: ' . $e->getMessage(), 0, $e);
        }
    }

    private function findMigrationOrFail(string $migrationId): array
    {
        $migrationId = trim($migrationId);
        if ($migrationId === '') {
            throw new RuntimeException('Missing --only migration id.');
        }
        foreach ($this->migrations() as $migration) {
        ...
        throw new RuntimeException('Requested migration does not exist: ' . $migrationId);
    }

    private function livestockFacilitiesPlan(PDO $pdo, string $migrationId, array $audit, array $compatibility): array
    {
        $livestock = $audit['livestock'] ?? [];
        $status = (string) ($compatibility['status'] ?? 'BLOCKED');
        $hasLivestock = (bool) ($livestock['tables']['livestock'] ?? false);
        $hasFacilities = (bool) ($livestock['tables']['livestock_facilities'] ?? false);
        $records = (int) ($livestock['records'] ?? 0);
        $households = (int) ($livestock['households'] ?? 0);
        $total = (float) ($livestock['total_quantity'] ?? 0);
        $pigRecords = (int) ($livestock['pig_records'] ?? 0);
        $pigTotal = (float) ($livestock['pig_total'] ?? 0);
        $blockedReasons = [];
        if (!in_array($status, ['SAFE', 'NO_LIVESTOCK_DATA', 'ALREADY_APPLIED'], true)) {
            $blockedReasons = array_values(array_map('strval', $compatibility['reasons'] ?? ['Compatibility status is ' . $status]));
        }
        ...
        return [
            ...
            'mode' => 'PLAN',
            'readOnly' => true,
            'actualSchema' => $audit['actualSchema'] ?? [],
            'hasLivestock' => $hasLivestock,
            'hasLivestockFacilities' => $hasFacilities,
            'records' => $records,
            'households' => $households,
            'totalQuantity' => $total,
            'pigRecords' => $pigRecords,
            'pigTotal' => $pigTotal,
            'animalTypes' => $livestock['animal_type_totals'] ?? [],
            'estimatedFacilitiesToCreate' => $this->estimatedLegacyFacilitiesToCreate($pdo),
            'estimatedRecordsToAttach' => $this->estimatedLegacyRecordsToAttach($pdo),
            'estimatedPigRecordsToUnclassified' => $this->estimatedPigRecordsToUnclassified($pdo),
            'schemaUpgradeOnly' => $status === 'NO_LIVESTOCK_DATA',
            'compatibility' => $compatibility,
            'blockedReasons' => $blockedReasons,
            'operations' => [
                'createTables' => ['livestock_facilities'],
                'addColumns' => ['livestock.facility_id', 'livestock.animal_group', 'livestock.unit'],
                'addIndexes' => ['livestock.facility_id', 'livestock.animal_group'],
                'addForeignKeys' => ['livestock.facility_id -> livestock_facilities.id'],
                'dataMigration' => ['create one default facility per household/village', 'attach legacy livestock rows', 'set unknown pig groups to UNCLASSIFIED'],
            ],
        ];
    }

    private function estimatedLegacyFacilitiesToCreate(PDO $pdo): int
    {
        if (!$this->tableExists($pdo, 'livestock') || !$this->tableExists($pdo, 'households')) return 0;
        $facilityColumn = in_array('facility_id', $this->columns($pdo, 'livestock'), true) ? 'l.facility_id IS NULL' : '1=1';
        if (!$this->tableExists($pdo, 'livestock_facilities')) {
            return (int) $pdo->query("SELECT COUNT(*) FROM (SELECT COALESCE(l.village_id, h.village_id) AS village_id, l.household_id FROM livestock l INNER JOIN households h ON h.id = l.household_id WHERE $facilityColumn GROUP BY COALESCE(l.village_id, h.village_id), l.household_id) x")->fetchColumn();
        }
        return (int) $pdo->query("SELECT COUNT(*) FROM (SELECT COALESCE(l.village_id, h.village_id) AS village_id, l.household_id FROM livestock l INNER JOIN households h ON h.id = l.household_id WHERE $facilityColumn GROUP BY COALESCE(l.village_id, h.village_id), l.household_id) legacy WHERE NOT EXISTS (SELECT 1 FROM livestock_facilities lf WHERE lf.household_id = legacy.household_id AND COALESCE(lf.village_id, 0) = COALESCE(legacy.village_id, 0) AND lf.note = 'Tao tu dong khi nang cap du lieu chan nuoi cu')")->fetchColumn();
    }

    private function estimatedLegacyRecordsToAttach(PDO $pdo): int
    {
        if (!$this->tableExists($pdo, 'livestock') || !$this->tableExists($pdo, 'households')) return 0;
        $facilityColumn = in_array('facility_id', $this->columns($pdo, 'livestock'), true) ? 'l.facility_id IS NULL' : '1=1';
        return (int) $pdo->query("SELECT COUNT(*) FROM livestock l INNER JOIN households h ON h.id = l.household_id WHERE $facilityColumn")->fetchColumn();
    }

    private function estimatedPigRecordsToUnclassified(PDO $pdo): int
    {
        if (!$this->tableExists($pdo, 'livestock')) return 0;
        $columns = $this->columns($pdo, 'livestock');
        if (!in_array('animal_type', $columns, true)) return 0;
        $groupCondition = in_array('animal_group', $columns, true) ? "(animal_group IS NULL OR TRIM(animal_group) = '')" : '1=1';
        return (int) $pdo->query('SELECT COUNT(*) FROM livestock WHERE ' . $groupCondition . ' AND ' . $this->pigSqlCondition('animal_type'))->fetchColumn();
    }
    ...
    {
        $tables = [];
        foreach (['villages','users','households','citizens','settings','platform_settings','rural_clean_water','defense_security_settings','defense_nvqs_records','defense_militia_records','defense_security_force_records','defense_security_records','defense_security_record_logs','defense_security_incident_types','defense_security_incidents','defense_security_incident_people','defense_security_incident_logs','organizations','organization_positions','organization_members','organization_member_history','livestock','livestock_facilities',self::MIGRATION_TABLE] as $table) {
        ...
        return [
            ...
            'features' => [
                ...
                'defense_security' => $tables['defense_security_settings']['exists'] && $tables['defense_nvqs_records']['exists'] && $tables['defense_militia_records']['exists'] && $tables['defense_security_force_records']['exists'] && $tables['defense_security_records']['exists'] && $tables['defense_security_record_logs']['exists'] && $tables['defense_security_incident_types']['exists'] && $tables['defense_security_incidents']['exists'] && $tables['defense_security_incident_people']['exists'] && $tables['defense_security_incident_logs']['exists'],
    ...
    private function pigSqlCondition(string $column): string
    {
        ...
        return "LOWER(TRIM($quoted)) IN ('lợn', 'lon', 'heo', 'pig')";
    }
