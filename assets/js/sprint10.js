(() => {
  ...
  function injectSprint10Screens() {
    ...
    if (!main) return;
    if (!document.querySelector('#logsScreen')) main.insertAdjacentHTML('beforeend', '<section id="logsScreen" class="screen"><div class="toolbar"><input id="logSearch" class="form-control" placeholder="Tìm user, hành động, module, chi tiết"><select id="logAction" class="form-select w-auto"><option value="">Tất cả hành động</option><option value="login">Đăng nhập</option><option value="logout">Đăng xuất</option><option value="create">Thêm</option><option value="update">Sửa</option><option value="delete">Xóa</option><option value="read">Xem</option><option value="export">Xuất dữ liệu</option><option value="restore">Khôi phục</option><option value="reset_password">Đặt lại mật khẩu</option></select></div><div class="content-card table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Thời gian</th><th>Người dùng</th><th>Thao tác</th><th>Module</th><th>IP</th><th>Chi tiết</th></tr></thead><tbody id="logRows"></tbody></table></div><div id="logPager" class="pager"></div></section>');
    ...
    if (!document.querySelector('#usersScreen')) main.insertAdjacentHTML('beforeend', '<section id="usersScreen" class="screen"><div class="toolbar"><input id="userSearch" class="form-control" placeholder="Tìm username, họ tên, email, số điện thoại"><select id="userPageSize" class="form-select w-auto"><option>20</option><option>50</option></select><button id="userAddBtn" class="btn btn-primary">Tạo tài khoản</button></div><div class="content-card table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Username</th><th>Họ tên</th><th>Email</th><th>Điện thoại</th><th>Chức vụ</th><th>Vai trò</th><th>Trạng thái</th><th>Ngày tạo</th><th>Lần đăng nhập cuối</th><th></th></tr></thead><tbody id="userRows"></tbody></table></div><div id="userPager" class="pager"></div></section>');
  ...
  async function loadUsers10() {
    ...
    document.querySelector('#userRows').innerHTML = (data.items || []).map(row => {
      ...
      return '<tr><td>' + escapeHtml(row.username || '') + '</td><td>' + escapeHtml(row.display_name || '') + '</td><td>' + escapeHtml(row.email || '') + '</td><td>' + escapeHtml(row.phone || '') + '</td><td>' + escapeHtml(row.position || '') + '</td><td>' + roleLabel(row.role) + '</td><td>' + escapeHtml(row.status || '') + '</td><td>' + escapeHtml(row.created_at || '') + '</td><td>' + escapeHtml(row.last_login_at || '') + '</td><td class="text-end"><button class="btn btn-sm btn-outline-primary" data-edit-user="' + row.id + '">Sửa</button> <button class="btn btn-sm btn-outline-warning" data-toggle-user="' + row.id + '" data-action="' + action + '">' + actionLabel + '</button> <button class="btn btn-sm btn-outline-secondary" data-reset-user="' + row.id + '">Đặt lại mật khẩu</button> <button class="btn btn-sm btn-outline-danger" data-delete-user="' + row.id + '">Xóa</button></td></tr>';
  ...
  async function loadPresence10(value, selector) {
    ...
    document.querySelector(selector).innerHTML = table10(['M\u00e3 h\u1ed9','H\u1ecd t\u00ean','Ng\u00e0y sinh','CCCD','\u0110i\u1ec7n tho\u1ea1i'], items.map(row => [row.household_code, (window.renderCitizenProfileLink ? window.renderCitizenProfileLink(row, row.full_name || '') : escapeHtml(row.full_name || '')), formatDate(row.date_of_birth), row.identity_number, row.phone]), { htmlColumns: [1] });
  }
  ...
  function table10(headers, rows, options = {}) { const htmlColumns = new Set(options.htmlColumns || []); return '<table class="table table-hover table-bordered align-middle mb-0"><thead><tr>' + headers.map(h => '<th>' + escapeHtml(h) + '</th>').join('') + '</tr></thead><tbody>' + (rows.length ? rows.map(r => '<tr>' + r.map((c, i) => '<td>' + (htmlColumns.has(i) ? (c ?? '') : escapeHtml(c ?? '')) + '</td>').join('') + '</tr>').join('') : '<tr><td colspan="' + Math.max(1, headers.length) + '" class="text-center text-muted py-3">Không có dữ liệu</td></tr>') + '</tbody></table>'; }
