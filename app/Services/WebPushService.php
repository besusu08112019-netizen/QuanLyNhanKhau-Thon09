...
final class WebPushService
{
    ...
    private function jwt(string $endpoint): string
    {
        ...
        if (!$privateKey || !openssl_sign($input, $der, $privateKey, OPENSSL_ALGO_SHA256)) {
            throw new \RuntimeException('Không ký được Web Push VAPID');
        }
    ...
    {
        $offset = 0;
        if (ord($der[$offset++]) !== 0x30) throw new \RuntimeException('Chữ ký VAPID không hợp lệ');
        $this->readLength($der, $offset);
        if (ord($der[$offset++]) !== 0x02) throw new \RuntimeException('Chữ ký VAPID không hợp lệ');
