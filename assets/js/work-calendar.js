(function () {
  ...
  const can = action => {
    ...
    return typeof window.thon09CanAccess === 'function' ? window.thon09CanAccess('work_calendar', action) : true;
  };
  ...
  document.addEventListener('thon09:screen-change', event => { if (event.detail?.screen === 'workCalendar') run(load); });
  ...
  function registerActions() {
    if (window.__thon09WorkCalendarActionsRegistered || !window.Thon09Platform?.actions) return;
    window.__thon09WorkCalendarActionsRegistered = true;
  ...
  function shell() {
    ...
    host.innerHTML = [
      '<section id="workCalendarDashboard" class="agri-kpi-grid" aria-label="Thống kê lịch công tác"></section>',
      '<section class="agri-filter-card" aria-label="Bộ lọc lịch công tác"><div class="agri-filter-row">',
      '<div class="agri-field agri-search-field"><label for="workCalendarSearch">Tìm kiếm</label><div class="module-search-input-wrap"><i class="fa-solid fa-magnifying-glass"></i><input id="workCalendarSearch" class="form-control" placeholder="Mã, tiêu đề, địa điểm, chủ trì..."></div></div>',
      '<div class="agri-field"><label for="workCalendarCategoryFilter">Loại</label><select id="workCalendarCategoryFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="workCalendarStatusFilter">Trạng thái</label><select id="workCalendarStatusFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="workCalendarAreaFilter">Địa bàn</label><input id="workCalendarAreaFilter" class="form-control" placeholder="Mã khu vực"></div>',
      '<div class="agri-field"><label for="workCalendarDateFrom">Từ ngày</label><input id="workCalendarDateFrom" class="form-control" type="date"></div>',
      '<div class="agri-field"><label for="workCalendarDateTo">Đến ngày</label><input id="workCalendarDateTo" class="form-control" type="date"></div>',
      '<div class="agri-field module-page-size-field"><label for="workCalendarPageSize">Hiển thị</label><select id="workCalendarPageSize" class="form-select"><option>20</option><option>50</option><option>100</option></select></div>',
      ...
      '</div></section>',
      '<section class="work-calendar-grid"><article class="module-list-card household-list-card"><div class="module-list-head"><div><h3>Lịch tháng</h3><span id="workCalendarMonthLabel">--</span></div></div><div id="workCalendarMonth" class="work-calendar-month"></div></article><article class="module-list-card household-list-card"><div class="module-list-head"><div><h3>Danh sách lịch công tác</h3><span id="workCalendarTotalCount">Tổng số: 0 lịch</span></div><button class="btn btn-success" type="button" data-platform-action="workCalendar.create"><i class="fa-solid fa-plus"></i> Thêm lịch</button></div><div class="table-responsive"><table class="table module-table align-middle mb-0"><thead><tr><th data-platform-action="workCalendar.sort" data-calendar-sort="event_code">Mã</th><th data-platform-action="workCalendar.sort" data-calendar-sort="title">Nội dung</th><th data-platform-action="workCalendar.sort" data-calendar-sort="category">Loại</th><th data-platform-action="workCalendar.sort" data-calendar-sort="start_at">Thời gian</th><th>Địa điểm</th><th data-platform-action="workCalendar.sort" data-calendar-sort="host">Chủ trì</th><th data-platform-action="workCalendar.sort" data-calendar-sort="status">Trạng thái</th><th class="text-end">Thao tác</th></tr></thead><tbody id="workCalendarRows"></tbody></table></div><div id="workCalendarPager" class="pager module-pager"></div></article></section>',
      formModal(),
      detailModal()
    ].join('');
    bind();
  ...
  async function catalogs() {
    ...
    fill($('#workCalendarCategoryFilter'), state.catalogs.categories, 'Tất cả');
    fill($('#workCalendarStatusFilter'), state.catalogs.statuses, 'Tất cả');
    fill($('#workCalendarCategoryInput'), state.catalogs.categories, 'Chọn loại');
  ...
  async function load() {
    ...
    window.thon09SyncResponsiveTableLabels?.($('#workCalendarScreen'));
  }

  function renderDashboard(data = {}) {
    const metrics = data.metrics || {};
    const cards = [['Tổng lịch', 'fa-calendar-days', metrics.total || 0], ['Hôm nay', 'fa-calendar-day', metrics.today_count || 0], ['7 ngày tới', 'fa-bell', metrics.week_count || 0], ['Hoàn thành', 'fa-circle-check', metrics.done_count || 0], ['Đã hủy', 'fa-ban', metrics.cancelled_count || 0]];
  ...
  function renderMonth(items) {
    ...
    $('#workCalendarMonthLabel').textContent = `Tháng ${month + 1}/${year}`;
  ...
  function renderRows(data = {}) {
    ...
    $('#workCalendarTotalCount').textContent = `Tổng số: ${number(data.total || 0)} lịch`;
    const tbody = $('#workCalendarRows');
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">Chưa có lịch công tác phù hợp bộ lọc.</td></tr>';
      return;
    }
    tbody.innerHTML = rows.map(item => {
      const actions = [`<button class="btn btn-sm btn-outline-primary" type="button" data-platform-action="workCalendar.detail" data-id="${item.id}" title="Xem"><i class="fa-solid fa-eye"></i></button>`, can('update') ? `<button class="btn btn-sm btn-outline-secondary" type="button" data-platform-action="workCalendar.edit" data-id="${item.id}" title="Sửa"><i class="fa-solid fa-pen"></i></button>` : '', can('delete') ? `<button class="btn btn-sm btn-outline-danger" type="button" data-platform-action="workCalendar.delete" data-id="${item.id}" title="Xóa"><i class="fa-solid fa-trash"></i></button>` : ''].filter(Boolean).join(' ');
  ...
  async function save(form) {
    ...
    toast('Đã lưu lịch công tác', 'success');
    await load();
  }
  ...
  async function openDetail(id) {
    const item = await request(API + '/' + id);
    state.current = item;
    $('#workCalendarDetailTitle').textContent = item.title || 'Chi tiết lịch công tác';
  ...
  function detailHtml(item) {
    const attendees = (item.attendees || []).map(a => `<tr><td>${safe(a.attendee_name)}</td><td>${safe(text(a.phone, ''))}</td><td>${safe(text(a.role_name, ''))}</td><td>${safe(attendanceLabel(a.attendance_status))}</td></tr>`).join('') || '<tr><td colspan="4" class="text-muted">Chưa có danh sách tham dự</td></tr>';
    ...
    return `<div class="row g-3"><div class="col-md-7"><dl class="row mb-0"><dt class="col-sm-3">Loại</dt><dd class="col-sm-9">${categoryBadge(item)}</dd><dt class="col-sm-3">Thời gian</dt><dd class="col-sm-9">${dateTime(item.start_at)} - ${dateTime(item.end_at)}</dd><dt class="col-sm-3">Nhắc việc</dt><dd class="col-sm-9">${dateTime(item.reminder_at)}</dd><dt class="col-sm-3">Địa điểm</dt><dd class="col-sm-9">${safe(text(item.location))}</dd><dt class="col-sm-3">Chủ trì</dt><dd class="col-sm-9">${safe(text(item.host_name))}</dd><dt class="col-sm-3">Nội dung</dt><dd class="col-sm-9">${safe(text(item.description))}</dd></dl></div><div class="col-md-5"><h6>File đính kèm</h6><div class="work-task-files">${attachments}</div></div><div class="col-12"><h6>Danh sách tham dự</h6><div class="table-responsive"><table class="table table-sm"><thead><tr><th>Họ tên</th><th>Điện thoại</th><th>Vai trò</th><th>Trạng thái</th></tr></thead><tbody>${attendees}</tbody></table></div></div></div>`;
  }
  ...
  async function remove(id) {
    ...
    if (!window.confirm('Xóa lịch công tác này?')) return;
    await request(API + '/' + id, { method: 'DELETE' });
    toast('Đã xóa lịch công tác', 'success');
    await load();
  }
  ...
  function statusLabel(status) { return { SCHEDULED: 'Đã lên lịch', DONE: 'Đã hoàn thành', CANCELLED: 'Đã hủy' }[status] || status; }
