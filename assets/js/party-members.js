...
enhanceStatusForm();
enhanceDateInputs(document.querySelector('#partyMemberForm') || document);
[['partyMemberBranchFilter', 'branch_name'], ['partyMemberTypeFilter', 'member_type'], ['partyMemberStatusFilter', 'party_status'], ['partyMemberPositionFilter', 'party_position'], ['partyMemberGenderFilter', 'gender']].forEach(([id, key]) => $('#' + id)?.addEventListener('change', e => { state[key] = e.target.value; state.page = 1; load(); }));
...
function enhanceStatusForm() {
const statusTab = $('#partyTabStatus .row');
...
['joined_party_date', 'official_party_date', 'status_changed_at', 'decision_date'].forEach(name => {
const input = root?.querySelector?.('[name="' + name + '"]');
if (!input) return;
input.type = 'text';
input.inputMode = 'numeric';
input.placeholder = 'dd/mm/yyyy';
input.classList.add('party-date-input');
});
}
async function load() {
if (!can('read')) return;
...
} else if (form.elements.activity_status) {
form.elements.activity_status.value = 'ACTIVE';
}
openModal('partyMemberModal');
}
function setForm(row) {
const form = $('#partyMemberForm');
const map = { id: 'id', citizen_id: 'citizen_id', citizen_code: 'citizen_code', full_name: 'full_name', date_of_birth: 'date_of_birth', gender: 'gender', identity_number: 'identity_number', address: 'address', party_card_number: 'party_card_number', joined_party_date: 'joined_party_date', official_party_date: 'official_party_date', branch_name: 'branch_name', parent_party_org: 'parent_party_org', party_position: 'party_position', government_position: 'government_position', education_level: 'education_level', professional_level: 'professional_level', political_theory_level: 'political_theory_level', member_type: 'member_type', party_status: 'activity_status', status_changed_at: 'status_changed_at', status_reason: 'status_reason', decision_number: 'decision_number', decision_date: 'decision_date', transfer_to: 'transfer_to', note: 'note' };
Object.entries(map).forEach(([key, name]) => { if (form.elements[name]) form.elements[name].value = dateInputValue(row[key] ?? ''); });
...
if (subtitle) subtitle.textContent = [row.citizen_code, row.party_card_number, row.branch_name].filter(Boolean).join(' - ');
body.innerHTML =
...
['Mã nhân khẩu', row.citizen_code], ['Số thẻ đảng viên', row.party_card_number], ['Ngày sinh', date(row.date_of_birth)], ['Giới tính', row.gender], ['CCCD', row.identity_number], ['Địa chỉ', row.address], ['Điện thoại', row.phone]
]) +
...
form.elements.date_of_birth.value = dateInputValue(item.date_of_birth || '');
...
state.party_status = $('#partyMemberStatusFilter')?.value || '';
...
Object.assign(state, { page: 1, search: '', branch_name: '', member_type: '', party_status: '', party_position: '', gender: '', age_from: '', age_to: '' });
...
['search','branch_name','member_type','party_status','party_position','gender','age_from','age_to'].forEach(k => { if (state[k]) p.set(k, state[k]); });
return p;
}
function sortBy(key) {
if (!key) return;
...
function dateInputValue(v) { const text = String(v || '').trim(); const m = text.match(/^(\d{4})-(\d{2})-(\d{2})/); return m ? m[3] + '/' + m[2] + '/' + m[1] : text; }
