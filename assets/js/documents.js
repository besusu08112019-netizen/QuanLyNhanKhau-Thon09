(function () {
  ...
  function shell() {
    ...
    host.innerHTML = [
      '<section id="documentsDashboard" class="agri-kpi-grid" aria-label="Thá»‘ng kÃª vÄƒn báº£n"></section>',
      '<section class="agri-filter-card" aria-label="Bá»™ lá»c vÄƒn báº£n"><div class="agri-filter-row">',
      '<div class="agri-field agri-search-field"><label for="documentsSearch">TÃ¬m kiáº¿m</label><div class="module-search-input-wrap"><i class="fa-solid fa-magnifying-glass"></i><input id="documentsSearch" class="form-control" placeholder="TiÃªu Ä‘á», sá»‘ vÄƒn báº£n, Ä‘Æ¡n vá»‹, ngÆ°á»i kÃ½..."></div></div>',
      '<div class="agri-field"><label for="documentsCategoryFilter">Loáº¡i</label><select id="documentsCategoryFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="documentsYearFilter">NÄƒm</label><select id="documentsYearFilter" class="form-select"></select></div>',
      '<div class="agri-field"><label for="documentsPageSize">Hiá»ƒn thá»‹</label><select id="documentsPageSize" class="form-select"><option>20</option><option>50</option><option>100</option></select></div>',
      ...
      '</div></section>',
      '<section class="module-list-card household-list-card"><div class="module-list-head"><div><h3>Danh sÃ¡ch vÄƒn báº£n</h3><span id="documentsTotalCount">Tá»•ng sá»‘: 0 vÄƒn báº£n</span></div><button class="btn btn-success" type="button" data-platform-action="documents.create"><i class="fa-solid fa-plus"></i> ThÃªm vÄƒn báº£n</button></div><div id="documentsLoadState" class="text-muted small px-3 pb-2 d-none"></div><div class="table-responsive"><table class="table module-table align-middle mb-0"><thead><tr><th data-platform-action="documents.sort" data-document-sort="title">TiÃªu Ä‘á»</th><th data-platform-action="documents.sort" data-document-sort="document_number">Sá»‘ vÄƒn báº£n</th><th data-platform-action="documents.sort" data-document-sort="category">Loáº¡i</th><th data-platform-action="documents.sort" data-document-sort="issued_date">NgÃ y ban hÃ nh</th><th data-platform-action="documents.sort" data-document-sort="uploader">NgÆ°á»i táº£i lÃªn</th><th data-platform-action="documents.sort" data-document-sort="created_at">Thá»i gian táº¡o</th><th>File</th><th class="text-end">Thao tÃ¡c</th></tr></thead><tbody id="documentsRows"></tbody></table></div><div id="documentsPager" class="pager module-pager"></div></section>',
      formModal(),
      detailModal()
    ].join('');
    bind();
  ...
  async function catalogs() {
    ...
    fill($('#documentsCategoryFilter'), state.catalogs.categories, 'Táº¥t cáº£');
    fill($('#documentsCategoryInput'), state.catalogs.categories, 'Chá»n loáº¡i');
    fill($('#documentsYearFilter'), state.catalogs.years, 'Táº¥t cáº£');
  ...
  async function load() {
    ...
    try {
      ...
      setLoading(false, error.message || 'KhÃ´ng táº£i Ä‘Æ°á»£c vÄƒn báº£n');
      toast(error.message || 'KhÃ´ng táº£i Ä‘Æ°á»£c vÄƒn báº£n', 'danger');
    } finally {
      state.loading = false;
    }
  }
  ...
  function renderDashboard(data = {}) {
    ...
    const cards = [['Tá»•ng vÄƒn báº£n', 'fa-file-lines', m.total || 0], ['Äang hiá»‡u lá»±c', 'fa-circle-check', m.active_count || 0], ['LÆ°u trá»¯', 'fa-box-archive', m.archived_count || 0], ['30 ngÃ y gáº§n Ä‘Ã¢y', 'fa-clock', m.recent_count || 0]];
  ...
  function renderRows(data = {}) {
    ...
    $('#documentsTotalCount').textContent = `Tá»•ng sá»‘: ${number(data.total || 0)} vÄƒn báº£n`;
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">ChÆ°a cÃ³ vÄƒn báº£n phÃ¹ há»£p bá»™ lá»c.</td></tr>'; return; }
    tbody.innerHTML = rows.map(item => {
      const actions = [
        ...
        item.attachment_count ? `<a class="btn btn-sm btn-outline-success" href="${API}/${item.id}/download" target="_blank" rel="noopener" title="Táº£i xuá»‘ng"><i class="fa-solid fa-download"></i></a>` : '',
        can('update') ? `<button class="btn btn-sm btn-outline-secondary" type="button" data-platform-action="documents.edit" data-id="${item.id}" title="Sá»­a"><i class="fa-solid fa-pen"></i></button>` : '',
        can('delete') ? `<button class="btn btn-sm btn-outline-danger" type="button" data-platform-action="documents.delete" data-id="${item.id}" title="XÃ³a"><i class="fa-solid fa-trash"></i></button>` : ''
      ].filter(Boolean).join(' ');
  ...
  async function openForm(id = null) {
    ...
    if (!form) return toast('KhÃ´ng má»Ÿ Ä‘Æ°á»£c form vÄƒn báº£n', 'danger');
    form.reset();
  ...
  async function save(form) {
    ...
    try {
      ...
      toast('ÄÃ£ lÆ°u vÄƒn báº£n', 'success');
      await load();
    } finally {
  ...
  async function openDetail(id) {
    const item = await request(API + '/' + id);
    state.current = item;
    $('#documentsDetailTitle').textContent = item.title || 'Chi tiáº¿t vÄƒn báº£n';
    ...
    const preview = pdf ? `<iframe src="${safe(pdf.preview_url)}" title="PDF preview" style="width:100%;height:520px;border:1px solid #dee2e6;border-radius:8px"></iframe>` : '<div class="text-muted border rounded p-3">File nÃ y khÃ´ng há»— trá»£ preview trÃªn trÃ¬nh duyá»‡t. Vui lÃ²ng táº£i xuá»‘ng Ä‘á»ƒ xem.</div>';
    ...
    $('#documentsDetailBody').innerHTML = `<div class="row g-3"><div class="col-md-5"><dl class="row mb-0"><dt class="col-sm-4">Sá»‘ vÄƒn báº£n</dt><dd class="col-sm-8">${safe(text(item.document_number))}</dd><dt class="col-sm-4">Loáº¡i</dt><dd class="col-sm-8">${safe(text(item.category_name))}</dd><dt class="col-sm-4">ÄÆ¡n vá»‹</dt><dd class="col-sm-8">${safe(text(item.issuing_unit))}</dd><dt class="col-sm-4">NgÆ°á»i kÃ½</dt><dd class="col-sm-8">${safe(text(item.signer_name))}</dd><dt class="col-sm-4">Ban hÃ nh</dt><dd class="col-sm-8">${date(item.issued_date)}</dd><dt class="col-sm-4">NgÆ°á»i táº£i</dt><dd class="col-sm-8">${safe(text(item.created_by_name))}</dd><dt class="col-sm-4">MÃ´ táº£</dt><dd class="col-sm-8">${safe(text(item.summary))}</dd></dl><h6 class="mt-3">File Ä‘Ã­nh kÃ¨m</h6><div class="work-task-files">${files}</div></div><div class="col-md-7">${preview}</div></div>`;
  ...
  async function remove(id) {
    ...
    if (!window.confirm('XÃ³a vÄƒn báº£n nÃ y? File váº­t lÃ½ sáº½ bá»‹ xÃ³a khá»i uploads.')) return;
    await request(API + '/' + id, { method: 'DELETE' });
    toast('ÄÃ£ xÃ³a vÄƒn báº£n', 'success');
    await load();
  }
