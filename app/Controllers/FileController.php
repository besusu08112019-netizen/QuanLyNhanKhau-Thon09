...
final class FileController extends BaseController
{
    ...
    public function show(string $id): void
    {
        ...
        if (!$file) $this->fail('KhÃ´ng tÃ¬m tháº¥y file', 404);
    ...
    public function update(string $id): void
    {
        try {
            ...
            $this->audit($user, $entityType, 'update_file', 'Cáº­p nháº­t thÃ´ng tin file Ä‘Ã­nh kÃ¨m', $file['entity_id'] ?? null, ['file' => (int) $id]);
    ...
    public function destroy(string $id): void
    {
        ...
        $this->audit($user, $entityType, 'delete_file', 'XÃ³a file Ä‘Ã­nh kÃ¨m', $file['entity_id'] ?? null, ['file' => (int) $id, 'name' => $file['original_name'] ?? $file['file_name'] ?? '']);
        $this->ok(['id' => (int) $id]);
    }
    ...
    private function requireFileMutationPermission(string $entityType): array
    {
        ...
        if (!$this->users()->can($user, $module, 'read') || !$this->users()->can($user, 'file', 'delete')) {
            $this->fail('KhÃ´ng cÃ³ quyá»n xÃ³a file', 403);
        }
        return $user;
    }
