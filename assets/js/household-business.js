(() => {
  const state = { page: 1, pageSize: 20, search: '', business_type: '', sector: '', status: '', license: '', tax: '', located: '', sort: 'household_code', direction: 'ASC' };
  ...
  async function save(event) {
    ...
    delete payload.id;
    try {
      await request(id ? '/api/household-business/' + encodeURIComponent(id) : '/api/household-business', { method: id ? 'PUT' : 'POST', body: payload });
  ...
  async function renderDashboard() {
    ...
    try {
      ...
      const cards = [
        ...
        ['Tá»•ng lao Ä‘á»™ng', metrics.business_worker_total, 'ngÆ°á»i']
      ];
  ...
  function addReportOptions() {
    ...
    [
      ...
      ['household-business-gis', 'Theo khu vá»±c GIS SX/KD']
