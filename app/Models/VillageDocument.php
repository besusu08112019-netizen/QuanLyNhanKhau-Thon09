...
SQL);
    ...
    public function catalogs(): array
    {
        ...
        return [
            ...
            'statuses' => [['value' => 'ACTIVE', 'label' => 'Äang hiá»‡u lá»±c'], ['value' => 'ARCHIVED', 'label' => 'LÆ°u trá»¯']],
    ...
    {
        $this->ensureSchema();
        if ($id && !$this->find($id)) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y vÄƒn báº£n');
    ...
    {
        $row = $this->find($id);
        if (!$row) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y vÄƒn báº£n');
    ...
    {
        if (!$this->find($id)) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y vÄƒn báº£n');
    ...
    public function deleteAttachment(int $documentId, int $fileId, int $userId): ?array
    {
        ...
        if (!$file) throw new RuntimeException('KhÃ´ng tÃ¬m tháº¥y file Ä‘Ã­nh kÃ¨m');
    ...
    public function report(array $filters = []): array
    {
        ...
        return [
            'title' => 'BÃ¡o cÃ¡o vÄƒn báº£n',
            'headers' => ['Ma', 'Sá»‘ vÄƒn báº£n', 'TiÃªu Ä‘á»', 'Loáº¡i', 'ÄÆ¡n vá»‹ ban hÃ nh', 'NgÆ°á»i kÃ½', 'NgÃ y ban hanh', 'NgÆ°á»i táº£i lÃªn', 'Táº¡o lÃºc'],
    ...
    private function params(array $data, int $userId): array
    {
        ...
        if ($title === '') throw new RuntimeException('TiÃªu Ä‘á» van ban la bat buoc');
    ...
    private function normalize(array $row): array
    {
        ...
        return [
            ...
            'status' => $status,
            'status_label' => $status === 'ARCHIVED' ? 'LÆ°u trá»¯' : 'Äang hiá»‡u lá»±c',
    ...
    {
        $items = [['notice', 'ThÃ´ng bÃ¡o'], ['decision', 'Quyáº¿t Ä‘á»‹nh'], ['official_dispatch', 'CÃ´ng vÄƒn'], ['plan', 'Káº¿ hoáº¡ch'], ['report', 'BÃ¡o cÃ¡o'], ['minutes', 'BiÃªn báº£n'], ['other', 'KhÃ¡c']];
        $order = 10;
