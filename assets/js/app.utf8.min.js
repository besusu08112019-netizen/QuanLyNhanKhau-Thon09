...
const nativeZoom = Number(options?.maxNativeZoom);
const originalGetTileUrl = typeof layer.getTileUrl === 'function' ? layer.getTileUrl.bind(layer) : null;
layer.getTileUrl = function(coords) {
const source = Object.assign({}, coords || {});
const z = Number(source.z);
if (Number.isFinite(nativeZoom) && Number.isFinite(z) && z > nativeZoom) {
const scale = Math.pow(2, z - nativeZoom);
source.z = nativeZoom;
source.x = Math.floor(Number(source.x || 0) / scale);
source.y = Math.floor(Number(source.y || 0) / scale);
}
if (originalGetTileUrl) return originalGetTileUrl(source);
return String(url || '').replace('{z}', source.z).replace('{x}', source.x).replace('{y}', source.y);
};
