...
async function fetchJson(path){var tk=token();if(!tk)throw new Error('Phien dang nhap da het han. Vui long dang nhap lai.');var res=await fetch(path,{headers:{Accept:'application/json',Authorization:'Bearer '+tk},cache:'no-store'});var json=await res.json().catch(function(){return null;});if(!res.ok||!json||!json.ok)throw new Error((json&&json.error&&json.error.message)||'Khong tai duoc bao cao.');return json.data||{};}
function setTitle(text){var el=q('#reportTitle');if(el)el.textContent=text||'Bao cao';}
function setCount(report){var el=q('#reportCount');if(!el)return;var rows=Number(report&&report.totalRows!=null?report.totalRows:(report&&report.rows?report.rows.length:0));el.textContent='Tong so: '+rows.toLocaleString('vi-VN')+' dong';}
function setActions(show){var el=q('#reportActions');if(el)el.classList.toggle('d-none',!show);}
function table(report){var headers=report.headers||[];var rows=report.rows||[];if(!headers.length)return '<div class="report-empty-state">Bao cao chua co cau truc hien thi.</div>';var head=headers.map(function(h){return '<th>'+esc(h)+'</th>';}).join('');var body=rows.length?rows.map(function(row){return '<tr>'+row.map(function(cell){return '<td>'+esc(cell)+'</td>';}).join('')+'</tr>';}).join(''):'<tr><td colspan="'+headers.length+'" class="text-center text-muted py-4">Khong co du lieu</td></tr>';return '<table class="table report-table align-middle mb-0"><thead><tr>'+head+'</tr></thead><tbody>'+body+'</tbody></table>';}
...
async function viewReport(){setActions(false);setTitle('Bao cao');var count=q('#reportCount');if(count)count.textContent='Dang tai du lieu...';showMessage('Dang sinh bao cao...','info');try{var report=await fetchJson(apiUrl('/api/reports/summary'));currentReport=report;setTitle(report.title||'Bao cao');setCount(report);var preview=q('#reportPreview');if(preview)preview.innerHTML=table(report);setActions(true);return report;}catch(e){currentReport=null;setTitle('Bao cao');if(count)count.textContent='Khong sinh duoc bao cao';showMessage(e.message||'Khong sinh duoc bao cao.','danger');throw e;}}
async function ensureReport(){return currentReport||viewReport();}
function download(kind){var tk=token();if(!tk){showMessage('Phien dang nhap da het han. Vui long dang nhap lai.','danger');return;}var url=apiUrl(kind==='excel'?'/api/reports/export-excel':'/api/reports/export-pdf');fetch(url,{headers:{Authorization:'Bearer '+tk},cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('Khong xuat duoc file.');return res.blob().then(function(blob){var name=(currentReport&&currentReport.title?currentReport.title:'bao_cao').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/d/g,'d').replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'')||'bao_cao';var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name+(kind==='excel'?'.xls':'.pdf');document.body.appendChild(a);a.click();URL.revokeObjectURL(a.href);a.remove();});}).catch(function(e){showMessage(e.message||'Khong xuat duoc file.','danger');});}
...
function ensurePersonActionButtons(){var rows=qs('#personRows');if(!rows)return;var canEdit=canPersonAction('update'),canDelete=canPersonAction('delete');qsa('[data-platform-action="persons.detail"]',rows).forEach(function(viewBtn){var id=viewBtn.dataset.id||viewBtn.dataset.personId||'';var cell=viewBtn.closest('td');if(!id||!cell)return;if(canEdit&&!cell.querySelector('[data-platform-action="persons.edit"]'))viewBtn.insertAdjacentHTML('afterend',' <button class="btn btn-sm person-row-btn person-row-edit" type="button" data-platform-action="persons.edit" data-id="'+escapeHtml(id)+'">Sửa</button>');if(canDelete&&!cell.querySelector('[data-platform-action="persons.delete"]'))cell.insertAdjacentHTML('beforeend',' <button class="btn btn-sm btn-outline-danger" type="button" data-platform-action="persons.delete" data-id="'+escapeHtml(id)+'">Xóa</button>');});}
window.loadPersons=async function loadPersonsAdvanced(){try{var searchText=normalizeSearchText((qs('#personSearch')&&qs('#personSearch').value||App.persons.search||'').trim());App.persons.search=(qs('#personSearch')&&qs('#personSearch').value||'').trim();var items=[],total=0;var data=await api('/api/persons?'+personParams(true).toString(),{cacheTtl:12000});items=data.items||[];total=data.total||0;var totalEl=qs('#personTotalCount');if(totalEl)totalEl.innerHTML='Tong so: <strong>'+number(total)+'</strong> nhan khau';var rows=qs('#personRows');if(rows){rows.innerHTML=renderPersonRows(items);ensurePersonActionButtons();}if(typeof refreshUiEnhancements==='function')refreshUiEnhancements(qs('#personsScreen')||document);if(typeof updateBulkDeleteButtons==='function')updateBulkDeleteButtons();renderPager('#personPager',{total:total,page:App.persons.page,pageSize:App.persons.pageSize},function(page){App.persons.page=page;window.loadPersons();});}catch(error){showToast('Khong tai duoc danh sach nhan khau: '+error.message,'danger');}};
...
function renderLoaderError(screen,loader){var target=targetFor(screen);if(!target||target.id!==screen+'Screen'||target.dataset.loaderErrorRendered==='1')return;target.dataset.loaderErrorRendered='1';var host=target.querySelector('.module-placeholder-screen, .content-card, .dashboard-kpi-grid, .agri-kpi-grid')||target;host.innerHTML='<div class="empty-state text-muted py-4 text-center">Khong tai duoc module. Vui long tai lai trang hoac kiem tra file JavaScript: '+loader+'</div>';}
