...
final class ProfileSummary extends BaseModel
{
    ...
    private function partyMember(int $citizenId): ?array
    {
        ...
        $row = $this->fetchOne(
            'SELECT pm.id, pm.party_card_number, pm.branch_name, pm.party_position, pm.member_type, pm.activity_status, pm.party_status, pm.joined_party_date, pm.official_party_date
        ...
        return [
            ...
            'summary' => trim(implode(' | ', array_filter([
                $row['branch_name'] ?? '',
                $row['party_position'] ?? '',
                $row['party_card_number'] ? 'Số thẻ: ' . $row['party_card_number'] : '',
            ]))),
    ...
    {
        if (!$this->tableExists('household_poverty_records') || !$this->tableExists('poverty_periods')) return null;
        $row = $this->fetchOne(
            'SELECT hpr.id, hpr.poverty_type, hpr.status, hpr.effective_from, hpr.effective_to, pp.name AS period_name
             FROM household_poverty_records hpr
             INNER JOIN poverty_periods pp ON pp.id = hpr.period_id AND pp.village_id = hpr.village_id
             INNER JOIN households h ON h.id = hpr.household_id AND h.village_id = hpr.village_id
             WHERE hpr.household_id = :household_id
               AND hpr.status = "ACTIVE"
               AND pp.status <> "DELETED"
               AND hpr.effective_from <= CURDATE()
               AND (hpr.effective_to IS NULL OR hpr.effective_to >= CURDATE())
               AND ' . $this->tenantWhere('hpr', 'household_poverty_records') . '
               AND ' . $this->tenantWhere('pp', 'poverty_periods') . '
               AND ' . $this->tenantWhere('h', 'households') . '
             ORDER BY hpr.effective_from DESC, hpr.id DESC LIMIT 1',
            $this->withTenant(['household_id' => $householdId])
        );
        if (!$row) return null;
    ...
    private function householdCategory(int $householdId): ?array
    {
        ...
        return [
            'poverty_type' => $household['poverty_type'] ?? null,
