...
return Object.assign({ openPopupId: App.gis?.openPopupId || '', selectedHouseholdId: App.gis?.selectedHouseholdId || '', hasMap: Boolean(App.gis?.map), markerCount: App.gis?.markerCache?.size || 0, lastViewportKey: App.gis?.lastViewportKey || '' }, extra || {});
}
...
marker.on('click', () => { gisDebugTrace('marker click -> openGisHouseholdPopup()', { id }); openGisHouseholdPopup(item.id, marker); });
['touchstart', 'touchend', 'pointerdown', 'pointerup'].forEach(type => marker.on(type, () => gisDebugTrace('marker ' + type, { id })));
marker.on('popupopen', () => {
...
if (App.gis.selectedHouseholdId === id) App.gis.selectedHouseholdId = '';
});
...
if (App.gis.selectedHouseholdId !== String(id) || App.gis.openPopupId !== String(id)) return;
App.gis.openPopupId = String(id);
