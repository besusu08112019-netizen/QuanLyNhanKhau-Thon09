(() => {
  ...
  function businessActivityDetailPanel(activity, index) {
    ...
    return '<article class="business-person-activity-panel business-dashboard-activity-view ' + (index === 0 ? 'is-active' : '') + '" data-business-activity-panel="' + index + '">'
      ...
      + businessDashboardSection('Thông tin', 'fa-circle-info', [
      ...
      + businessDashboardSection('Lao động & doanh thu', 'fa-users-gear', '<div class="business-dashboard-metrics">' + businessDashboardMetric(num(activity.worker_count || 0), 'người lao động') + businessDashboardMetric(activity.annual_revenue ? num(activity.annual_revenue) : '0', 'doanh thu') + '</div>' + [
  ...
  function businessDashboardMetric(value, label) {
    return '<div class="business-dashboard-metric"><strong>' + esc(value) + '</strong><span>' + esc(label) + '</span></div>';
  }
  ...
  function businessCertificationBadges(activity) {
    return '<div class="business-dashboard-certificates">'
      + flagBadge(businessTruthy(activity.is_ocop), businessTruthy(activity.is_ocop) ? (activity.ocop_star ? 'OCOP ' + activity.ocop_star + ' sao' : 'OCOP') : 'OCOP ✕')
      + flagBadge(businessTruthy(activity.food_safety_certified), businessTruthy(activity.food_safety_certified) ? 'ATTP ✓' : 'ATTP ✕')
      + flagBadge(businessTruthy(activity.social_insurance), businessTruthy(activity.social_insurance) ? 'BHXH ✓' : 'BHXH ✕')
      + '</div>';
  }
