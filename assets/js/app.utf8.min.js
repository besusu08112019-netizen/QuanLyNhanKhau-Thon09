...
function gisAreaStyle(area, active = false) {
return active ? { color: '#1976D2', fillColor: '#1976D2', fillOpacity: .45, weight: 4 } : { color: '#2E7D32', fillColor: '#2E7D32', fillOpacity: .2, weight: 2 };
}
...
const area = layer.__gisArea || (App.gis.areas || []).find(item => String(item.id) === String(id));
if (layer.setStyle) layer.setStyle(gisAreaStyle(area, active));
...
App.gis.selectedAreaId = String(area.id || '');
...
if (layer && !App.gis.areaLayerMap?.has(String(area.id))) App.gis.areaLayerMap?.set(String(area.id), layer);
...
const area = (App.gis.areas || []).find(item => String(item.id) === String(id));
const layer = App.gis.areaLayerMap?.get(String(id));
...
const polygon = L.polygon(latLngs, gisAreaStyle(area, String(area.id) === String(App.gis.selectedAreaId || '')));
polygon.__gisArea = area;
App.gis.areaLayerMap.set(String(area.id), polygon);
polygon.bindPopup(gisAreaPopup(area));
if ((data.areas || []).length <= 30) polygon.bindTooltip(gisTooltip(area), { permanent: true, direction: 'center', className: gisAreaTooltipClass(String(area.id) === String(App.gis.selectedAreaId || '')) });
...
const active = String(area.id) === String(App.gis.selectedAreaId || '');
return '<article class="gis-area-item' + (active ? ' is-active' : '') + '" data-gis-area-id="' + Number(area.id) + '" aria-selected="' + (active ? 'true' : 'false') + '"><button class="gis-area-main" type="button"><span><b>' + gisEscape(area.name) + '</b><small>' + gisEscape(area.area_code) + '</small></span><em>' + gisNumber(area.stats?.households) + ' h\u1ed9 / ' + gisNumber(area.stats?.citizens) + ' NK</em></button><div class="gis-area-row-actions">' + (canAccess('gis','update') ? '<button class="btn btn-sm btn-outline-primary" type="button" onclick="editGisArea(' + Number(area.id) + ')"><i class="fa-solid fa-pen-to-square"></i></button>' : '') + (canAccess('gis','delete') ? '<button class="btn btn-sm btn-outline-danger" type="button" onclick="deleteGisArea(' + Number(area.id) + ')"><i class="fa-solid fa-trash"></i></button>' : '') + '</div></article>';
...
const area = (App.gis.areas || []).find(item => String(item.id) === String(areaId));
...
const layer = App.gis.areaLayerMap?.get(String(area.id));
