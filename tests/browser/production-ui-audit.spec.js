...
async function mockApis(page) {
  await page.route('**/api/**', async (route) => {
    ...
    if (url.includes('/api/public/login-config')) return fulfill({ settings: { systemName: 'Hệ thống quản lý hành chính', hamletName: 'Thôn mẫu', communeName: 'Xã cấu hình', version: 'v2.0' }, metrics: {} });
