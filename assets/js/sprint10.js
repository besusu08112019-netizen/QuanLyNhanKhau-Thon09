(() => {
  ...
  async function saveUser10(event) {
    ...
    closeModal('userModal'); showToast('Đã lưu tài khoản'); loadUsers10();
  }
  async function resetUserPassword10(id) {
    ...
    const password = prompt('Nhap mat khau moi toi thieu 8 ky tu'); if (!password) return;
    await api('/api/users/' + id + '/reset-password', { method: 'POST', body: { password } }); showToast('Da dat lai mat khau');
  }
