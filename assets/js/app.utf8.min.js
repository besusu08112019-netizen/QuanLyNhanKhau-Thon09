...
App.gis.selectedArea = area;
const idEl = $('#gisAreaId'); if (idEl) idEl.value = area.id || '';
const name = $('#gisAreaName'); if (name) name.value = area.name || '';
const code = $('#gisAreaCode'); if (code) code.value = area.area_code || '';
const note = $('#gisAreaNote'); if (note) note.value = area.note || '';
const color = $('#gisAreaColor'); if (color) color.value = area.color || '#0f8a4b';
...
gisRemoveDrawnLayer();
App.gis.editingArea = null;
App.gis.activeDrawHandler = new L.Draw.Polygon(App.gis.map, { allowIntersection: false, showArea: true, shapeOptions: { color: $('#gisAreaColor')?.value || '#0f8a4b' } });
...
const payload = { id: areaId || undefined, name: $('#gisAreaName')?.value.trim(), area_code: $('#gisAreaCode')?.value.trim(), color: $('#gisAreaColor')?.value || '#0f8a4b', note: $('#gisAreaNote')?.value.trim(), geometry: latLngs };
...
polygon.on('click', () => selectGisArea(area, polygon));
polygon.addTo(group);
...
const latLngs = gisPolygonLatLngs(area);
if (latLngs.length) { gisDebugTrace('map.fitBounds() focusGisArea', { areaCode }); App.gis.map.fitBounds(latLngs, { padding: [28, 28], maxZoom: 17 }); }
}
