(function () {
  ...
  async function renderDashboard() {
    ...
    host.innerHTML = [
      ...
      ['Hộ trung bình', m.medium, 'fa-house-user'],
    ...
    if (trend) trend.innerHTML = (data.trend || []).length ? (data.trend || []).map(row => '<div class="d-flex justify-content-between border-bottom py-1"><span>' + esc(row.year) + '</span><strong>Nghèo: ' + num(row.poor) + ' · Cận nghèo: ' + num(row.near_poor) + ' · Trung bình: ' + num(row.medium) + '</strong></div>').join('') : '<div class="text-muted">Chưa có dữ liệu biến động theo năm.</div>';
  }
  ...
  function fill(selector, items, first) {
    ...
    el.innerHTML = '<option value="">' + esc(first || 'Chọn') + '</option>' + (items || []).map(item => '<option value="' + esc(item.value) + '">' + esc(catalogLabel(selector, item)) + '</option>').join('');
    el.value = current;
  }
