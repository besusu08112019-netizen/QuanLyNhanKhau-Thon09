...
attach() { bindListeners(); if (group && typeof group.addTo === 'function' && map && !map.hasLayer?.(group)) group.addTo(map); return group; },
replace(markers) {
...
if (group && typeof group.addLayers === 'function') group.addLayers(list);
else if (group && typeof group.addLayer === 'function') list.forEach(marker => group.addLayer(marker));
refresh();
},
refresh,
debugState() {
...
return { useCluster, markerCount: markersById.size, coordinateBucketCount: coordinateBuckets.size, duplicateCoordinateBuckets: duplicateBuckets.length, listenerCount: clusterListenerCount, markerLoadCycles, options: group?.options || {} };
}
};
}
