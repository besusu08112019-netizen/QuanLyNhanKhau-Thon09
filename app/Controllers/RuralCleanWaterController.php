...
final class RuralCleanWaterController extends BaseController
{
    ...
    public function show(string $id): void
    {
        ...
        if (!$row) $this->fail('Không tìm thấy bản ghi nước sạch', 404);
        $this->ok($row);
    }
    ...
    public function store(): void
    {
        ...
        $this->requireInputFields($input, ['household_id' => 'Hộ gia đình']);
        $row = $this->water->upsert($input, (int) $user['id']);
        $this->audit($user, 'rural_clean_water', 'create', 'Thêm bản ghi nước sạch nông thôn', $row['id'], ['before' => null, 'after' => $row]);
        $this->ok($row);
    }
    ...
    public function update(string $id): void
    {
        ...
        if (!$before) $this->fail('Không tìm thấy bản ghi nước sạch', 404);
        $row = $this->water->upsert((array) $this->input(), (int) $user['id'], (int) $id);
        $this->audit($user, 'rural_clean_water', 'update', 'Cập nhật bản ghi nước sạch nông thôn', $id, ['before' => $before, 'after' => $row]);
        $this->ok($row);
    }
    ...
    public function destroy(string $id): void
    {
        ...
        $this->audit($user, 'rural_clean_water', 'delete', 'Xóa bản ghi nước sạch nông thôn', $id, ['before' => $before, 'after' => null]);
        $this->ok(['id' => (int) $id]);
    }
